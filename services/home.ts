...
class HomeService {
  ...
  async subscribeToTopic({ topic }: { topic: string }) {
    ...
    let unsubscribedTopics =
      JSON.parse(localStorage.getItem("unsubscribedTopics")) || [];
  ...
  async handleTopicsOnPageRefresh() {
    ...
    const uniqueUpdatedUnsubscribedTopics = Array.from(
      new Set(updatedUnsubscribedTopics)
    );
    ...
    localStorage.setItem(
      "unsubscribedTopics",
      JSON.stringify(uniqueUpdatedUnsubscribedTopics)
    );
    ...
    for (const newTopic of updatedTopics) {
      if (
        !storageTopics.includes(newTopic) &&
        !unsubscribedTopics.includes(newTopic)
      ) {
        await fetch("/api/subscribeToTopic", {
          method: "POST",
          body: JSON.stringify({
