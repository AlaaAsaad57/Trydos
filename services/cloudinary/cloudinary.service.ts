...
export class CloudinaryService {
   ...
   */
  async uploadSitemap(filePath: string, publicId: string): Promise<UploadResult | null> {
    try {
      ...
      console.error('[CloudinaryService] Failed to upload sitemap to Cloudinary', {
        public_id: publicId,
   ...
   * @returns Promise<UploadResult | null> The upload result or null on failure
   */
  async uploadSitemapContent(content: string, publicId: string): Promise<UploadResult | null> {
    try {
      ...
      console.error('[CloudinaryService] Failed to upload sitemap content to Cloudinary', {
        public_id: publicId,
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async getSitemapUrl(publicId: string): Promise<string | null> {
    try {
      ...
      console.error('[CloudinaryService] Failed to get sitemap URL from Cloudinary', {
        public_id: publicId,
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async deleteSitemap(publicId: string): Promise<boolean> {
    try {
      ...
      const result = await cloudinary.uploader.destroy(`${this.folder}/${publicId}`, {
        resource_type: 'raw',
      });
      ...
      console.error('[CloudinaryService] Failed to delete sitemap from Cloudinary', {
        public_id: publicId,
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async listSitemaps(): Promise<AssetResult[]> {
    try {
      ...
      console.log(`[CloudinaryService] Found ${result.resources.length} sitemaps`);
