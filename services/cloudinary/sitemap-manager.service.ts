...
export class SitemapManagerService {
  private readonly sitemapTypes = ['home', 'products', 'static', 'search'] as const;

  /**
   * Generate and upload all sitemaps to Cloudinary
   */
  async generateAndUploadAllSitemaps(): Promise<SitemapManagerResult> {
    console.log('[SitemapManager] Starting generation and upload of all sitemaps');
    ...
    try {
      ...
      console.error('[SitemapManager] Error in generateAndUploadAllSitemaps:', error);
   ...
   */
  async generateAndUploadSitemap(type: 'home' | 'products' | 'static' | 'search'): Promise<SitemapUploadResult> {
    try {
      ...
      const uploadResult = await cloudinaryService.uploadSitemapContent(xmlContent, publicId);
      ...
      console.error(`[SitemapManager] Error generating/uploading ${type} sitemap:`, error);
  ...
  async generateAndUploadSitemapIndex(): Promise<SitemapUploadResult> {
    try {
      ...
      // Upload to Cloudinary
      const uploadResult = await cloudinaryService.uploadSitemapContent(xml, publicId);
      ...
      console.error('[SitemapManager] Error generating/uploading sitemap index:', error);
