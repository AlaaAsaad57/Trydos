describe.only("Should add product to cart from any boutique page", () => {
  ...
  it("Check your boutique components on the home page", () => {
    cy.ChooseBoutiqueAndOpenItsPage();
  });
  it("Verify your boutique components on the boutique page.", () => {
    cy.checkOutBoutiquePage();
  });
  it("Should Add Product To Cart From boutique Page", () => {
    cy.AddProductToCartFromBoutiquePage();
  });
  it("Should verify components if the product has colors and sizes", () => {
    cy.verifyColorsAndSizesIfFounded();
  });
  it("Should verify extended area components", () => {
    cy.verifyExtendedArea();
  });
  ...
  it("Should Get The Name Of Product That Was Added To cart To Compare It With The Name Obtained Previously", () => {
    cy.get("[data-cy=productNameInCart]")
      .invoke("text")
      .then((text) => {
        const productNameInCart = text as string;
        cy.log(
