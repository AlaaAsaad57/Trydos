...
| `workflow.capabilities` | no | Owner-selected capabilities engaged for **this work item**. A tag here does nothing unless a stage of this workflow also declares it and `capabilities/<tag>/` exists; an empty list is the default. | list of capability ids, e.g. `[tdd]` |
| `status` | yes | Orthogonal health and terminal status. | `active` \| `blocked` \| `completed` \| `cancelled` |
| `active_blocker_id` | when blocked | Identity of the blocker halting progress. Set with `status: blocked`; cleared on resume. | e.g. `BLK-ACCESS-01` |
...
| `links` | no | Optional delivery links (metadata only; never workflow state). `github` is set by `/mw:publish-pr`. | `{clickup, github}` URLs (may be empty) |

Stage ids and their legal transitions are defined canonically per workflow in
`workflows/<type>/workflow.yaml`. This item runs `development`:
...
- stage: implement
  ...
  timestamp: 2026-09-19
  blocker_id: BLK-PLAN-01
  evidence_ref: "plan.md > Plan check > Round 8 (B-1..B-4 closed in steps 9b, 10, 12c, 12d, 13b, 15); CLAUDE.md:319-333 (B-5: base branch corrected to development)"
