...
created_at: 2026-07-26   # YYYY-MM-DD
updated_at: 2026-07-26   # YYYY-MM-DD (bumped on every state change)
...
# Ticket Record — unify-delivery-day-calculation
...
- state: spec-complete
  ...
  by: developer
  timestamp: 2026-07-26
# Owner-directed manual transition, performed outside the command set: no
# command implements spec-complete -> research-complete, though the lifecycle
# permits it. Reason: after four review rounds whose findings all landed in
# transition-state behaviour, the owner elected to narrow the ticket to the
# defect reported at intake and remove AC-11, which was introduced during /spec
# rather than requested. spec.md was re-authored in the same action.
- state: research-complete
  event: spec-reopened
  by: developer
  timestamp: 2026-07-26
...
- state: spec-complete
  ...
  by: developer
  timestamp: 2026-07-26
# Front-matter correction: the /plan initial run after the spec reopen appended
# the `spec-validated` history entry above but failed to update the front-matter
# `state:` field, which remained `research-complete` (a TS-4 omission). The two
# `plan-revised` entries and the /review gate below were therefore recorded while
# the front-matter disagreed with the history. The history is authoritative on
# what occurred; the field is corrected here as part of the APPROVED transition.
- state: plan-complete
