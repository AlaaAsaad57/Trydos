...
import { fetchCurrency, getProductDataForAddToCart } from "serverRequests";
...
async function GetProductDataFunc(params) {
  ...
  try {
    ...
    if (data?.product && data?.product?.images) {
      ...
      let productData = await getProductDataForAddToCart({
        slug: slug,
        language,
        country,
      });

      if (productData?.offer_price && productData?.images) {
