// The two boutique pages of the seller dashboard: open one boutique, and add a
// new one. Each page reads the route, sets the text direction, builds the back
// link to the dashboard, and hands the ids to the boutique editor.
...
vi.mock("components/SellerDashboard/boutiqueEdit/BoutiqueEditor", () => ({
...
import EditPage from "app/(client)/[lang]/sellerProfile/sellerDashboard/[sellerId]/boutiques/[boutiqueId]/page";
import NewPage from "app/(client)/[lang]/sellerProfile/sellerDashboard/[sellerId]/boutiques/new/page";

describe("seller boutique pages", () => {
  it("the edit page passes the boutique and seller ids and links back to the dashboard", async () => {
    const { container } = await renderWithProviders(<EditPage />, {
      country: "sy",
      params: { sellerId: "77", boutiqueId: "12" },
    });
    const bar = screen.getByTestId("backbar");
    expect(bar.dataset.name, "the edit page title is wrong").toBe("Boutique");
  ...
  it("the edit page in Kurdish is right-to-left", async () => {
    const { container } = await renderWithProviders(<EditPage />, {
      country: "iq",
      language: "ku",
      params: { sellerId: "1", boutiqueId: "2" },
    });
