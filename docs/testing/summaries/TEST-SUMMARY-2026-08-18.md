# Test summary — 18 August 2026

**This run covers how much of the app is checked. Every check passes, and just under a tenth of the app's code is reached by a test.**

| | |
|---|---|
| **New checks added this time** | 156 |
| **Checks in the app in total** | 1210 |
| **Result** | ✅ All passing |
| **How much of the app is checked** | 9.8% of the code |
| **Date** | 2026-08-18 |

This time we were asked for the coverage picture only, so the new checks are counted
above but not described one by one.
...
|---|---|---|---|
| Lines of code | 2729 | 27942 | 9.8% |
| Decision points | 2080 | 25151 | 8.3% |
| Functions | 416 | 7135 | 5.8% |

Of the 726 parts of the app in this report, 30 have a test written for them on purpose.
Another 56 are only touched in passing, and 640 have nothing at all.

### The parts we set out to test

| Part of the app | Share checked |
|---|---|
| Sending every visitor to the right language and country — proxy.ts | 100.0% |
...
| Not asking the same thing twice at once — serverRequests/requestDedup.ts | 100.0% |
...
| Stopping code requests being spammed — utils/otpLocks.ts | 100.0% |
| The settings the shop starts with — utils/startingSettings.ts | 100.0% |
| Renewing a session behind the scenes — utils/server/authRefresh.ts | 100.0% |
| Recording how code sending behaves — utils/server/otpTelemetry.ts | 100.0% |
| Deciding which service a pass belongs to — utils/server/tokenManager.ts | 98.9% |
| Fetching data from the browser — utils/fetchData.ts | 98.7% |
| Knowing who a code was sent to — utils/server/otpIdentity.ts | 98.4% |
| Turning a breakage into a safe message — utils/errorSerialization.ts | 97.4% |
| Sending a one-time code — serverActions/sendOtp.ts | 97.3% |
| Remembering the filters a shopper picked — utils/listing/filterItemState.ts | 97.0% |
| Shared helpers used on the server — utils/server/helpers.ts | 95.6% |
| Signing a shopper in — app/api/auth/login/route.ts | 94.1% |
| Ending a session that has expired — app/api/auth/expire/route.ts | 93.5% |
| Clearing a single service's pass — app/api/auth/clear-tokens/route.ts | 87.5% |
| Signing a shopper out — app/api/auth/logout/route.ts | 81.5% |
| Passing a request on to a backend — app/api/proxy/route.ts | 73.3% |
| Small shared helpers — utils/tinyUtils.tsx | 63.8% |
| Search helpers — services/elastic/helpers.ts | 51.8% |
| Short-term storage — serverRequests/radis/index.ts | 19.3% |

### Reading these numbers

- **What is checked well:** signing in, signing out, sessions and passes, sending a
  visitor to the right language and country, and the shared price and fetch helpers —
  most of these are checked end to end.
- **What has nothing yet:** the screens people actually see. All 478 of them are at
  0.1%, and the small helpers behind the screens and the page-scaling code have nothing
  at all.
