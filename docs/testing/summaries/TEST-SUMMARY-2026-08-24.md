# Test summary — 24 August 2026

**This run adds 263 new checks, almost all of them on signing in, phone numbers and the profile. Every check passes, and about an eighth of the app's code is now reached by a test.**

| | |
|---|---|
| **New checks added this time** | 263 |
| **Checks in the app in total** | 1474 |
| **Result** | ✅ All passing |
| **How much of the app is checked** | 13.0% of the code |
| **Date** | 2026-08-24 |
...
### Sending a visitor to the right page

- When a visitor opens /privacy-policy, then the address is left alone, not read as a country and language.
- When a visitor opens /terms-of-service, then the address is left alone, not read as a country and language.
- When a visitor opens /gift-cards/buy, then the address is left alone, not read as a country and language.
- When the pre-launch setting is not turned on, then the storefront answers as normal.
- When the pre-launch setting is on, then the root address shows the landing page.
- When the pre-launch setting is on and another address is opened, then the visitor goes to the landing page.
- When the visitor is sent to the landing page, then the move is temporary and no browser keeps it.
- When an address carries no language, then the pre-launch check answers first and no language is added.
- When the pre-launch page is showing, then the app contacts no backend at all.

### Codes sent to a tester's number

- When a number is on the tester list, then the send skips the rate limit and the real code still goes out.
- When a tester number is written with a leading plus, then it is still matched on its digits alone.
- When a tester number is written with spaces and brackets, then it is still matched on its digits alone.
- When the tester list holds several numbers, then a number listed among them is still matched.
- When a tester number is written with untidy separators, then it is still matched on its digits alone.
- When a number is not on the tester list, then it gets no exemption.
- When no tester list is configured at all, then no number gets an exemption.
- When the backend itself refuses the send, then no cooldown is placed on the number.
- When a send goes to a tester number, then it is still recorded, and named as a test number.

### Recognising a tester's number in the list

- When the list names a number exactly as configured, then it is recognised.
- When the list writes a number with a leading plus, then it is recognised.
- When the number itself carries a leading plus, then it is recognised.
- When the list writes a number with punctuation, then it is recognised.
- When the number itself carries punctuation, then it is recognised.
- When the list holds several numbers, then one entry among them is recognised.
- When the list uses untidy separators, then its numbers are still recognised.
- When a number is simply not in the list, then it is not recognised.
- When a longer number contains a listed one, then it is not recognised.
- When a number is only the start of a listed one, then it is not recognised.
- When the list is not set at all, then nobody is exempt.
- When the list is set but empty, then nobody is exempt.
- When the list is only separators, then nobody is exempt.
- When the list is only punctuation, then nobody is exempt.
- When the number to check is empty, then the answer is no, whatever the list holds.
- When the list is changed, then the new value is followed rather than the first one read.

### Showing a phone number properly

- When a Syrian number is shown, then it is printed in the usual international groups.
- When a Turkish number is shown, then it is printed in the usual international groups.
- When a Lebanese number is shown, then it is printed in the usual international groups.
- When an Iraqi number is shown, then it is printed in the usual international groups.
- When an Emirati number is shown, then it is printed in the usual international groups.
- When a number arrives full of brackets and dashes, then they are stripped and the digits regrouped.
- When a number arrives as a plain number rather than text, then it is still printed correctly.
- When the country code is one we do not know, then the number is shown as a plus and its digits.
- When a known country has no grouping of its own, then its digits are shown in threes.
- When there is nothing after the country code, then only the country code is shown.
- When the value is empty, then nothing is shown.
- When there are fewer than four digits, then nothing is shown.
- ⚠️ When exactly four digits are given, then they print as a number — recorded as it is today, not approved.
- When a number is given with no international prefix, then the prefix is added.
- ⚠️ When a national leading zero is present, then it stays — recorded as it is today, not approved.
- When a plus is typed in the middle of a number, then only the leading one is kept.

### Deciding a phone number can be used

- When a normal international number is checked, then it is accepted.
- When a number carries spaces, dashes and a plus, then it is still accepted.
- When a number arrives as a plain number rather than text, then it is still accepted.
- When the value is empty, then it is rejected.
- When the value has no digits at all, then it is rejected.
- When the value is one of the placeholders people type before a country code, then it is rejected.
- When a number is too short, then it is rejected.
- When a number is too long, then it is rejected.
- When a number sits at either end of the ten-to-fifteen digit range, then it is accepted.
- When a value is only brackets and dashes, then it is rejected.

### Typing a phone number

- When a number is typed with a plus, then it is read as the same number.
- When a number is typed with a double zero, then it is read as the same number.
- When a number is typed with a national leading zero, then it is read as the same number.
- When a number is pasted with spaces in it, then it is read as the same number.
- When a number is pasted with brackets and dashes, then it is read as the same number.
- When a number is already plain digits, then it is left as it is.
- When the field is empty, then nothing breaks.
- When only part of a number is typed, then there is no way to send.
- When the country's digits are all there, then the send appears.
- When more digits are typed than the country has, then the extra ones are refused.
- When the arrow is tapped, then the number is sent.
- When Enter is pressed, then the number is sent, for somebody typing rather than tapping.
- When Enter is pressed before the number is whole, then nothing is sent.
- When a send is already going out, then the send stays closed.
- When a country code is typed, then the country it belongs to is shown.
- When the field is empty, then it says what to type.
- When the field is opened on a phone, then the app's own keypad is used.
- When a key on that keypad is pressed, then one digit is added.
- When the number is whole on a phone, then the send is still offered.
- When the keypad would not fit, then the device's own keyboard is used instead.

### Starting the verify step and sending the code

- When the shopper has no number yet, then the flow starts on the number step.
- When the account already owns the number, then the number step is skipped.
- When the number is locked but none was given, then the shopper is still asked for one.
- When a stored number carries a plus, then it is reduced to plain digits.
- When a number is typed with a double zero, then it is reduced to plain digits.
- When a number is typed with a national leading zero, then it is reduced to plain digits.
- When a number is typed with spaces, then it is reduced to plain digits.
- When a method is chosen, then the code is sent on it and the code step opens.
- When SMS is chosen, then the send is marked as SMS and not as WhatsApp.
- When a code is sent, then it is reported to analytics, naming the screen that asked for it.
- When the step has moved, then the host screen is told to run its own change.
- When a cooldown is running on that number, then no code is sent.
- When the session has used its allowance of numbers, then no code is sent.
- When the host screen supplies its own blocked message, then that message is shown.
- When the host screen already shows a countdown, then the flow adds nothing of its own.
- When the send is refused after the request, then the reason is shown.
- When a countdown is already running, then the backend's fixed wait message is dropped.
- When a failure carries nothing to show, then a translated line is used instead.

### Asking for another code and checking it

- When the resend is tapped, then the code goes again on the method already chosen.
- When no method has been chosen yet, then the resend does nothing.
- When the number is on cooldown, then the resend does nothing.
- When a code is resent, then it is reported with the attempts made since the last send.
- When a code is resent, then the attempt count starts again.
- When a new code is sent, then any code left in the boxes is cleared.
- When a code is typed, then it is checked against the send it belongs to.
- When the code is right, then it is marked good and the result is handed to the screen.
- When the boxes fire more than once, then the code is still only checked once.
- When the code is wrong, then the shopper is told, and it is reported.
- When the code is wrong, then the boxes are cleared for the next try.
- When a code has been refused, then the shopper can try again.

### The full-screen verify — the number and how the code arrives

- When a shopper confirms a number they typed, then the flow opens on the number step.
- When only part of a number is typed, then there is no way to send.
- When the number is whole, then the shopper moves to the method step.
- When the shopper moves on, then the number they typed is carried with them.
- When the shopper goes back, then they can correct the number.
- When the account already owns the number, then the flow opens straight on the method step.
- When the account owns the number, then there is no way to swap it.
- When the account owns the number, then there is no way to swap it from the code step either.
- When WhatsApp is chosen, then the code is sent and the code step opens.
- When SMS is chosen, then the code is sent and the screen says so.
- When the send is refused, then the shopper is told why nothing happened.
- When the number is on cooldown, then a countdown shows and both methods are closed.
- When the session has used its allowance of numbers, then the shopper is told.

### The full-screen verify — entering the code and leaving

- When the code step opens, then there is one box for each digit of the code.
- When the last digit is typed, then the code is checked at once.
- When the code is right, then the screen that opened the flow is told.
- When the code is wrong, then the shopper is told it was wrong.
- When a code has just been sent, then the screen counts down instead of offering a resend.
- When the countdown runs out, then the resend is offered.
- When the shopper wants the code another way, then they can go back to the method step.
- When a send left no cooldown behind, then the shopper can still type the code and get through.
- When the shopper closes from the number step, then the flow closes.
- When the shopper closes from the method step, then the flow closes.
- When the shopper closes from the code step, then the flow closes.
- When the shopper is browsing in Arabic, then the number step is shown in Arabic.

### The verify panel inside the basket — opening and sending

- When the account has no number on file, then the shopper is asked first whether they already have one.
- When the shopper says they have an account, then they go straight to the number.
- When the shopper is new, then the terms are put in front of them first.
- When a new shopper accepts the terms, then it is recorded the same way the full signup records it.
- When the account already owns a number, then the panel opens on the method step.
- When the account owns the number, then no way to swap it is offered.
- When the shopper typed the number themselves, then they can go back and fix it.
- When a method is tapped, then the code is sent on that method.
- When a code is on its way, then the code boxes appear.
- When the number is on cooldown, then both methods are closed.
- When the session has used its allowance of numbers, then the panel says so.

### The verify panel inside the basket — waiting, typing and closing

- When a send worked, then the countdown reads as the code being on its way.
- When a send was refused, then the countdown reads as the code not being sent.
- When one number was refused, then that refusal is not carried over to another number.
- When a code is typed, then it is checked against the send it belongs to.
- When the shopper is verified, then the basket is told.
- When the cooldown is running, then the resend stays closed.
- When the resend is tapped inside the cooldown, then nothing is sent.
- When a code is refused, then the reason is shown.
- When the panel is used on a phone, then the device's own keyboard is used, never the in-app keypad.
- When the panel is closed, then the close is handed back to the basket.

### Typing the code into the boxes

- When the boxes open on a phone, then the app's own keypad appears and no text field.
- When keys are pressed, then the boxes fill in order.
- When the back key is pressed, then the last digit is taken back.
- When the sixth digit lands, then the code is submitted.
- When fewer than six digits are in, then nothing is submitted.
- When the code is accepted, then the keypad is taken away.
- When the code is being checked, then no more keys are taken.
- When the boxes open at a desk, then the device's own keyboard is used.
- When the browser sees the code arrive, then it is offered to the shopper.
- When digits are typed, then they show in the boxes.
- When something that is not a digit is typed, then it is ignored.
- When six digits are in, then the code is submitted.
- When the code is being checked, then nothing more is taken.

### The code screen and its two clocks

- When a send left no cooldown behind, then the shopper can still type the code they were sent.
- When a code has just arrived, then the screen does not claim it has expired.
- When nothing is holding the send back, then another code is still offered.
- When the send cooldown runs out, then the code can still be typed.
- When the code's own life runs out, then the screen says it has expired.
- When a code is resent, then it gets a life of its own.
- When a cooldown is running, then the screen counts down to the resend.
- When the code step opens, then the screen names the way the code was sent.
- When the code step opens, then the screen shows the number the code went to.
- When a code was refused, then the reason is shown.
- When the code is being checked, then the boxes are closed.
- When the code has been accepted, then the boxes stay closed.
- When the resend is tapped while the cooldown runs, then nothing is sent.
- When a new code is asked for, then the old one is cleared out of the boxes.
- When the cooldown runs out, then it is reported once, and only once.

### Asking a shopper to confirm their number

- When the account has a number, then that number is used and cannot be changed.
- When the account has no number, then the shopper is asked for one.
- When the stored number is a plain zero, then it counts as no number at all.
- When the stored number is the text "0", then it counts as no number at all.
- When a session has expired, then the number from that session is used again.
- When that number is used, then it is not handed to any other flow.
- When the flow opens, then it is recorded, along with where it was opened from.
- When the flow opens with no source, then it counts as a checkout, because that is what raises it.
- When the overlay is up, then the page behind it stops scrolling.
- When the shopper finishes verifying, then the overlay is taken down.
- When a checkout shopper finishes, then they are reported as returned to checkout.
- When a shopper who came to post a story finishes, then the story composer opens.
- When a shopper who came to send a message finishes, then chat opens.
- When an expired session is recovered, then its stored number is forgotten.
- When the shopper walks away, then the requests waiting on them are settled.
- When the shopper walks away, then the extra service passes left behind are cleared.
- When the session had expired, then those passes are not cleared a second time.
- When the shopper walks away, then the page reloads to match whatever pass is now stored.
- When the shopper is on the seller dashboard, then they are sent away rather than the page reloading.
- When the clean-up call fails, then the shopper still leaves.

### When a session has run out

- When the shopper chooses to sign in again, then the verify flow opens, marked as an expired session.
- When that shopper is on the seller dashboard, then they are marked as a seller instead.
- When the shopper chooses to sign in again, then the parked requests wait rather than failing.
- When the shopper carries on as a guest, then the waiting requests are settled as cancelled.
- When the shopper carries on as a guest, then the page reloads so the gone account stops showing.
- When a guest is on the seller dashboard, then they are sent away instead of the page reloading.
- When the prompt appears, then it tells the shopper why they are being asked.
- When the prompt appears, then it speaks the language the shopper is browsing in.

### The clock on a borrowed session

- When an ordinary shopper is browsing, then no session clock is shown.
- When an unrelated value changes in storage, then the clock is not woken.
- When a tester is borrowing a session, then the clock shows how long is left.
- When a tester is borrowing a session, then the clock counts the real time left, not a fixed two minutes.
- When the stored end time is already in the past, then nothing is shown.
- When the stored end time is already in the past, then it is cleared out of storage.
- When the stored end time is already in the past, then the borrowed account is signed out.
- When the stored end time is more than half an hour away, then it is not trusted either.
- When the session runs out while the tester watches, then the screen says so and reloads onto what is left.

### Countdowns on screen

- When a countdown is shown, then every part is padded to two digits.
- When the caller wants seconds only, then only the seconds are shown.
- When there are hours left, then they are counted as their own part, not as extra minutes.
- When the countdown reaches zero, then the caller is told, once.
- When a product is still being added, then the redeem countdown holds.
- When that product is finished, then the redeem countdown runs on.

### The profile card

- When a signed-in shopper opens their settings, then their own name is on the card.
- When a signed-in shopper opens their settings, then there is a link to the picture page and the profile.
- When a signed-in shopper opens their settings, then the verify control is shown.
- When a visitor is not signed in, then they are invited to log in instead of shown a card.
- When a visitor is not signed in, then there is no profile link to follow.
- When a visitor taps the card, then the sign-in surface opens instead of the app navigating away.
- When the stored picture is the word "guest", then it is treated as no picture.
- When the stored picture is the word "verified_guest", then it is treated as no picture.
- When the stored picture is the misspelt "verfied_guest", then it is treated as no picture.
- When the stored picture is the word "null", then it is treated as no picture.
- When the stored picture is empty, then it is treated as no picture.
- When there is a real picture, then it is shown.

### The profile form

- When the name is left empty, then the form refuses to save and shows the message the screen really uses.
- When the name is shorter than the minimum, then the form refuses to save.
- When the phone number has been cleared, then the form refuses to save.
- When an e-mail is not an e-mail, then the form refuses to save, though an empty e-mail is allowed.
- When no gender is chosen, then the form refuses to save.
- When a field is corrected, then its message goes away.
- When a visitor is not signed in, then the sign-in surface opens instead of a save.
- When a visitor is not signed in, then they are stopped before checking, so no messages appear.
- When the phone number has been changed, then the re-verify step opens instead of a save.
- When the phone number was not touched, then the profile saves directly.

### The verify button on the profile

- When the account already has a usable phone, then the control reads Verified.
- When the account already has a usable phone, then tapping the control opens nothing.
- When the account has no usable phone, then the control reads Verify Now.
- When there is a number to work with, then tapping the control opens the re-verify overlay.
- When there is no number at all, then the shopper is sent to the sign-in surface instead.
- When a global sign-in surface opens, then this overlay closes and does not reopen when that clears.
- When a re-authentication demand appears, then this overlay stands down for it too.

### The profile picture

- When a picture is chosen, then the screen is ready for the shopper to save it.
- When the shopper saves, then the picture is uploaded and its stored path goes on to the profile.
- When the picture is removed, then it is cleared and the profile is saved with no picture.
- When the media backend refuses the upload, then nothing is saved and the shopper stays where they were.
- ⚠️ When an upload is refused, then the shopper is told nothing — recorded as it is today, not approved.

### Which services the app will pass a request to

- When no service is named at all, then the request is refused.
- When a service name carries a stray space, then the request is refused.

Another 3 checks keep the testing setup itself honest — they protect the tests, not the app,
...
|---|---|---|---|
| Lines of code | 3644 | 28016 | 13.0% |
| Decision points | 2894 | 25228 | 11.5% |
| Functions | 647 | 7133 | 9.1% |

Of the 728 parts of the app in this report, 47 have a test written for them on purpose.
Another 71 are only touched in passing, and 610 have nothing at all.

### The parts we set out to test

| Part of the app | Share checked |
|---|---|
| Sending every visitor to the right page — proxy.ts | 100.0% |
| Telling the app who is signed in — app/api/auth/me/route.ts | 100.0% |
| Renewing an expired session — app/api/auth/refresh/route.ts | 100.0% |
| Giving a new device a guest session — app/api/auth/register-device/route.ts | 100.0% |
| Saving changes to a shopper's profile — app/api/auth/update-user/route.ts | 100.0% |
| Handing the wallet its own pass — app/api/auth/wallet-token/route.ts | 100.0% |
| Asking a shopper to confirm their number — components/Login/ConfirmMobilePhoneWidget.tsx | 100.0% |
| Telling a shopper their session ran out — components/Login/SessionExpiredWidget.tsx | 100.0% |
| The countdowns on screen — components/Login/Timer.tsx | 100.0% |
| The verify panel inside the basket — components/Login/Enhanced/InlineVerifyPanel.tsx | 100.0% |
| The screen where the code is typed — components/Login/Enhanced/screens/EnterPinScreen.tsx | 100.0% |
| Asking a backend for data as the signed-in shopper — serverRequests/HandleAuthedFetch.ts | 100.0% |
| Fetching data on the server — serverRequests/ServerFetch.tsx | 100.0% |
| Not asking the same question twice — serverRequests/requestDedup.ts | 100.0% |
| What the app remembers about the shopper — store/auth/reducer.tsx | 100.0% |
| Showing a phone number properly — utils/formatPhone.ts | 100.0% |
| Shared helpers, translation included — utils/functions.tsx | 100.0% |
| Holding back a second code sent too soon — utils/otpLocks.ts | 100.0% |
| Deciding a phone number can be used — utils/phone.ts | 100.0% |
| Reading the shop's starting settings — utils/startingSettings.ts | 100.0% |
| Renewing a pass on the server — utils/server/authRefresh.ts | 100.0% |
| The tester numbers that skip the limits — utils/server/otpAllowlist.ts | 100.0% |
| Recording each code that goes out — utils/server/otpTelemetry.ts | 100.0% |
| Which services may be asked for a pass — utils/server/tokenManager.ts | 98.9% |
| Talking to a backend from the browser — utils/fetchData.ts | 98.7% |
| Working out who asked for a code — utils/server/otpIdentity.ts | 98.4% |
| Sending a code — serverActions/sendOtp.ts | 97.4% |
| Writing an error down so it can be reported — utils/errorSerialization.ts | 97.4% |
| Which filters are switched on in a listing — utils/listing/filterItemState.ts | 97.0% |
| The steps of verifying a number — components/Login/Enhanced/usePhoneVerifyFlow.ts | 96.0% |
| Shared helpers on the server — utils/server/helpers.ts | 95.6% |
| Signing in — app/api/auth/login/route.ts | 94.1% |
| The full-screen verify — components/Login/Enhanced/VerifyPhoneFlow.tsx | 93.8% |
| Ending a session on purpose — app/api/auth/expire/route.ts | 93.5% |
| The clock on a borrowed session — components/Login/SessionTimer.tsx | 90.9% |
| The code boxes — components/Login/Enhanced/ui/RdbPinInputs.tsx | 89.2% |
| Clearing the passes a shopper leaves behind — app/api/auth/clear-tokens/route.ts | 87.5% |
| The verify button on the profile — components/setting/profile/VerifyUser.tsx | 87.0% |
| Typing a phone number — components/Login/Enhanced/ui/RdbPhoneInput.tsx | 85.5% |
| The profile form — components/setting/profile/PersonalInfoForm.tsx | 82.0% |
| Signing out — app/api/auth/logout/route.ts | 81.5% |
| Passing a request on to a backend — app/api/proxy/route.ts | 73.3% |
| The profile card — components/setting/profile/index.tsx | 73.1% |
| Small shared helpers — utils/tinyUtils.tsx | 68.1% |
| Building a search — services/elastic/helpers.ts | 51.8% |
| The profile picture — components/settings/UploadProfilePhoto.tsx | 36.5% |
| The short-term store the server keeps — serverRequests/radis/index.ts | 19.3% |

### Reading these numbers

- **What is checked well:** signing in, phone numbers and codes, sessions, and the profile screens.
- **What has nothing yet:** the shop itself — browsing, the basket, checkout, payment, orders and live selling.
...
tests/components/settings/UploadProfilePhoto.test.tsx :: an upload the media backend refuses tells the shopper nothing — pinned, not endorsed
