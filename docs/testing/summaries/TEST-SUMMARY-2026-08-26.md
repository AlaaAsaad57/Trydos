# Test summary — 26 August 2026

**This run adds 20 new checks: how we pick a visitor's country, what crawlers and link previews may fetch, a refused profile picture, and returned items on an order. Everything passes.**

| | |
|---|---|
| **New checks added this time** | 20 |
| **Checks in the app in total** | 1512 |
| **Result** | ✅ All passing |
| **How much of the app is checked** | 13.4% of the code |
| **Date** | 2026-08-26 |
...
- When the network in front of the site reports the visitor's country, then that country is used first.
- When that network is not in front of the site, then the country the host works out is used instead.
- When the country cannot be worked out, then the app treats it as no country at all.

### What crawlers and link previews may fetch

- When Facebook fetches a page to build a link preview, then it is allowed.
- When Twitter fetches a page to build a link preview, then it is allowed.
- When LinkedIn fetches a page to build a link preview, then it is allowed.
- When Telegram fetches a page to build a link preview, then it is allowed.
- When Discord fetches a page to build a link preview, then it is allowed.
- When any other crawler asks for a page before launch, then it is still shut out.
- When a crawler looks for the list of all our pages, then no such list is published.
- When indexing is switched on, then the whole site is open to every crawler.

### Keeping private details out of error reports

- When a request carrying a sign-in credential fails, then the credential never reaches the error reporter.
...
- When the service name ends in a stray space, then it is refused — the allowed list is matched exactly.

### The profile picture

- When the picture upload is refused, then the shopper is told that it failed.

### Returned items on an order

- When a return request is accepted, then the returned amount is shown on the item.
- When a return request is cancelled, then the returned amount is hidden.
- When a return request is still a draft, then the returned amount is hidden.
- When the item was never returned, then no returned amount is shown.
- When the returned amount is zero, then nothing is shown.

Another 21 checks keep the testing setup itself honest — they protect the tests, not the app,
...
|---|---|---|---|
| Lines of code | 3748 | 28040 | 13.4% |
| Decision points | 2974 | 25241 | 11.8% |
| Functions | 656 | 7136 | 9.2% |

### The parts we set out to test

| Part of the app | Share checked |
|---|---|
| Sending every visit to the right language and country — proxy.ts | 100.0% |
| The crawler rules — app/robots.ts | 100.0% |
| Who am I — app/api/auth/me | 100.0% |
| Renewing a credential — app/api/auth/refresh | 100.0% |
| Registering a device — app/api/auth/register-device | 100.0% |
| Saving profile details — app/api/auth/update-user | 100.0% |
| The wallet credential — app/api/auth/wallet-token | 100.0% |
| Confirming a phone number — ConfirmMobilePhoneWidget | 100.0% |
| The "please sign in again" prompt — SessionExpiredWidget | 100.0% |
| The code countdown — Timer | 100.0% |
| The sign-in panel — InlineVerifyPanel | 100.0% |
| The code entry screen — EnterPinScreen | 100.0% |
| Returned items on an order — returnedQty | 100.0% |
| Talking to a backend on the server — HandleAuthedFetch | 100.0% |
| Reading data on the server — ServerFetch | 100.0% |
| Not asking for the same thing twice — requestDedup | 100.0% |
| What the app remembers about the signed-in shopper — auth store | 100.0% |
| Writing a phone number out — formatPhone | 100.0% |
| Shared helpers — functions | 100.0% |
| Not letting a code be re-sent too soon — otpLocks | 100.0% |
| Reading a phone number — phone | 100.0% |
| Platform settings such as delivery days — startingSettings | 100.0% |
| Renewing a credential on the server — authRefresh | 100.0% |
| Test numbers allowed past the limits — otpAllowlist | 100.0% |
| Reporting on code sending — otpTelemetry | 100.0% |

### Reading these numbers

- **What is checked well:** signing in, staying signed in, language and country routing, shared helpers.
- **What has nothing yet:** the money path (basket, checkout, order), search, and the seller screens.
