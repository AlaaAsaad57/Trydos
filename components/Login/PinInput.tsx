  ...
  useEffect(() => {
    if (user && !failedLogin) {
      ...
      setTimeout(() => {
        ...
        setPin("");
        document
          .querySelectorAll(".pincode-input-text")
      ...
      setTimeout(() => {
        setRender(true);
        document
          .querySelectorAll<HTMLInputElement>(".pincode-input-text")[0]
          ?.focus();
      }, 2000);
    }
  }, [user, failedLogin]);
          ...
          .map((e, index) => (
            <div
              ...
              className={
                "pin-border-element" +
                " " +
                (user?.id && "input-success") +
                " " +
                ((wrongNumber || failedLogin) && !user && "input-failed")
              }
              style={{
                backgroundColor: pin[index] || disabled ? "#f5f5f5" : "#fafafa",
                borderRadius: "15px",
              }}
            >
              <svg
