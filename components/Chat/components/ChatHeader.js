...
function ChatHeader({chats,activeChat,openDetails}) {
  ...
  return (
    ...
    {/* <VideoIcon onClick={()=>dispatch({ type: "VIDEO_CALL" })} className="vcall" ></VideoIcon>
      ...
      <VideoIcon className={`${(callLoading==='video') && 'loading-svg'} vcall`} onClick={()=>{!callLoading&&makeVideoCall(activeChat.id,activeChat.channel_members.filter((s)=>parseInt(s.user_id)!==parseInt(getUserChat()?.id))[0].user.name,activeChat.channel_members.filter((s)=>parseInt(s.user_id)!==parseInt(getUserChat()?.id))[0].user.photo_path,activeChat.id,activeChat.channel_members.filter((s)=>parseInt(s.user_id)!==parseInt(getUserChat()?.id))[0].user.mobile_phone); }} ></VideoIcon>
      <CallIcon className={`${(callLoading==='voice') && 'loading-svg'} call`} onClick={()=> {!callLoading&&makeVoiceCall(activeChat.id,activeChat.channel_members.filter((s)=>parseInt(s.user_id)!==parseInt(getUserChat()?.id))[0].user.name,activeChat.channel_members.filter((s)=>parseInt(s.user_id)!==parseInt(getUserChat()?.id))[0].user.photo_path,activeChat.id,activeChat.channel_members.filter((s)=>parseInt(s.user_id)!==parseInt(getUserChat()?.id))[0].user.mobile_phone);}} ></CallIcon>
    </div>
  </div>
  )
}
