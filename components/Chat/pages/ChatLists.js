...
const handleClick=(e)=>{
  ...
  return (
    <div className='chat-list-items'>
     ...
     {
      chats?.filter((s)=>s.channel_members.filter((mem)=>mem.user_id===getUserChat().id)[0].pin===1).map((chat)=>{
        return(
      ...
      {chats?.filter((s)=>s.channel_members.filter((mem)=>mem.user_id===getUserChat().id)[0].pin===0&&pinned.filter((p)=>p.id===s.id).length===0).map((chat)=>{
        return(
