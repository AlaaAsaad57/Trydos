  ...
  new Promise((resolve) => {
    onMessage(messaging, (payload) => {
     ...
     if(payload.data.type==="VoiceCallEvent"){
      ...
      let channel=store.getState().chat.data.filter((ch)=>parseInt(ch.id)===parseInt(data.channelId))[0]?store.getState().chat.data.filter((ch)=>parseInt(ch.id)===parseInt(data.channelId))[0]:  {id:JSON.parse(payload.data.data).message.channel.id,messages:[JSON.parse(payload.data.data).message],channel_members:[{user_id:data.user_id,user:{id:data.user_id,name:data.callerName,photo_path:data.callerPhoto},mute:0,pin:0,archived:0},{mute:0,pin:0,archived:0,user_id:getUserChat().id,user:getUserChat()}]}
