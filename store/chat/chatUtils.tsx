...
export const getMessageStatus = ({
  ...
  if (
    message_status.filter((a) => a.user_id !== user?.id).length > 0 &&
    message_status.filter((a) => a.user_id !== user?.id)[0]?.is_watched ===
      true &&
    message_status.filter((a) => a.user_id !== user?.id)[0]?.watched_at
  ) {
    return (
      <>
        {
    ...
    message_status.filter((a) => a.user_id !== user?.id).length > 0 &&
    message_status.filter((a) => a.user_id !== user?.id)[0]?.is_received ===
      1 &&
    message_status.filter((a) => a.user_id !== user?.id)[0]?.received_at
  ) {
    return (
      <>
        {
...
export const getMessageTime = (ti, zone) => {
  let d;
  if (ti) {
    if (zone) {
