import * as React from "react";

const Tw = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    viewBox="-60 -40 240 160"
    {...props}
  >
    <rect width="100%" height="100%" x="-60" y="-40" fill="#fe0000" />
    <rect width="50%" height="50%" x="-60" y="-40" fill="#000095" />
    <path id="a" fill="#fff" d="M8 0 0 30-8 0l8-30M0 8l30-8L0-8l-30 8" />
    <use xlinkHref="#a" transform="rotate(30)" />
    <use xlinkHref="#a" transform="rotate(60)" />
    <circle r="17" fill="#000095" />
    <circle r="15" fill="#fff" />
  </svg>
);

export default Tw;
