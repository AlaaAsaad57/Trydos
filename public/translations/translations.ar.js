const translations = {
  "Typing...": "يكتب...",
  "Employees": "الموظفون",
  "Roles": "الأدوار",
  "Job Titles": "المسميات الوظيفية",
  "Offices": "المكاتب",
  "Departments": "الأقسام",
  "Work Forms": "أنماط العمل",
  "Currencies": "العملات",
  "Countries": "الدول",
  "Comments": "التعليقات",
  "Admin": "الإدارة",
  "Read Locations": "عرض المواقع",
  "Create Location": "إنشاء موقع",
  "Update Location": "تحديث الموقع",
  "Change Location Status": "تغيير حالة الموقع",
  "Read Shop Info": "عرض معلومات المتجر",
  "Update Shop Info": "تحديث معلومات المتجر",
  "Read Product Images": "عرض صور المنتج",
  "Upload Product Images": "رفع صور المنتج",
  "Delete Product Images": "حذف صور المنتج",
  "Read Story": "عرض القصص",
  "Read Comments": "عرض التعليقات",
  "Reply Comment": "الرد على تعليق",
  "Tries left": "المحاولات المتبقية",
  "Too many wrong codes. Ask for a new code.": "محاولات خاطئة كثيرة. اطلب رمزاً جديداً.",
  "This product has pending changes awaiting admin approval. The form below shows your submitted changes; the live product still shows the previous values until approval.": "يحتوي هذا المنتج على تغييرات معلّقة بانتظار موافقة الإدارة. يعرض النموذج أدناه التغييرات التي أرسلتها، بينما يستمر المنتج المنشور في عرض القيم السابقة حتى تتم الموافقة.",
  "Your last changes to this product were denied. The live product still shows the previous values.": "تم رفض آخر تغييرات أجريتها على هذا المنتج. لا يزال المنتج المنشور يعرض القيم السابقة.",
  "Couldn't load your shop details. Product creation is unavailable until they load.": "تعذّر تحميل بيانات متجرك. لا يمكن إنشاء منتج حتى يتم تحميلها.",
  "Adding a product needs permission to view shop info. Ask a shop admin to grant you that permission, then try again.": "تتطلب إضافة منتج إذن عرض معلومات المتجر. اطلب من مدير المتجر منحك هذا الإذن ثم أعد المحاولة.",
  "Couldn't load your shop details. Editing this product is unavailable until they load.": "تعذّر تحميل بيانات متجرك. لا يمكن تعديل هذا المنتج حتى يتم تحميلها.",
  "Opening a product needs permission to view shop info. Ask a shop admin to grant you that permission, then try again.": "يتطلب فتح منتج إذن عرض معلومات المتجر. اطلب من مدير المتجر منحك هذا الإذن ثم أعد المحاولة.",
  "Bold": "عريض",
  "Italic": "مائل",
  "Underline": "تسطير",
  "Heading": "عنوان",
  // Listing share widget
  "Share this page": "شارك هذه الصفحة",
  "Send this page to your favourite apps": "أرسل هذه الصفحة إلى تطبيقاتك المفضلة",
  "Link Copied to Clipboard": "تم نسخ الرابط",
  // Listing sort widget
  "Sort products": "ترتيب المنتجات",
  "Choose how products are ordered": "اختر طريقة ترتيب المنتجات",
  Default: "الافتراضي",
  Recommended: "موصى به",
  "Best match for your search": "الأفضل لبحثك",
  "Best sellers": "الأكثر مبيعاً",
  "Most bought right now": "الأكثر شراءً الآن",
  "New arrivals": "وصل حديثاً",
  "By date added": "حسب تاريخ الإضافة",
  Newest: "الأحدث",
  Oldest: "الأقدم",
  "By product price": "حسب سعر المنتج",
  "Low to High": "من الأقل إلى الأعلى",
  "High to Low": "من الأعلى إلى الأقل",
  Alphabetical: "أبجدياً",
  "A to Z": "من أ إلى ي",
  "Z to A": "من ي إلى أ",
  // Become Seller modal
  "Tell us about you and your shop to start selling":
    "أخبرنا عنك وعن متجرك لتبدأ البيع",
  Password: "كلمة المرور",
  "Select document type": "اختر نوع المستند",
  John: "محمد",
  Doe: "أحمد",
  "example@mail.com": "example@mail.com",
  "+1 234...": "+1 234...",
  "My Shop": "متجري",
  "Street 1": "شارع 1",
  Warehouse: "مستودع",
  "Full Address": "العنوان الكامل",
  Identity: "الهوية",
  Passport: "جواز السفر",
  "Commercial License": "رخصة تجارية",
  "Tax Certificate": "شهادة ضريبية",
  "Bank Statement": "كشف حساب بنكي",
  "Address Proof": "إثبات العنوان",
  "Authorization Letter": "خطاب تفويض",
  "Please select a document type": "الرجاء اختيار نوع المستند",
  "Something went wrong": "حدث خطأ ما",
  "We could not load this part of the page": "لم نتمكن من تحميل هذا الجزء من الصفحة",
  "Upload URL not found": "لم يتم العثور على رابط الرفع",
  "Failed to add contact": "فشل إضافة جهة الاتصال",
  "Failed to upload document": "فشل رفع المستند",
  // Report Story flow
  "Inappropriate Content": "محتوى غير لائق",
  "Harassment or Hate Speech": "مضايقة أو خطاب كراهية",
  "Spam or Scam": "محتوى غير مرغوب فيه أو احتيال",
  "Intellectual Property Violation": "انتهاك الملكية الفكرية",
  "Violence or Dangerous Content": "عنف أو محتوى خطير",
  Other: "أخرى",
  "Report Reason": "سبب الإبلاغ",
  "Details (optional)": "تفاصيل (اختياري)",
  "Details": "تفاصيل",
  "Write details here...": "اكتب التفاصيل هنا...",
  "Submit Report": "إرسال البلاغ",
  "Tell us what went wrong so we can improve your experience":
    "أخبرنا بما حدث من خطأ حتى نتمكن من تحسين تجربتك",
  "Additional notes": "ملاحظات إضافية",
  Optional: "اختياري",
  "Write more details here": "اكتب المزيد من التفاصيل هنا",
  "We received your report. Thanks for your thoughts":
    "لقد استلمنا بلاغك. شكرًا لملاحظاتك",
  "Could not submit your report. Please try again":
    "تعذّر إرسال بلاغك. يرجى المحاولة مرة أخرى",
  "We received your report": "لقد استلمنا بلاغك",
  "Thanks for your thoughts": "شكرًا لملاحظاتك",
  "Please select at least one reason or provide details.":
    "يرجى تحديد سبب واحد على الأقل أو تقديم تفاصيل.",
  "Copy Data": "نسخ البيانات",
  "copy reason of verification success!": "تم نسخ سبب التحقق بنجاح!",
  "Pick a category, download its template, fill it in, then upload it.":
    "اختر فئة، ثم نزّل القالب الخاص بها، واملأه، ثم ارفعه.",
  Category: "الفئة",
  "Loading categories...": "جاري تحميل الفئات...",
  "Select a category": "اختر فئة",
  "Failed to load categories": "فشل تحميل الفئات",
  "Please select a category first": "يرجى اختيار فئة أولاً",
  "Download Template": "تنزيل القالب",
  "Preparing template...": "جاري تجهيز القالب...",
  "Template is not available": "القالب غير متوفر",
  "Failed to download template": "فشل تنزيل القالب",
  "Supports .xlsx, .xls, .xlsm, .xlsb": "يدعم .xlsx و .xls و .xlsm و .xlsb",
  "Please upload a valid Excel file (.xlsx, .xls, .xlsm, .xlsb)":
    "يرجى رفع ملف إكسل صالح (.xlsx أو .xls أو .xlsm أو .xlsb)",
  "File uploaded and processed successfully!": "تم رفع الملف ومعالجته بنجاح!",
  "Select Shop": "اختيار المتجر",
  "Customers Comments": "تعليقات العملاء",
  "Upload Excel File": "تحميل ملف إكسل",
  "FAQ": "الأسئلة الشائعة",
  "Reviewing": "المراجعات والتقييمات",
  "Comment ID": "معرف التعليق",
  "Customer": "الزبون",
  "Comment": "التعليق",
  "Rating": "التقييم",
  "Variant": "النوع",
  "Date": "التاريخ",
  "Reply Text": "نص الرد",
  "Reply": "رد",
  "Edit Reply": "تعديل الرد",
  "Write a reply...": "اكتب رداً...",
  "Submit Reply": "إرسال الرد",
  "Drag & drop Excel file here, or click to select": "اسحب وأفلت ملف إكسل هنا، أو انقر للاختيار",
  "Select Excel File": "اختر ملف إكسل",
  "Upload Excel": "رفع ملف إكسل",
  "Uploading...": "جاري الرفع...",
  "File uploaded successfully!": "تم رفع الملف بنجاح!",
  "File upload failed.": "فشل رفع الملف.",
  "No file selected": "لم يتم اختيار ملف",
  "Product Price When Returned": "سعر المنتج عند الإرجاع",
  Full: "كامل",
  "Please enable notification permissions to use camera features":
    "يرجى تفعيل أذونات الإشعارات لاستخدام ميزات الكاميرا",
  "Update Return Request For This Product": "تحديث طلب الإرجاع لهذا المنتج",
  "Buyer Rate": "زبون قام بالتقييم",
  "Shipping Company": "شركة الشحن",
  Recommendation: "توصية",
  "Confirm Returning Items": "تأكيد إرجاع العناصر",
  "Cancel All Return Requests": "إلغاء جميع طلبات الإرجاع",
  Complaint: "شكوى",
  Visitors: "الزوار",
  "And Not Necessarily From Customers Who Have Purchased The Product Before. These Are Pre-Purchase Questions, And They Are Answered Directly By The Seller":
    "وليست بالضرورة من عملاء قاموا بشراء المنتج من قبل. هذه أسئلة ما قبل الشراء، ويتم الإجابة عليها مباشرة من البائع",
  "All The Questions Below Are From": "جميع الأسئلة أدناه من",
  Quality: "الجودة",
  "There is No Comments Yet..": "لا توجد تعليقات بعد..",
  "Buyers Product Recommend To Buy": "توصية المشترين بشراء المنتج",
  "All Recommendations Are Genuine From Customers Who Purchased And Actually Received The Product Through":
    "جميع التوصيات حقيقية من العملاء الذين اشتروا المنتج واستلموه فعليًا من خلال",
  Buyer: "زبون",
  "Buyers Product Rate": "تقييم المنتج من قبل المشترين",
  "All Reviews Are Genuine From Customers Who Purchased And Actually Received The Product Through":
    "جميع التقييمات حقيقية من العملاء الذين اشتروا المنتج واستلموه فعليًا من خلال",
  "All Comments Are Genuine From Customers Who Purchased And Actually Received The Product Through":
    "جميع التعليقات حقيقية من العملاء الذين اشتروا المنتج واستلموه فعليًا من خلال",
  "Overall Good Quality": "جودة عامة جيدة",
  "Expected Shipping & delivery Date": "تاريخ التوصيل المتوقع",
  "Based On The Previous Delivery Statistics Below To Your Area, We Conclude That The Expected Delivery Time For Your Product Is":
    "بناءً على إحصائيات التسليم السابقة أدناه إلى منطقتك، نستنتج أن وقت التسليم المتوقع لمنتجك هو",
  "Get a": "احصل على",
  "Refund Of The Product Price If Shipping Is Delayed":
    "استرداد سعر المنتج إذا تأخر الشحن",
  "Views Product": "مشاهدات المنتج",
  "If Delivered After": "إذا تم التسليم بعد",
  "You Will Get 25 USD To Your Wallet Automatically":
    "ستحصل على 25 دولارًا أمريكيًا في محفظتك تلقائيًا",
  within: "خلال",
  "Days After Receiving The Product, You Can Return It Without Conditions Or Reasons With Complete Ease And Get The Amount Back":
    "أيام بعد استلام المنتج، يمكنك إرجاعه دون شروط أو أسباب بكل سهولة واسترداد المبلغ",
  "Very Bad": "سيئ جدًا",
  Bad: "سيئ",
  "Days After Receiving The Product, You Can Return It Without Conditions Or Reasons With Complete Ease And":
    "خلال أيام بعد استلام المنتج، يمكنك إرجاعه دون شروط أو أسباب بكل سهولة و",
  Normal: "عادي",
  Good: "جيد",
  "Very Good": "جيد جدًا",
  "get the Amount back": "المبلغ المسترد",
  January: "يناير",
  February: "فبراير",
  March: "مارس",
  April: "أبريل",
  May: "مايو",
  June: "يونيو",
  July: "يوليو",
  August: "أغسطس",
  September: "سبتمبر",
  October: "أكتوبر",
  November: "نوفمبر",
  December: "ديسمبر",
  "Refund Processing": "معالجة الاسترجاع",
  "Buyer Rate": "تقييم المشتري",
  "Recommend It By": "تم التوصية به من قبل",
  "Buyers Comment": "تعليق المشتري",
  "Good Quality": "جودة جيدة",
  "True Size": "المقاس الصحيح",
  "Recommend It": "أنصح به",
  "Dont Recommend It": "لا أنصح به",
  "FAQ Buyer & Seller": "الأسئلة الشائعة بين المشتري والبائع",
  Dear: "عزيزي",
  "Sizes Available": "المقاسات المتوفرة",
  "Out For Return": "خارج للإرجاع",
  "Buyers Reviews On Product Sizing": "مراجعات المشترين حول مقاسات المنتج",
  Translate: "ترجمة",
  "Show Original": "عرض النسخة الأصلية",
  "Ask Seller Your Question About This Product …":
    "اسأل البائع سؤالك حول هذا المنتج…",
  "Product Return Has Been Requested": "تم طلب إرجاع المنتج",
  "Waiting…": "جارٍ الانتظار…",
  Wait: "انتظر",
  "Wait": "انتظر",
  "s": "ثا",
  "before trying again": "قبل المحاولة مرة أخرى",
  "Notification is Blocked in This Browser Please Enable Notification premission and refresh":
    "تم حظر الإشعارات في هذا المتصفح، يرجى تفعيل إذن الإشعارات ثم تحديث الصفحة",
  "Product Return Request Approve": "الموافقة على طلب إرجاع المنتج",
  "Product Return Needs Confirm": "طلب إرجاع المنتج يحتاج إلى تأكيد",
  "You Need To Confirm Your Request": "تحتاج إلى تأكيد طلبك",
  "Product Return Request Approved": "تمت الموافقة على طلب إرجاع المنتج",
  "Product Collection Within 1 Day": "سيتم استلام المنتج خلال 1 يوم ",
  "Product Has Been Returned Successfully": "تم إرجاع المنتج بنجاح",
  "Back To Your Wallet": "سترجع لمحفظتك",
  "Product Has Been Resolved Successfully": "تم حل مشكلة المنتج بنجاح",
  "Cancel Return Request & Get": "إلغاء طلب الإرجاع والحصول على",
  "Add Story": "اضافة قصة",
  "Add Photo": "أضف صورة",
  "Failed To Remove Image..Try Again": "فشل حذف الصورة ..حاول مجددا",
  "Please Add Photos Of The Product You Received So That We Can Provide You With The Best Service To Avoid This Issue.":
    "يرجى إضافة صور للمنتج الذي استلمته حتى نتمكن من تزويدك بأفضل خدمة وتجنب هذه المشكلة.",
  "You Can Return The Product Without Any Conditions According To The Return Policy And Get A Refund":
    "يمكنك إرجاع المنتج دون أي شروط وفقًا لسياسة الإرجاع واسترداد المبلغ",
  "Cart Loading Error": "خطأ في تحميل السلة",
  "View Product": "عرض المنتج",
  "Failed To Retrive Results Please Try Again":
    "فشل في جلب النتائج، يرجى المحاولة مرة أخرى",
  "We encountered an error while loading your cart. Don't worry, we'll fix this issue for you.":
    "حدث خطأ أثناء تحميل سلتك. لا تقلق، سنقوم بحل هذه المشكلة من أجلك.",
  "Try Again": "حاول مرة أخرى",
  "Please log in first": "يرجى تسجيل الدخول أولاً",
  "Please verify the number first.": "يرجى التحقق من الرقم أولاً.",
  "Are you sure you want to delete this comment?":
    "هل أنت متأكد أنك تريد حذف هذا التعليق؟",
  "Delete Comment": "حذف التعليق",
  "Load More": "تحميل المزيد",
  brands: "العلامات التجارية",
  colors: "الألوان",
  sizes: "المقاسات",
  "More From": "مزيد من",
  "Confirm Country Change": "تأكيد تغيير البلد",
  "Are you sure you want to change your country?":
    "هل أنت متأكد أنك تريد تغيير بلدك؟",
  Current: "الحالي",
  New: "الجديد",
  "Tap “Agree & Continue” To Accept trydos":
    'اضغط على "موافقة ومتابعة" لقبول trydos',
  "Cancel Return Request": "إلغاء طلب الإرجاع",
  "Return Request": "طلب الارجاع",
  "Failed To Load Return Details Try again":
    "فشل في تحميل تفاصيل الإرجاع، حاول مرة أخرى",
  "You Need To Confirm Your Request": "تحتاج إلى تأكيد طلبك",
  "Product Return Needs Confirm": "يحتاج إرجاع المنتج إلى تأكيد",
  "Initializing Return": "جارٍ تهيئة الإرجاع",
  "return this product is not allowed": "إعادة هذا المنتج غير مسموحة",
  "Notification Permission:": "إذن الإشعارات:",
  "this option dosent have enough quantity":
    "هذا الخيار لا يحتوي على كمية كافية",
  "Preparing Your Experience": "جارٍ إعداد تجربتك",
  "Story hidden successfully.": "تم إخفاء القصة بنجاح",
  "Failed to report story.": "فشل في الإبلاغ عن القصة",
  "Story reported successfully.": "تم الإبلاغ عن القصة بنجاح",
  "Report Story": "الإبلاغ عن القصة",
  "Are you sure you want to report this story?":
    "هل أنت متأكد أنك تريد الإبلاغ عن هذه القصة؟",
  "Reset Redeemed Products": "إعادة تعيين المنتجات المُستردة",
  "Request Log": "سجل الطلبات",
  "Make Stories Token Expired": "جعل توكن القصص منتهي الصلاحية",
  "Make Chat Token Expired": "جعل توكن الدردشة منتهي الصلاحية",
  s: "ثا",
  "I Read And Agree To": "قرأت ووافقت على",
  "are allowed.": "مسموح بها.",
  "Please enter a valid URL (e.g., example.com or www.example.com)":
    "الرجاء إدخال عنوان URL صالح (مثال: example.com أو www.example.com)",
  "Only links from": "فقط الروابط من",
  "Yes, I Agree": "نعم، أوافق",
  "We Will Ignore The First Size And Send Your Order With The New Size.":
    "سنتجاهل الحجم الأول وسنرسل طلبك بالحجم الجديد.",
  "We Will Ignore The First Color And Send Your Order With The New Color.":
    "سنتجاهل اللون الأول وسنرسل طلبك باللون الجديد.",
  "You Can Add Up To 5 Photos Only": "يمكنك إضافة 5 صور كحد أقصى",
  "Each Photo Must Be 5 MB Or Less":
    "يجب أن يكون حجم كل صورة 5 ميغابايت أو أقل",
  "The Change Size Terms.": ".شروط تغيير الحجم",
  "The Change Color Terms.": ".شروط تغيير اللون",
  "To New Size": "إلى الحجم الجديد",
  "To New Color": "إلى اللون الجديد",
  "Change Below Size": "غيّر الحجم أدناه",
  "Change Below Color": "غيّر اللون أدناه",
  "Confirm the Changes First": "قم بتأكيد التغييرات أولاً",
  "Chat with delivery worker": "الدردشة مع عامل التوصيل",
  "Take Photo": "التقاط صورة",
  "Upload Photo/Video": "تحميل صورة/فيديو",
  "Failed To Load Filters Try Again":
    "فشل تحميل الفلترة، يرجى المحاولة مرة أخرى",
  "Failed To Load Products Retring in 3 seconds":
    "فشل تحميل المنتجات، سيتم المحاولة مرة أخرى في 3 ثوان",
  "Failed To Load Products After 3 Attempts":
    "فشل تحميل المنتجات بعد 3 محاولات",
  "No media selected": "لم يتم اختيار أي وسائط",
  "Add link...": "أضف رابط...",
  "I’m Afraid Of Sizes": "أخاف من المقاسات",
  "Hide This Pack": "إخفاء المجموعة",
  "Hide This Pack From My List": "إخفاء المجموعة من قائمتي",
  "Are you sure you want to hide this order?": "هل أنت متأكد أنك تريد إخفاء هذا الطلب؟",
  "Are you sure you want to hide this product?": "هل أنت متأكد أنك تريد إخفاء هذا المنتج؟",
  "This is the only product in this order, so hiding it will hide the whole order.":
    "هذا هو المنتج الوحيد في هذا الطلب، لذا فإن إخفاءه سيؤدي إلى إخفاء الطلب بالكامل.",
  "You Can Cancel This Pack And Back Your Money":
    "يمكنك إلغاء المجموعة واسترجاع أموالك",
  "Cancel This Pack": "إلغاء المجموعة",
  "Cancellation Terms.": "شروط الإلغاء.",
  Qty: "الكمية",
  "You Can't Change Qty To Higher Than The Current Qty":
    "لا يمكنك تغيير الكمية إلى أكثر من الكمية الحالية",
  "Select quantity": "اختر الكمية",
  "Report This Product": "أبلغ عن هذا المنتج",
  "Delivery Time, Delivery Man, Delivery Car":
    "وقت التسليم، رجل التوصيل، سيارة التوصيل",
  "Add Comment…": "أضف تعليق…",
  Pack: "مجموعة",
  "Please Add Photos Of The Product You Received So That We Can Provide You With The Best Service To Avoid This Issue.":
    "يرجى إضافة صور للمنتج الذي استلمته حتى نتمكن من تقديم أفضل خدمة لك لتجنب هذه المشكلة",
  "Rate & Get Money": "قيم  واربح المال",
  "Your Accurate Rating Of The Products You Receive Helps Us Improve Quality And Helps Other Customers Purchase.":
    "تقييمك الدقيق للمنتجات التي تتسلمها يساعدنا في تحسين الجودة ويساعد العملاء الآخرين على الشراء.",
  "Story deleted successfully.": "تم حذف القصة بنجاح.",
  "Failed to delete story.": "فشل في حذف القصة.",
  "Are you sure you want to delete this story?":
    "هل أنت متأكد أنك تريد حذف هذه القصة ؟",
  Confirm: "تأكيد",
  "Delete Story": "حذف القصة",
  "The number was copied successfully": "تم نسخ الرقم بنجاح",
  "Number copy failed": "فشل نسخ الرقم",
  "You Have New Messages From Delivery Worker":
    "لديك رسالة جديدة من عامل التوصيل..انقر للمزيد",
  "Share Story": "مشاركة القصة",
  Cancel: "الغاء",
  "For All": "للكل",
  "You are about to leave our site": "أنت على وشك مغادرة موقعنا",
  "External Link Warning": "تحذير رابط خارجي",
  "For Me": "حذف لدي",
  Imagee: "صورة",
  Video: "فيديو",
  Audio: "تسجيل صوت",
  File: "ملف",
  "Why Was The Product Return?": "لماذا تم إرجاع المنتج؟",
  "To Your Account": "إلى حسابك",
  "You Can Cancel The Product Without Any Conditions According To The Cancel Policy And Get A Full Refund":
    "يمكنك إلغاء المنتج دون أي شروط وفقًا لسياسة الإلغاء واسترداد المبلغ بالكامل",
  "Premium Shopping": "تسوق مميز",
  Experience: "تجربة",
  "Featured Products": "المنتجات المميزة",
  "Flash Deals": "عروض سريعة",
  "Flash Deal": "عرض سريع",
  "Show More": "عرض المزيد",
  "Premium Brands": "علامات تجارية مميزة",
  "TryDos - Premium Shopping Experience": "ترايدوس - تجربة تسوق مميزة",
  "Discover premium products on TryDos":
    "اكتشف المنتجات المميزة في ترايدوس - وجهتك الأمثل للتسوق مع المنتجات المميزة والعروض السريعة ومجموعات البوتيك.",
  "Featured Products - TryDos":
    "المنتجات المميزة - ترايدوس | مجموعة مختارة مميزة",
  "Flash Deals - TryDos": "العروض السريعة - ترايدوس | عروض لفترة محدودة",
  "Compare Products - TryDos":
    "مقارنة المنتجات - ترايدوس | أداة مقارنة المنتجات",
  "Categories - TryDos": "الفئات - ترايدوس | تصفح حسب الفئة",
  "Settings - TryDos": "الإعدادات - ترايدوس | إعدادات الحساب",
  "Premium Curated Collection": "مجموعة مختارة مميزة",
  "Limited Time Offers": "عروض لفترة محدودة",
  "Product Comparison Tool": "أداة مقارنة المنتجات",
  "Browse by Category": "تصفح حسب الفئة",
  "Account Settings": "إعدادات الحساب",
  "Your ultimate shopping destination": "وجهتك الأمثل للتسوق",
  "Curated selection of premium featured products":
    "مجموعة مختارة من المنتجات المميزة عالية الجودة",
  "Limited-time offers with great discounts": "عروض لفترة محدودة بخصومات رائعة",
  "Find and compare your favorite products": "ابحث وقارن منتجاتك المفضلة",
  "Browse products by categories": "تصفح المنتجات حسب الفئات",
  "Manage your account preferences": "إدارة تفضيلات حسابك",
  shopping: "تسوق",
  "online shopping": "تسوق إلكتروني",
  "premium products": "منتجات مميزة",
  "best deals": "أفضل العروض",
  "shopping mall": "مول تسوق",
  boutiques: "بوتيكات",
  brands: "علامات تجارية",
  fashion: "أزياء",
  electronics: "إلكترونيات",
  "home garden": "منزل وحديقة",
  "Action About Your Order": "اجراءات حول طلبك",
  "Change Delivery Note": "تغيير ملاحظة التوصيل",
  "Add delivery instructions...": "أدخل تعليمات التوصيل...",
  "Action About This Product": "الاجراء الخاص بهذا المنتج",
  "And Back Your Money": "واسترد أموالك ",
  Hours: "ساعات  ",
  "Return This Product": "إرجاع هذا المنتج",
  "Change Size, Color, Other": "تغيير الحجم واللون وغير ذلك",
  "Change Product Request": "طلب تغيير المنتج",
  "You Can Change Delivery Address": "يمكنك تغيير عنوان التوصيل",
  "Delivery Address & Note": "عنوان التوصيل والملاحظات",
  "Delivery Address": "عنوان التوصيل",
  "Delivery Note": "ملاحظة التوصيل",
  Close: "إغلاق",
  "Hide This Product From My List": "إخفاء هذا المنتج من قائمتي",
  "Hide This Product": "إخفاء هذا المنتج",
  "Cancel This Order": "إلغاء هذا الطلب",
  "Cancel This Product": "إلغاء هذا المنتج",
  "You Can Cancel This Order And Back Your Money":
    "يمكنك إلغاء الطلب واسترجاع المبلغ",
  "You Can Change Size, Color, Qty Of  The Product Without Any Conditions According To The Change Policy":
    "يمكنك تغيير المقاس واللون وكمية المنتج دون أي شروط وفقًا لسياسة التغيير",
  "Change Qty": "تغيير الكمية",
  "Change Size": "تغيير الحجم",
  "Change Color": "تغيير اللون",
  "Change From": "من",
  "To New Color?": "إلى اللون الجديد؟",
  Clarification: "إيضاح",
  "About Return Your Product": "عن إرجاع المنتج",
  "You Will Not Be Charged Any Fees.": "لن يتم فرض أي رسوم عليك.",
  "Cancel Request": "إلغاء الطلب",
  "You Will Receive Your Refund Within 12 Hours.":
    "سوف تستلم على المبلغ المسترد  خلال 12 ساعة.",
  "Repeated Cancellations Will Affect Your Rating, Which Will Affect Your Ability To Receive New Offers Or Opportunities From Us.":
    "ستؤثر عمليات الإلغاء المتكررة على تقييمك، مما سيؤثر بدوره على قدرتك على تلقي عروض أو فرص جديدة منا.",
  "Repeated Returns Will Affect Your Rating, Which Will Affect Your Ability To Receive New Offers Or Opportunities From Us.":
    "ستؤثر عمليات الإرجاع المتكررة على تقييمك، مما سيؤثر بدوره على قدرتك على تلقي عروض أو فرص جديدة منا.",
  "Terms Of Return": "شروط الإرجاع",
  "Return Terms.": "شروط الإرجاع.",
  "I Agree & Cancel": "أوافق و ألغي",
  "I Disagree": "أنا أعترض",
  "Terms Of Cancellation Terms": "شروط الإلغاء",
  "Cancellation Terms": "شروط الإلغاء",
  "I Read And Agree To The": "لقد قرأت وأوافق على",
  "Terms Of Change Address": "شروط تغيير العنوان",
  "Change Request": "طلب تغيير",
  "Return This Product In": "إرجاع هذا المنتج في",
  "About Cancel Your Order": "حول إلغاء طلبك",
  "About Change Request Product": "حول طلب تغيير المنتج",
  "About Cancel Your Product": "حول إلغاء المنتج",
  "Cancel This Product In": "إلغاء هذا المنتج في",
  Invite: "دعوة",
  "Filter By brands": "تصفية حسب العلامات التجارية",
  "Filter By categories": "تصفية حسب الفئات",
  "Filter By colors": "تصفية حسب الألوان",
  "Filter By sizes": "تصفية حسب الأحجام",
  "Filter By boutiques": "تصفية حسب المتاجر",
  "Filter By Prices": "تصفية حسب الأسعار",
  "Filter By search_text": "تصفية حسب",
  "Filter By prices": "تصفية حسب الأسعار",
  "Total Products:": "إجمالي المنتجات:",
  "Please Review Your Cart Some Products Not Available":
    "الرجاء مراجعة السلة بعض المنتجات غير متوفرة",
  "You cannot send messages or calls to this user":
    "لا يمكنك إرسال رسائل أو مكالمات لهذا المستخدم",
  "Please Verify Your Phone Number And Try Again":
    "الرجاء تاكيد رقم الهاتف والمحاولة مجددا",
  "Please verify your phone number to continue":
    "الرجاء تاكيد رقم هاتفك للمتابعة",
  "Close": "إغلاق",
  "Modify Order": "تعديل الطلب",
  "Please select a reason for canceling this order":
    "الرجاء اختيار سبب لالغاء الطلب",
  "Manage Your Order": "إدارة الطلب",
  "Change Delivery Address & Note": "تغيير عنوان التسليم والملاحظة",
  "You Can Change Your Shipping Address And Delivery Notes":
    "يمكنك  تغيير عنوان التسليم ومذكرة التسليم",
  "Cancel Order": "إلغاء الطلب",
  "You Can Cancel & Back Your Money": "يمكنك إلغاء الطلب واسترجاع المبلغ",
  "Information About Your Order Modify Or Cancel":
    "معلومات عن طلبك تعديل أو إلغاء",
  "We Have Other Solutions Instead Of Cancellation.":
    "لدينا حلول أخرى بدلاً من إلغاء الطلب",
  "Learn More Tips.": "تعرف على المزيد من النصائح",
  "Why Was The Order Cancelled?": "لماذا تم إلغاء الطلب؟",
  "I Changed My Mind": "غيرت رأيي",
  "I Fear Quality": "أخشى جودة المنتج",
  "I Fear The Delivery Time": "أخشى زمن التوصيل",
  "I'm Afraid Of Sizes": "أخشى الأحجام",
  "I Saw A Better Price": "رأيت سعراً أفضل",
  "Shipping Cost": "تكلفة الشحن",
  "Syncing contacts...": "جاري مزامنة جهات الاتصال...",
  "Failed to sync contacts": "فشل مزامنة جهات الاتصال",
  "Add Contacts": "إضافة جهات الاتصال",
  "Get from your contacts": "احصل عليه من جهات اتصالك",
  "Enter name": "أدخل الاسم",
  "Enter phone number": "أدخل الهاتف",
  "Contacts API not supported in this browser":
    "API جهات الاتصال غير مدعومة في هذا المتصفح",
  "Session Expired..please Login again..Reloading...":
    "انتهت صلاحية الجلسة ,الرجاء تسجيل الدخول . إعادة التحميل",
  Reschedule: "تأجيل",
  "Please Select an Address": "الرجاء اختيار عنوان",
  "Please Be Accurate and select your Location":
    "كن دقيقا من فضلك باختيار موقعك الجغرافي",
  "Request submitted successfully": "تم إرسال طلبك بنجاح",
  "Request already submitted": "تم إرسال الطلب مسبقاً",
  "You have already submitted a seller request. We are processing it and will get back to you soon.":
    "لقد قمت بالفعل بإرسال طلب بائع. نحن نعالجه وسنعاود التواصل معك قريباً.",
  "Please fill in all fields": "يرجى تعبئة جميع الحقول",
  Documents: "المستندات",
  Remove: "حذف",
  "No documents uploaded": "لم يتم تحميل أي مستندات",
  "Document Type": "نوع المستند",
  "Choose File": "اختر ملف",
  "e.g., Trade License": "مثال: رخصة تجارية",
  "Upload Document": "رفع مستند",
  Uploading: "جاري الرفع",
  "Please select a file": "الرجاء اختيار ملف",
  "Please upload at least one document": "الرجاء تحميل مستند واحد على الأقل",
  "Personal Details": "البيانات الشخصية",
  "First Name": "الاسم الأول",
  "Last Name": "اسم العائلة",
  "Repeat Password": "أعد كلمة المرور",
  "Passwords do not match": "كلمات المرور غير متطابقة",
  "Enter a valid email address": "أدخل بريدًا إلكترونيًا صالحًا",
  "Password is required": "كلمة المرور مطلوبة",
  "Min 8 characters, including a letter and a number":
    "8 أحرف على الأقل، تتضمن حرفًا ورقمًا",
  "Please repeat your password": "يرجى إعادة إدخال كلمة المرور",
  "Shop Information": "معلومات المتجر",
  "Shop Address": "عنوان المتجر",
  "Location Details": "تفاصيل الموقع",
  "Location Name": "اسم الموقع",
  "Location Address": "عنوان الموقع",
  Submit: "إرسال",
  "Failed to recognize your voice": "فشل التعرف على الصوت",
  "Full Name": "الاسم الكامل",
  "Wallet Already Selected": "المحفظة محددة بالفعل",
  Verified: "تم التحقق",
  "Discount Coupon": "قسيمة خصم",
  "Your Size Info": "معلومات الحجم",
  "Height is required": "الطول مطلوب",
  "Weight is required": "الوزن مطلوب",
  "Height must be between 110 and 250 cm": "يجب أن يكون الطول بين 110 و 250 سم",
  "Weight must be between 40 and 180 kg": "يجب أن يكون الوزن بين 40 و 180 كجم",
  "000 CM": "000 سم",
  "Loading...": "جاري التحميل...",
  "No more orders": "لا يوجد طلبات أخرى",
  "Order Details": "تفاصيل الطلب",
  "Order Date": "تاريخ الطلب",
  Recipient: "المستلم",
  "Recipient Phone": "رقم الهاتف",
  "Orders Details": "تفاصيل الطلب",
  "Item Status": "حالة المنتج",
  "Composed Of": "مكون من",
  Buying: "شراء",
  "Expected Delivery Date": "تاريخ الشحن والتسليم المتوقع",
  "Order Status": "حالة الطلب",
  Pieces: "قطعة",
  "Work Days": "أيام العمل",
  "Order Invoice": "فاتورة الطلب",
  "Order Number": "رقم الطلب",
  "What Is Your Weight?": "كم وزنك؟",
  "No orders found for this status.": "لا يوجد طلبات لهذا الحالة",
  "Order Not Found": "الطلب غير موجود",
  "This order doesn't exist or the link is incorrect.":
    "هذا الطلب غير موجود أو الرابط غير صحيح.",
  "Go to My Orders": "الذهاب إلى طلباتي",
  "Available Countries": "البلدان المتاحة",
  "Profile | Countries": "الملف الشخصي | البلدان",
  "Profile | Languages": "الملف الشخصي | اللغة",
  Languages: "لغات",
  Preparing: "تحضير",
  "000 KG": "000 كجم",
  "How Tall Are You?": "كم طولك؟",
  "Your Address Info": "معلومات العنوان",
  "Profile | Size Info": "الملف الشخصي | الحجم",
  "Profile | Address Info": "الملف الشخصي | العنوان",
  "Profile | Bank Cards": "الملف الشخصي | بطاقات البنك",
  "Your Bank Cards Info": "معلومات بطاقات البنك",
  All: "الكل",
  Pending: "قيد الانتظار",
  Processing: "قيد المعالجة",
  "Ready to Shipping": "قيد التحضير للشحن",
  Shipped: "تم الشحن",
  "Out for Delivery": "قيد التوصيل",
  Delivered: "تم التوصيل",
  "Partial Return": "إرجاع جزئي",
  Returned: "مرتجع",
  Failed: "فشل",
  Canceled: "ملغي",
  "Canceled Archived": "ملغي ومؤرشف",
  Gender: "الجنس",
  Man: "ذكر",
  Woman: "أنثى",
  Other: "غير ذلك",
  "Enter Email": "ادخل البريد الإلكتروني",
  Email: "البريد الإلكتروني",
  "Enter Alternative Phone": "ادخل رقم الهاتف البديل",
  Phone: "الهاتف",
  Action: "عملية",
  "Enter Phone": "ادخل الهاتف",
  "Enter Full Name": "ادخل الاسم الكامل",
  "Personal Info": "المعلومات الشخصية",
  Size: "القياس",
  "Profile | Address": "الملف الشخصي | العنوان",
  "Profile | Personal Info": "الملف الشخصي | المعلومات الشخصية",
  "Add Profile Picture": "إضافة الصورة الشخصية",
  "Uploading Profile Photo …": "تحميل الصورة الشخصية ...",
  "Remove Photo": "حذف الصورة",
  "Choose From Library": "اختيار من ملفاتي",
  "take photo": "التقاط صورة",
  "Copy Invite": " نسخ الدعوة",
  "Choose how to send the invitation": "اختر كيفية إرسال الدعوة",
  "Confirm Return Request": "تأكيد طلب الإرجاع",
  "Cancel Return Request": "إلغاء طلب الإرجاع",
  "Shared a product": " شارك منتجًا",
  Address: "العنوان",
  "Bank Cards": "بطاقات البنك",
  "Share App": "مشاركة التطبيق",
  "About Us": "حولنا",
  "Terms & Conditions": "الشروط والاحكام",
  "Add Size": "إضافة الحجم",
  "Legal Information": "المعلومات القانونية",
  "Verify Now": "تأكيد الان",
  "No Address Selected": "لم يتم اختيار عنوان",
  "Only Allowed To Pay through TryDos Wallet":
    "يسمح فقط بالدفع عبر محفظة ترايدوس",
  "Product Restricted in This Country": "المنتج محظور في هذا البلد",
  "Product Not available": "المنتج غير متوفر",
  "Please Enter Shipping Address To Receive Your Bag":
    "الرجاء ادخال عنوان التوصيل لاستلام المشتريات",
  "Shipment Will Be Sent To The Address Below":
    "سيتم إرسال الشحنة إلى العنوان أدناه",
  "Your Shopping Bag": "سلة التسوق",
  Compare: "مقارنة",
  "Added To Compare! Click To Go To Compare Page":
    "تم إضافة للمقارنة! انقر للذهاب إلى صفحة المقارنة",
  "Already Added To Compare!": "تم إضافة للمقارنة",
  "Added To Compare": "تم إضافة للمقارنة",
  "Total Items": "إجمالي المنتجات",
  "One of the products was not found. Please try searching for a different product.":
    "لم يتم العثور على أحد المنتجات. يرجى البحث عن منتج آخر.",
  "Please Enter Coupon Information": "أدخل معلومات الكود",
  "Applying...": "يتم التطبيق...",
  "I Have a Discount Coupon": "لدي كود خصم",
  "Not Available Now, Stock Is Sold Out": "غير متوفر حاليا, الكمية مباعة",
  "No Results Found": "لا يوجد نتائج",
  Reset: "إعادة تعيين",

  "Search History": "سجل البحث",
  "Popular Search": "البحث الشائع",
  "Clear All": "مسح الكل",

  pending: "قيد الانتظار",
  ready_to_shipping: "قيد التحضير للشحن",
  out_for_delivery: "قيد التوصيل",
  delivered: "تم التوصيل",
  partial_return: "إرجاع جزئي",
  returned: "مرتجع",
  canceled_archived: "ملغي ومؤرشف",
  failed: "فشل",
  paid: "مدفوع",
  unpaid: "غير مدفوع",
  "Payment Method": "طرق الدفع",
  Country: "البلد",
  "Shipping Address": "عنوان التوصيل",
  name: "الاسم",
  phone: "رقم الهاتف",
  email: "البريد الإلكتروني",
  Profile: "الملف الشخصي",
  Save: "حفظ",
  "Click to view details": "اضغط لعرض التفاصيل",
  Notifications: "الإشعارات",
  Settings: "الإعدادات",
  Orders: "الطلبات",
  Language: "اللغة",
  Logout: "تسجيل الخروج",
  "Sorry This Product Not Available In Your Country":
    "عذرا هذا المنتج غير متوفر في بلدك",
  "Sorry This Product Not Available Now": "عذرا هذا المنتج غير متوفر حاليا",
  "Shipping & Payment": "الدفع والتوصيل",
  Done: "تم",
  Availabilty: "التوفر",
  "Out Of Stock": "انتهى من المخزون",
  "Not Available In Your Country": "غير متوفر في بلدك",
  "Not Available Now": "غير متوفر حالياً",
  "These Shots Are Made By Users Who Have Already Purchased And Received The Product":
    "هذه الصور تم التقاطها من قبل مستخدمين قاموا بشراء المنتج واستلامه",
  "Please Choose Your Payment Method About Your Bag":
    "يرجى اختيار طريقة الدفع الخاصة بحقيبتك",
  "Trydos Wallet": "محفظة ترايدوس",
  "Credit Cards": "بطاقات الائتمان",
  Crypto: "عملة مشفرة",
  "Your Balance": "رصيدك",
  "Cash On Delivery": "دفع بعد التوصيل",
  "I Have Discount Coupon": "عندي قسيمة خصم",
  Apply: "موافق",
  "Coupon No": "رقم القسيمة",
  "Confirm Shipping & Payment": "تأكيد الشحن والدفع",
  "I read and agree to the": "قرأت ووافقت على",
  policies: "السياسات",
  terms: "الشروط",
  and: "و",
  "Place Order": "تقديم الطلب",
  "The Purchase Was Completed Successfully": "تمت عملية الشراء بنجاح",
  "Your Order Number": "رقم الطلب",
  "You Can Track The Status Of Your Order Through ":
    "يمكنك تتبع حالة طلبك من خلال",
  "My Account / My Orders": "حسابي / طلباتي",
  "Find Products": "ايجاد منتجات",
  "Find Brands": "ايجاد ماركات",
  "Find Categories": "ايجاد تصانيف",
  "Find Boutiques": "ايجاد البوتيكات",
  "Bag Shipping & Delivery Address": "عناوين التوصيل والشحن",
  "You Can Also Create Multiple Addresses To Use":
    "يمكنك انشاء عدة عناوين لاستخدامها",
  "Your Address List": "قائمة عناوينك",
  "Entering The Information Below Clearly And Completely Will Ensure That Your Order Arrives Without Problems And Faster.":
    "ادخال المعلومات ادناه بشكل واضح وكامل يضمن ايصال طلبك بشكل اسرع وبدون مشاكل",
  "Locate Your Location On Map": "حدد موقعك على الخريطة",
  "Location Is Accurate, Making It Easy To Receive Shipments":
    "الدقة في تحديد المزقع تسهل عملية استلام الطلب",
  "Address Info": "معلومات العنوان",
  "Country | Region": "البلد | المنطقة",
  "Change From List": "اختر من القائمة",
  "Detailed Address & Note": "العنوان بالتفصيل",
  "Write The Address Clearly, Including The Street Address, Building, Flat, Door, Unit.":
    "ادخل العنوان بشكل واضح متضمنا رقم الشارع,البناء,الشقة,الطابق ..",
  "Address Title": "اسم العنوان",
  "Ex: Home, My Office, 2 Home Ect.": "مثلا: المنزل,المكتب ..",
  "Recipient Name": "اسم المستلم",
  "Enter Full Recipient Name": "ادخل الاسم الكامل للمستلم",
  "Contact Phone": "رقم الهاتف",
  "Enter Recipient Phone": "ادخل رقم الهاتف",
  "Alternative Phone": "رقم هاتف بديل",
  "(Optional)": "(اختياري)",
  "Enter Alternative Recipient Phone": "ادخل رقم هاتف بديل",
  "Add & Save": "اضافة و حفظ",
  "Edit & Save": "تعديل و حفظ",
  Select: "اختيار",
  Cancel: "الغاء",
  "Delete Below Address?": "حذف العنوان ادناه ؟",
  "Yes Delete": "نعم , حذف",
  "Delivery Guarantee": "ضمان التسليم",
  Delivery: "توصيل",
  "Return Guarantee": "ضمان الاسترجاع",
  within: "خلال",
  Days: "أيام",
  "Secure Privacy": "الخصوصية الآمنة",
  "Safe & Easy Payment": "دفع آمن وسهل",
  "Purshase Protection": "حماية المشتريات",
  "Earn Money With This Order": "كسب المال مع هذا الطلب",
  "Contact Info": "معلومات التواصل",
  "Add New Shipping Address": "اضافة عنوان شحن جديد",
  "Shipping & Delivery Address": "عناوين الشحن والتوصيل",
  "stock is limited": "لا يوجد كمية اضافية من المنتج",
  "Continue as Guest": "الاستمرار كـ ضيف",
  "Name Should be atleast 8 characters":
    "الاسم يجب ان يتالف من 8محارف على الاقل",
  Items: "عناصر",
  "Shopping Bag": "سلة التسوق",
  "your session has expired please login again or continue as guest":
    "لقد انتهت جلستك، يرجى تسجيل الدخول مرة أخرى أو الاستمرار كضيف",
  "Your session has expired": "لقد انتهت جلستك",
  "Please login again to get back to your account, or continue browsing as a guest.":
    "يرجى تسجيل الدخول مرة أخرى للعودة إلى حسابك، أو الاستمرار في التصفح كضيف.",
  "enter number of pieces you want to return":
    "ادخل عدد القطع التي ترغب بإرجاعها",
  "Please Select The Appropriate": "الرجاء اختيار",
  "Enter Your Phone Number To Complete Order":
    "الرجاء ادخال الرقم لاكمال الطلب",
  Buy: "شراء",
  "Composed Of:": "مكون من",
  "change address request": "طلب تغيير العنوان",
  "Change Below Address": "تغيير العنوان أدناه",
  "To New Address": "إلى العنوان الجديد",
  "The Change Addres Terms.": "شروط تغيير العنوان.",
  "We Will Ignore The First Address And Send Your Order To The New Address.":
    "سوف نتجاهل العنوان الأول ونرسل طلبك إلى العنوان الجديد.",
  "Agree & Change": "أوافق وأغير",
  Piece: "قطعة",
  "Hide All": "إخفاء الكل",
  Shipping: "الشحن",
  "Cart is Empty": "السلة فارغة",
  "Hurry Up!": "أسرع!",
  "Quantity Running Out. ": "الكمية تنفذ.",
  "Time Running Out. ": "الوقت ينفذ.",
  "Confirm & Continue": "ناكيد & المتابعة",
  items: "عناصر",
  Size: "حجم",
  "color,": "لون, ",
  Saved: "توفير",
  "Time Running Out.": "الوقت ينفذ",
  "Add Again?": "الإضافة مجدداً؟",
  Hide: "إخفاء",
  "Back To HomePage": "العودة إلى الرئيسية",
  "Out Of Bag!": "خارج السلة!",
  Details: "تفاصيل",
  color: "لون",
  "Add To Bag": "اضافة الى السلة",
  "All Inclusive Without Additions": "شامل كل شيء بدون إضافات",
  "Ship To You Accepted": "تم قبول الشحن إليك في",
  "type a comment": "اكتب تعليقاً",
  "Free Shipping": "شحن مجاني",
  "Shipping Is Completely Free Without Any Extras":
    "الشحن مجاني بالكامل بدون أي إضافات",

  Gift: "هدية",
  "First Shopping": "أول شراء",
  Item: "عنصر",
  Total: "المجموع",
  Price: "السعر",
  "Normal Price": "السعر العادي",
  "Total Discount": "مجموع الحسومات",
  "Click To Show All Discount": "اضغط لإظهار كل الحسومات",
  "Return Is Completely Free Without Any Extras":
    "الإعادة مجانية بالكامل بدون أي إضافات",
  "Within 3 Days After Receiving The Product, You Can Return It Without Conditions Or Reasons With Complete Ease And Get The Amount Back":
    "خلال 3 أيام من استلام المنتج يمكنك إرجاعه بدون شروط أو أسباب بكل سهولة واستعادة المبلغ",
  "Reach End": "آخر الصفحة",
  "You've reached the end": "لقد وصلت إلى النهاية",
  "You've seen everything in this list.": "لقد شاهدت كل ما في هذه القائمة.",
  "No products found": "لا توجد منتجات",
  "Try changing or clearing your filters.": "جرّب تغيير عوامل التصفية أو مسحها.",
  "Clear filters": "مسح عوامل التصفية",
  "Notify Me About The Product When": "أرسل لي إشعار عن هذا المنتج عندما",
  "Before Stock Out": "يكون على وشك النفاذ",
  "Change In Price": "يتغير السعر",
  Discounts: "يحدث تخفيضات",
  "Follow Comments": "شخص ما يعلق",
  Offers: "تحدث عروض",
  "Add To My Checklist": "إضافة إلى المفضلة",
  "My Checklist": "قائمتي",
  "Add To Compare": "إضافة إلى المقارنة",
  Recommended: "مقترح",
  "For You": "لك",
  "Need Help Finding Your Size?": "هل تحتاج مساعدة في إيجاد قياسك؟",
  "More Options": "خيارات إضافية",
  "Share This Product With": "مشاركة هذا المنتج مع",
  "Comment About This Product": "التعليق على هذا المنتج",
  "Good Quality Product": "منتج بجودة عالية",
  "Good Quality": "جودة عالية",
  "True Size": "حجم حقيقي",
  "Recommend It": "ينصح به",
  "Verified by trydos": "تم التحقق بواسطة ترايدوس",
  "Made In Turkey": "صنع في تركيا",
  "Made In {country}": "صنع في {country}",
  "You May Also Like": "قد يعجبك أيضاً",
  "Free Return": "ارجاع مجاني",
  "Read More": "عرض المزيد",
  "Read Less": "عرض اقل",
  "Product Shipping & Delivery": "شحن المنتج والتوصيل",
  "Expected Within": "متوقع خلال",
  Days: "أيام",
  "At Your Address In": "إلى عنوانك في",
  Lebanon: "لبنان",
  "Specify Your Address To Calculate The Delivery Time":
    "حدد عنوانك لحساب وقت التوصيل",
  "Product Story": "قصص المنتج",
  "You Will Get A": "سوف تستلم",
  Refund: "استعادة",
  "Of The Product Price If Shipping Is Delayed":
    "من سعر المنتج بحالة تأخر الشحن",
  Color: "لون",
  Sizes: "قياس",
  Shot: "صورة",
  "Buyers Camera": " صور الزبائن ",
  "Available ": " متوفر ",
  "In Your Address": "في عنوانك",
  "In Lebanon": "في لبنان",
  "in one word what is in this photo?":
    "في كلمة واحدة ماذا يوجد في هذه الصورة أجبني باللغة العربية؟",

  "To Take Advantage Of All The Advantages Of The Application, Please Join Us In Quick And Easy Steps And For Just One Time":
    "للاستفادة من ميزات التطبيق انضم لنا بخطوات سربعة وبسيطة لمرة واحدة",
  "Terms Of Services ": "قواعد الاستخدام",
  "I have Already Account": "لدي حساب بالفعل",
  "Your Privacy Is Completely Safe, We Not Share Your Information With Anyone":
    "خصوصيتك امنة تماما,نحن لن نشارك معلوماتك مع اي احد",
  "To ": "لـ",
  "Logged In Successfully !": "تم تسجيل الدخول بنجاح",
  "Hello,": "مرحبا ,",
  "We Recommend That You Complete Your Profile To Make The Most Of The App's Features, Such As Shopping, Chatting, Stories, Taking Advantage Of Offers, Interests, And Much More":
    "ننصح باكمال معلومات الحساب للاستفادة من ميزات التطبيق كـ التسوق ,الدردشة ,القصص, الاهتمامات,الاستفادة من العروض وغيرها الكثير",
  "Complete My Profile": "اكمال ادخال معلومات الحساب",
  "Shop Info":"معلومات المتجر",
  "Enjoy With Our Services": "استمتع بخدماتنا",
  "Skip For Now": "تخطي",
  "Choose The Verification Method, Receive Code Via:":
    "اختر طريقة للتحقق لارسال الرمز عبر",
  "Agree & Continue": "موافق ومتابعة",
  'Tap "Agree & Continue" To Accept trydos':
    "انقر على موافق لقبول سياسة الخصوصية لترايدوس",
  "Terms Of Services": "شروط الاستخدام",
  "Create New Account ": "انشاء حساب ",
  "No Answer": "لا استجابة",
  "Enter Your Phone Number To Login": "ادخل رقمك لتسجيل الدخول",
  "the Method Of Receiving": "طريقة الاستلام",
  "Invalid Phone Number": "رقم خاطئ",
  "Later, Take A Look At The Site": "لاحقا, القي نظرة على الموقع",
  "The Code Sent Has Expired": "انتهت فترة استلام الرمز",
  "Didn't You Receive A Code?": "لم تستلم رمزا؟",
  Change: "تغيير",
  "We Will Send A Verification Code To The Number":
    "سنرسل رمز تاكيد الى هذا الرقم",
  "Please Enter The Verification Code Sent To Your Whatsapp":
    "الرجاء ادخال رمز التاكيد المرسل لك عبر واتساب",
  "Last Step And Enjoy Our Services": "خطوة اخيرة واستمتع بخدماتنا",
  "Enter Your Name": "ادخل اسمك",
  "The Number Verified Successfully !": "تم تاكيد الرقم بنجاح",
  "Login & Continue": "متابعة وتسجبل الدخول",
  "This Number Already Registered With Us !": "هذا الرقم مرتبط بحساب اخر",
  "You Can Log In Now": "يمكنك تسجيل الدخول الان",
  "Register & Create New Account With Us In A Few Simple Steps":
    "سجل وانشئ حساب جديد بخطوات بسيطة",
  "Cancel & Take A Look At The Site": "خروج والقاء نظرة على الموقع",
  "Create New Account & Continue": "انشاء حساب ومتابعة",
  "Voice Call": "مكالمة صوتية",
  "Video Call": "مكالمة فيديو",
  "User declined": "تم انهاء المكالمة",
  "Calling ...": "جاري الاتصال...",
  "End Call": "انهاء المكالمة",
  "Contacts List": "جهات الاتصال",
  "Search, Chat, Contact, Start New Chat":
    "ابحث عن دردشة ,جهة اتصال,أو ابدأ محادثة جديدة",
  Sale: "تخفيض",
  "Initialize Call please wait..": "تهيئة الاتصال الرجاء الانتظار",
  "Call Answered from another account": "تم الرد من حساب اخر",
  "This Offer Is For Only": "هذا العرض فقط لـ",
  Hours: "ساعات",
  "user not found": "لم يتم العثور على حساب مرتبط بهذا الرقم",
  Remaining: "متبقي",
  More: "المزيد",
  "failed to send otp code please try again":
    "فشل ارسال الرمز يرجى اعادة المحاولة لاحقاً",
  Second: "تخفيض اضافي",
  "Buy 1 gift 1": "اشتري 1 واربح 1 ",
  Category: "الأصناف",
  "Mango Famous Turkish Brand Best Discounts":
    "أفضل التخفيضات من العلامة التركية المشهورة Mango",
  "Mango Famous Turkish Brand Best Offers":
    "أفضل العروض من العلامة التركية المشهورة Mango",
  "Products With Great Fast And Limited Offers":
    "منتجات بعروض رائعة وسريعة ولفترة محدودة",
  "Quick Offer": "عرض سريع",
  "Products With Great Offers": "منتجات بعروض رائعة",
  "Enjoy Shopping From All Categories & Products From Various Brands":
    "استمتع بالتسوق من جميع الأصناف والمنتجات من مختلف العلامات التجارية",
  Brands: "العلامات التجارية",
  Offer: "عرض",
  "Best Offers From Brands": "أفضل العروض من العلامات التجارية",
  "this message was deleted": "تم حذف هذه الرسالة",
  Kids: "أطفال",
  "You will be notified for this product already":
    "لقد قمت بالفعل بتفعيل الاشعارات لهذا المنتج",
  Home: "منزلي",
  Electrical: "كهربائيات",
  Shop: "متجر",
  Welcome: "أهلا بك",
  Hello: "مرحبا",
  "Why We Know You ?": "لماذا نريد معرفتك ",
  "Can We Know You ?": "هل نستطيع معرفتك ",
  Login: "تسجيل الدخول",
  Search: "بحث",
  "By Mobile Phone Number": "عن طريق رقم الهاتف الشخصي",
  "By Scan Qr From Trydos App": "عن طريق مسح رمز من تطبيق ترايدوس",
  "Scan This Qr Code From You Trydos App In Your Phone":
    "قم بمسح هذا الرمز عبر تطبيق ترايدوس من هاتفك",
  "Enter Your Phone Number Registered With Us":
    "ادخل رقم هاتفك الشخصي المسجل به معنا",
  "Don't Have Account?": "لا تملك حسابا؟",
  "Create New Account": "أنشئ حساباً جديداً",
  "Choose The Verification Method": "أختر طريقة التحقق",
  "Send Verification Code To": "ارسال رسالة التحقق إلى",
  SMS: "رسالة",
  WhatsApp: "واتساب",
  "Please Enter The Verification Code Sent To Your Phone":
    "ادخل رمز التحقق الذي ارسلناه لهاتفك",
  "You Can Resend The Code After": "يمكنك اعادة ارسال الرمز بعد ",
  "Resend Code": "اعادة ارسال الرمز",
  "Sorry, This Number Is Not Registered With Us !":
    "عذراً، لم نتمكن من العثور على حساب مرتبط بهذا الرقم",
  "Register With Us In A Few Simple Steps": "قم بالتسجيل ببضع خطوات بسيطة",
  "Please Enter The Correct Code Sent To Your Phone":
    " رجاءً ادخل الرمز الصحيح الذي تم ارساله لهاتفك",
  "You Have": "لديك",
  Attempts: "محاولات",
  "Change The Method Of Receiving": "تغيير طريقة استلام الرمز",
  Copy: "نسخ",
  Reply: "رد",
  Forward: "تحويل",
  CategoryMessage: "تصنيف",
  Reminder: "تذكير",
  Edit: "تعديل",
  Delete: "حذف",
  Archive: "أرشفة",
  Unmute: "الغاء الكتم",
  Mute: "كتم",
  Pin: "تثبيت",
  Unpin: "الغاء التثبيت",
  Read: "تحديد كـ مقروء",
  Unread: "تحديد كـ غير مقروء",
  Sunday: "الأحد",
  Monday: "الاثنين",
  Tuesday: "الثلاثاء",
  Wednesday: "الاربعاء",
  Thursday: "الخميس",
  Friday: "الجمعة",
  Saturday: "السبت",
  Yesterday: "الأمس",
  Video: "فيديو",
  File: "ملف",
  Audio: "صوت",
  "Incoming Voice Call..": "مكالمة صوتية واردة..",
  "Incoming Video Call..": "مكالمة فيديو واردة ..",
  Calling: "جاري الاتصال ...",
  "Missed Call": "مكالمة فائتة",
  "Incoming Call": "مكالمة واردة",
  "Outgoing Call": "مكالمة صادرة",
  "Please Enable Notification to use Chat":
    "الرجاء السماح للاشعارات لاستخدام الدردشة",
  OK: "موافق",
  "No Contacts": "لا يوجد جهات اتصال",
  "Log in Through our App to access contacts":
    "قم بالدخول من خلال التطبيق للوصول الى جهات الاتصال",
  "Fast Packing & Start Shipping": "تحميل سريع وسرعة في الشحن",
  "Same Day Packing & Ship If Buy Before":
    "التحميل والشحن بنفس اليوم إذا تمَ الشراء قبل",
  Today: "اليوم",
  "Time Is Expected, It May Take More Or Less Than 2 Days":
    "الوقت تقديري قد يستغرق أكثر أو أقل من يومين",
  "We Will Inform You When this Is Available": "سنعلمك عند توفر المنتج",
  "Notify Me When Product Is Available": "ارسل اشعار عند توفر المنتج",
  "Notify Me": "ارسل اشعارا",
  "Notifications Settings": "إعدادات الإشعارات",
  "Notification Test": "اختبار الإشعارات",
  "Enabled Notifications Topic:": "مواضيع الإشعارات المفعلة:",
  "Disabled Notifications Topic:": "مواضيع الإشعارات المعطلة:",
  "Unsubscribing...": "جاري إلغاء الاشتراك...",
  Unsubscribe: "إلغاء الاشتراك",
  "Loading Topics...": "جاري تحميل المواضيع...",
  "No topics subscribed.": "لا توجد مواضيع مشتركة.",
  "Subscribing...": "جاري الاشتراك...",
  Subscribe: "اشتراك",
  "notifications subscription:": "اشتراك الإشعارات:",
  "Enable Email Notifications": "تفعيل إشعارات البريد الإلكتروني",
  "Enable FireBase Notifications": "تفعيل إشعارات FireBase",
  "Enable WhatsApp Notifications": "تفعيل إشعارات واتساب",
  "notifications Receiving Preference:": "تفضيلات استلام الإشعارات:",
  "Select An Option": "اختر خياراً",
  daily: "يومياً",
  weekly: "أسبوعياً",
  monthly: "شهرياً",
  "Continue with": "متابعة مع",
  OR: "أو",
  "Select Your Country": "اختر بلدك",
  uk: "المملكة المتحدة",
  uae: "الإمارات",
  "Name is required": "الاسم مطلوب",
  "Name must be at least 2 characters": "يجب أن يتكون الاسم من حرفين على الأقل",
  "Name must be less than 50 characters": "يجب أن يكون الاسم أقل من 50 حرفاً",
  "Must not exceed 10 characters": "يجب ألا يتجاوز 10 أحرف",
  "Email is required": "البريد الإلكتروني مطلوب",
  "Please enter a valid email address": "الرجاء إدخال عنوان بريد إلكتروني صحيح",
  "Phone number is required": "رقم الهاتف مطلوب",
  "Please enter a valid phone number": "الرجاء إدخال رقم هاتف صحيح",
  "Change Profile Photo": "تغيير صورة الملف الشخصي",
  "Upload Photo": "رفع صورة",
  "From Files": "من الملفات",
  from: "من",
  "From Camera": "من الكاميرا",
  "Change Photo": "تغيير الصورة",
  "JPG, PNG or GIF (max. 5MB)": "JPG أو PNG أو GIF (الحد الأقصى 5 ميجابايت)",
  CheckList: "قائمة الرغبات",
  "Your CheckList is empty": "قائمة رغباتك فارغة",
  "Compare Products": "مقارنة المنتجات",
  "Search for Product 1": "البحث عن المنتج الأول",
  "Search for Product 2": "البحث عن المنتج الثاني",
  Name: "الاسم",
  Image: "الصورة",
  "Pick Your Deleivery Location Inside Your Country":
    "اختر عنوان التوصيل ضمن حدود بلدك",
  Colors: "الألوان",
  "Offer Price": "سعر العرض",
  "Order Number has been Copied": "تم نسخ رقم الطلب",
  "your TryDos Wallet balance is empty": "لا يوجد رصيد في محفظتك",
  "Product Comparison": "مقارنة المنتجات",
  "Loading your selected products for a detailed side-by-side comparison. This will help you make an informed decision by comparing features, prices, and specifications.":
    "جاري تحميل المنتجات المحددة لمقارنة مفصلة جنباً إلى جنب. سيساعدك هذا في اتخاذ قرار مستنير من خلال مقارنة الميزات والأسعار والمواصفات.",
  "Your Current Location is Not belong to Country Bounds":
    "موقعك الحالي لا يقع ضمن حدود البلد المختار",
  // Alt attributes
  sy: "سوريا",
  "info icon": "أيقونة المعلومات",
  login: "تسجيل الدخول",
  offer: "عرض",
  avatar: "صورة رمزية",
  story: "قصة",
  user: "مستخدم",
  imgs: "صور",
  Preview: "معاينة",
  "TryDos Logo": "شعار ترايدوس",
  truck: "شاحنة",
  deliveryman: "موصل",
  Omar: "عمر",
  image: "صورة",
  "buy Button": "زر الشراء",
  Upload: "رفع",
  TryDos: "ترايدوس",
  // Console messages
  "Server cookie API failed:": "فشل خادم ملفات تعريف الارتباط:",
  "Navigation blocked - already in progress":
    "التنقل محظور - قيد التقدم بالفعل",
  "Navigating to:": "الانتقال إلى:",
  "FCM token is older than 1 day. Refreshing...":
    "رمز FCM أقدم من يوم واحد. جاري التحديث...",
  "Notification permission denied or dismissed.":
    "تم رفض إذن الإشعار أو تجاهله.",
  "Do not close this window": "لا تغلق هذه النافذة",
  "Preparing your experience...": "جاري تحضير تجربتك...",
  // Date
  "2 June": "2 يونيو",
  // Messages
  "User In Another Call": "المستخدم في مكالمة أخرى",
  "Initializing...": "جاري التهيئة...",
  "Loading countries...": "جاري تحميل البلدان...",
  "Preparing options...": "جاري إعداد الخيارات...",
  "Almost ready...": "شبه جاهز...",
  "Ready!": "جاهز!",
  "Please wait while we set up your country selection":
    "يرجى الانتظار بينما نقوم بإعداد اختيار بلدك",
  "You previously visited from": "لقد زرت سابقاً من",
  "but now accessing from": "لكن الآن تدخل من",
  "Choose your country to get the best experience":
    "اختر بلدك للحصول على أفضل تجربة",
  "Loading...": "جاري التحميل...",
  "No more notifications": "لا توجد إشعارات أخرى",
  "Currency Request": "طلب العملة",
  January: "يناير",
  February: "فبراير",
  March: "مارس",
  April: "أبريل",
  May: "مايو",
  June: "يونيو",
  July: "يوليو",
  August: "أغسطس",
  September: "سبتمبر",
  October: "أكتوبر",
  November: "نوفمبر",
  December: "ديسمبر",
  M: "م",
  K: "أ",
  m: "م",
  k: "أ",
  "Info Message": "رسالة معلومات",
  "Entering The Information Below Clearly And Completely Will Ensure That Your Order Arrives Without Problems And Faster.":
    "إدخال المعلومات أدناه بشكل واضح وكامل يضمن وصول طلبك بشكل أسرع وبدون مشاكل.",
  "Notification Premission:": "إذن الإشعارات:",
  Enabled: "مفعل",
  "Not Enabled": "غير مفعل",
  "FireBase Supported": "دعم Firebase",
  Supported: "مدعوم",
  "Not Supported": "غير مدعوم",
  "User ID:": "معرف المستخدم:",
  "FCM Token:": "رمز FCM:",
  "FCM Error:": "خطأ FCM:",
  "Notification Settings": "إعدادات الإشعارات",
  Product: "منتج",
  "this message was deleted": "تم حذف هذه الرسالة",
  "You will be notified for this product already":
    "لقد قمت بالفعل بتفعيل الإشعارات لهذا المنتج",
  "Session Expired..please Verify Your Phone Number":
    "انتهت صلاحية الجلسة.. يرجى التحقق من رقم هاتفك",
  "Failed to update like status. Please try again.":
    "فشل تحديث حالة الإعجاب. يرجى المحاولة مرة أخرى",
  "Product Not Found": "المنتج غير موجود",
  "Sorry, we couldn't load this product. It might have been removed or is temporarily unavailable.":
    "عذراً، لم نتمكن من تحميل هذا المنتج. قد يكون قد تم إزالته أو غير متاح مؤقتاً.",
  "Don't worry, you can try again or browse our other amazing products.":
    "لا تقلق، يمكنك المحاولة مرة أخرى أو تصفح منتجاتنا الرائعة الأخرى.",
  "Back to Home": "العودة إلى الصفحة الرئيسية",
  "Try Again": "حاول مرة أخرى",
  "Retrying...": "إعادة المحاولة...",
  "Trydos — Flash Deals, Boutique Finds & Featured Products":
    "تريدوس — عروض سريعة، اكتشافات البوتيك والمنتجات المميزة",
  Trydos: "تريدوس",

  Bar: "بار",
  Boutiques: "بوتيكات",

  "Online Shopping": "تسوق عبر الإنترنت",
  TryDos: "تريدوس",
  "online shopping": "تسوق عبر الإنترنت",
  "premium products": "منتجات مميزة",

  "flash deals": "عروض سريعة",
  "best deals": "أفضل العروض",
  "shopping mall": "مركز تسوق",
  boutiques: "بوتيكات",
  brands: "علامات تجارية",
  fashion: "أزياء",
  electronics: "إلكترونيات",
  "home garden": "المنزل والحديقة",
  "Trydos — Flash Deals & Featured Boutique Picks":
    "تريدوس — عروض سريعة واختيارات بوتيك مميزة",
  "Your destination for exclusive bar items, boutique fashion, and daily flash deals.":
    "وجهتك للعناصر الحصرية للبار، أزياء البوتيك، والعروض السريعة اليومية.",
  "Trydos — Shop Boutique Products & Flash Deals":
    "تريدوس — تسوق منتجات البوتيك والعروض السريعة",
  "Explore Trydos for curated categories like Bar, Boutiques, and limited-time Flash Deals.":
    "استكشف تريدوس لفئات مختارة مثل البار، البوتيكات، والعروض السريعة محدودة الوقت.",
  "Discover Trydos: your online store for bar items, boutique products, and flash deals.":
    "اكتشف تريدوس: متجرك الإلكتروني لعناصر البار، منتجات البوتيك، والعروض السريعة.",
  "our top categories": "أفضل فئاتنا",
  "trusted brands": "علامات تجارية موثوقة",
  "all standard sizes": "جميع المقاسات القياسية",
  "all colors": "جميع الألوان",
  "Search results for ": "نتائج البحث عن ",
  Shop: "تسوق",
  From: "من",
  "Discover a curated selection of": "اكتشف مجموعة مختارة من",
  Featuring: "تحتوي على",
  "and prices from": "والأسعار من",
  to: "إلى",
  on: "على",
  "Search results for": "نتائج البحث عن",
  "Trydos -": "تريدوس -",
  "Discover New ": "اكتشف الجديد ",
  "Tags: ": "العلامات: ",
  "Boutiques: ": "البوتيكات: ",
  "Categories: ": "الفئات: ",
  "Brands: ": "العلامات التجارية: ",
  "Colors: ": "الألوان: ",
  "Sizes: ": "المقاسات: ",
  "Products on Trydos": "المنتجات على تريدوس",
  Trydos: "تريدوس",
  "View More": "عرض المزيد",
  "Only This Piece": "فقط هذه القطعة",
  "SVG Images Not Allowed": "الصور بصيغة SVG غير مسموح بها",
  "File size should not exceed 10 MB": "يجب ألا يتجاوز حجم الملف 10 ميغابايت",
  "File size should not exceed 25 MB": "يجب ألا يتجاوز حجم الملف 25 ميغابايت",
  "Best Price": "أفضل سعر",
  "Last 3 Days!": "آخر 3 أيام!",
  Trend: "رائج",
  "Color !": "اللون!",
  "Best Sell": "الأكثر مبيعاً",
  "Last Week !": "الأسبوع الماضي!",
  "Fast Packing": "تغليف سريع",
  "& Today Shipping If Buy Before 13:00 Today":
    "والشحن اليوم إذا تم الشراء قبل الساعة 13:00 اليوم",
  "Open Try-On": "فتح التجربة الافتراضية",
  "Choose how to add your photo": "اختر كيفية إضافة صورتك",
  "Take a new photo with your camera": "التقط صورة جديدة بكاميرتك",
  "Select from your gallery": "اختر من معرض الصور",
  "Your photo is ready!": "صورتك جاهزة!",
  "Click 'Try On' to see how this product looks on you":
    "انقر على 'جرب' لترى كيف يبدو هذا المنتج عليك",
  "Processing your virtual try-on...": "جاري معالجة تجربتك الافتراضية...",
  "This may take a few moments": "قد يستغرق هذا بضع لحظات",
  "Try again": "حاول مرة أخرى",
  "Capture Photo": "التقط صورة",
  "Processing…": "جارٍ المعالجة…",
  "Try On": "جرّب",
  "Not now": "ليس الآن",
  Allow: "السماح",
  "Back-in-stock and quantity alerts": "تنبيهات عند إعادة التوفر والكميات",
  "New boutiques & categories": "بوتيكات جديدة وفئات",
  "Order updates": "تحديثات الطلب",
  "Chat messages": "رسائل الدردشة",
  "Enable notifications for a more effortless shopping experience:":
    "فعّل الإشعارات لتجربة تسوق أكثر سهولة:",
  "Stay in the loop": "ابقَ على اطلاع",
  "Luck!": "حظ!",
  "Add To Bag Within ": "أضف إلى السلة خلال",
  seconds: "ثواني",
  "stroke of luck": "ضربة حظ",
  "Today Shipping": "شحن اليوم",
  "If Buy Before": "إذا اشتريت قبل",
  Syria: "سوريا",
  Iraq: "العراق",
  Lebanon: "لبنان",
  Turkey: "تركيا",
  "Selected Color": "اللون المختار",
  "Select Color": "اختر اللون",
  "Select Your Required Size": "اختر المقاس المطلوب",
  "Added To Your Bag": "تمت الإضافة إلى سلتك",
  "Removed From Your Bag": "تمت الإزالة من سلتك",
  Added: "تمت الإضافة",
  "To Your Bag": "إلى سلتك",
  "Take A Look At Other Colors": "ألقِ نظرة على الألوان الأخرى",
  "Notify Me When Variant Is Available": "أبلغني عندما يتوفر المنتج",
  "We Will Inform You When Variant Is Available":
    "سوف نعلمك عندما يتوفر المنتج",
  "Rate Your Experience": "قيّم تجربتك",
  "Share your thoughts about this product": "شارك أفكارك حول هذا المنتج",
  "See Our Many Offers And Take Advantage Of Discounts And Special Products":
    "شاهد عروضنا الكثيرة واستفد من الخصومات والمنتجات الخاصة",
  "Expected Delivery": "موعد التسليم المتوقع",
  "Delivery Note": "ملاحظة التسليم",
  "Show Address List": "عرض قائمة العناوين",
  "Notification Is Not Enabled! please Allow Notification Access":
    "الإشعارات غير مفعلة! يرجى السماح بالوصول إلى الإشعارات",
  "New Price": "السعر الجديد",
  "Your Balance Not meet purchase value": "رصيدك لا يغطي قيمة الشراء",
  "Please Login so You Can Add a comment":
    "يرجى تسجيل الدخول حتى تتمكن من إضافة تعليق",
  "Update Rating": "تحديث التقييم",
  "Edit Your Question": "تعديل سؤالك",
  "Update your question about this product": "حدّث سؤالك حول هذا المنتج",
  "Update Question": "تحديث السؤال",
  "Your Comment": "تعليقك",
  Edit: "تعديل",
  "Add your comment": "أضف تعليقك",
  // Wallet Transactions
  "Wallet Transactions": "معاملات المحفظة",
  Wallet: "المحفظة",
  Back: "رجوع",
  Date: "التاريخ",
  Amount: "المبلغ",
  Type: "النوع",
  Description: "الوصف",
  Inflow: "وارد",
  Outflow: "صادر",
  "Order Refund": "استرداد طلب",
  "Purchase Payment": "دفع شراء",
  "Promo Credit": "رصيد ترويجي",
  "Order Payment": "دفع طلب",
  "Submit Rating": "إرسال التقييم",
  "Do you want to delete this message?": "هل تريد حذف هذه الرسالة؟",
  Last: "متبقي",
  "Waiting Seller Reply...": "في انتظار رد البائع...",
  "Add More to Your Bag": "أضف المزيد إلى السلة",
  "Please enable camera permissions to use camera feature":
    "يرجى تفعيل أذونات الكاميرا لاستخدام ميزة الكاميرا",
  Small: "صغير",
  Large: "كبير",
  True: "حقيقي",
  "Max Allowed Quantity Reached":
    "تم الوصول إلى الحد الأقصى للكمية المسموح بها",
  "I Agree & Return": "أوافق وأرجع",
  "Delay Confirmation. I want to Return more product":
    "تأجيل. أرغب في إرجاع المزيد من المنتجات",
  "chosse an image or video from camera or files":
    "اختر صورة أو فيديو من الكاميرا أو الملفات",
  camera: "الكاميرا",
  files: "الملفات",
  Block: "حظر",
  UnBlock: "إلغاء الحظر",
  "Delete Chat": "حذف المحادثة",
  "Are you sure you want to delete this chat?": "هل أنت متأكد أنك تريد حذف هذه المحادثة؟",
  "Save To Gallery": "حفظ في المعرض",
  Never: "أبداً",
  "Media & Files": "الوسائط والملفات",
  Search: "بحث",
  "Video as مكالمة فيديو": "مكالمة فيديو",
  Call: "اتصال",
  "Go to Seller Dashboard": "الذهاب إلى لوحة تحكم المورد",
  "Become A Seller At Trydos": "كن مورداً في تريدوس",
  "Product is Shared Successfully": "تمت مشاركة المنتج بنجاح",
  "Product Share error": "خطأ في مشاركة المنتج",
  now: "الآن",
  image: " صورة",
  video: " فيديو",
  file: " ملف",
  "voice message": " رسالة صوتية",
  message: " رسالة",
  "Please enable  permissions (camera,mic) to use calls features":
    "يرجى تفعيل الأذونات (الكاميرا، الميكروفون) لاستخدام ميزات المكالمات",
  Online: "متصل",
  "last Seen ": "آخر ظهور",
  "Outgoing Voice Call": "مكالمة صوتية صادرة",
  "Outgoing Video Call": "مكالمة فيديو صادرة",
  "Incoming Voice Call": "مكالمة صوتية واردة",
  "Incoming Video Call": "مكالمة فيديو واردة",
  "Missed Voice Call At": "مكالمة صوتية فائتة عند",
  "Missed Video Call At": "مكالمة فيديو فائتة عند",
  "Missed Voice Call": "مكالمة صوتية فائتة",
  "Missed Video Call": "مكالمة فيديو فائتة",
  "Loading products...": "جاري تحميل المنتجات...",
  "Loading boutiques...": "جاري تحميل البوتيكات...",
  "Loading orders...": "جاري تحميل الطلبات...",
  "Loading permissions...": "جاري تحميل الصلاحيات...",
  "Loading users...": "جاري تحميل المستخدمين...",
  "Checking permissions...": "جاري التحقق من الصلاحيات...",
  Retry: "إعادة المحاولة",
  "Access Denied": "تم رفض الوصول",
  "You don't have permission to view products": "لا تملك صلاحية عرض المنتجات",
  "You don't have permission to manage users":
    "لا تملك صلاحية إدارة المستخدمين",
  "You don't have permission to view permissions":
    "لا تملك صلاحية عرض الصلاحيات",
  Users: "المستخدمون",
  "Add User": "إضافة مستخدم",
  "Add User to Shop": "إضافة مستخدم إلى المتجر",
  "User added successfully!": "تمت إضافة المستخدم بنجاح",
  Delete: "حذف",
  "Leave Shop": "مغادرة المتجر",
  "Search roles...": "ابحث عن دور...",
  "No roles found": "لم يتم العثور على أدوار",
  "Load more roles": "تحميل المزيد من الأدوار",
  "No orders found": "لا توجد طلبات",
  Update: "تحديث",
  "Updating...": "جاري التحديث...",
  "Change status:": "تغيير الحالة:",
  Products: "المنتجات",
  Boutiques: "البوتيكات",
  "No products found": "لا توجد منتجات",
  "No boutiques found": "لا توجد بوتيكات",
  "please Select a shop to continue": "الرجاء اختيار متجر",
  "Loading...": "جاري التحميل...",
  "No shops available": "لا توجد متاجر متاحة",
  "Shop Name": "اسم المتجر",
  "Seller ID": "رقم البائع",
  Actions: "الإجراءات",
  Enter: "دخول",
  Leave: "مغادرة",
  "Confirm leave shop": "تأكيد مغادرة المتجر",
  "Are you sure you want to leave this shop? This will remove your access to the shop dashboard.":
    "هل أنت متأكد أنك تريد مغادرة هذا المتجر؟ سيتم إزالة وصولك إلى لوحة تحكم المتجر.",
  Cancel: "إلغاء",
  "Leaving...": "جاري المغادرة...",
  "Failed to leave shop": "فشل في مغادرة المتجر",
  "Access Denied": "تم رفض الوصول",
  "You don't have permission to view products": "لا تملك صلاحية عرض المنتجات",
  Permissions: "الصلاحيات",
  "Name / Phone": "الاسم / رقم الهاتف",
  Role: "الدور",
  Actions: "الإجراءات",
  "Seller ID": "رقم البائع",
  "Searching roles...": "جاري البحث عن الأدوار...",
  "Loading roles...": "جاري تحميل الأدوار...",
  "Phone Number": "رقم الهاتف",
  "Format: +(country_code)XXX (e.g., +9611234567)":
    "الصيغة: +(رمز الدولة)XXX (مثال: +9611234567)",
  "Seller Dashboard": "لوحة تحكم البائع",
  "Seller ID:": "رقم البائع:",
  "please Select a shop to continue": "يرجى اختيار متجر",
  "No shops available": "لا توجد متاجر متاحة",
  "Super Admin": "مشرف عام",
  "You have full access to all features": "لديك حق الوصول الكامل لجميع الميزات",
  "Recommended Products": "المنتجات الموصى بها",
  "Settings - TryDos": "الإعدادات - تريدوس",
  "Manage your account settings and preferences.":
    "قم بإدارة إعدادات حسابك وتفضيلاتك",
  "To New Size?": "الى قياس جديد",
  "Compare products side by side on TryDos - Make informed purchasing decisions.":
    "قارن المنتجات جنبًا إلى جنب على ترايدوس - اتخذ قرارات شراء مدروسة.",
  "Discover premium products on TryDos - Your ultimate shopping destination with featured products, flash deals, and boutique collections.":
    "اكتشف المنتجات المميزة في ترايدوس - وجهتك الأمثل للتسوق مع المنتجات المميزة والعروض السريعة ومجموعات البوتيك.",
  "TryDos - Boutique & Product Listing": "ترايدوس - قائمة البوتيكات والمنتجات",
  "Browse TryDos boutiques and products with powerful filters for brand, category, color, size, and more.":
    "تصفح بوتيكات ومنتجات ترايدوس مع فلاتر قوية حسب الماركة والفئة واللون والقياس والمزيد.",
  "Wallet Deposits": "إيداعات المحفظة",
  "View / Create": "عرض / إنشاء",
  "RDB Wallet": "محفظة RDB",
  "Retry fetching wallet balance": "إعادة المحاولة في تحميل رصيد المحفظة",
  "Submit Deposit": "إرسال الإيداع",
  "Processing...": "جاري المعالجة...",
  "Click to upload": "انقر لتحميل",
  "Proof of Transfer": "اثبات التحويل",
  Tax: "الضريبة",
  "You Receive": "تحصل على",
  Fee: "الرسوم",
  "No banks support this currency for deposit": "",
  "Select a bank": "اختر بنك",
  "No deposits found": "لا توجد إيداعات",
  "Create Deposit": "إنشاء إيداع",
  Deposits: "إيداعات",
  "Related Categories": "الفئات ذات الصلة",
  minute: "دقيقة",
  "Call End in": "انتهاء المكالمة في",

  "Read Products": "عرض المنتجات",
  "Create Product": "إنشاء منتج",
  "Update Product": "تحديث المنتج",
  "Change Product Status": "تغيير حالة المنتج",

  "Read Butiks": "عرض المتاجر",
  "Create Butiks": "إنشاء متجر",
  "Update Butiks": "تحديث المتجر",
  "Delete Butiks": "حذف المتجر",
  "Change Boutique Status": "تغيير حالة المتجر",

  "Read Categories": "عرض الفئات",
  "Create Categories": "إنشاء فئة",
  "Update Categories": "تحديث الفئة",
  "Delete Categories": "حذف الفئة",
  "Change Category Status": "تغيير حالة الفئة",

  "Read Brands": "عرض العلامات التجارية",
  "Create Brands": "إنشاء علامة تجارية",
  "Update Brands": "تحديث العلامة التجارية",
  "Delete Brands": "حذف العلامة التجارية",
  "Change Brand Status": "تغيير حالة العلامة التجارية",

  "Read Orders": "عرض الطلبات",
  "Update Order Info": "تحديث معلومات الطلب",
  "Change Order Status": "تغيير حالة الطلب",
  "Read Order Payments": "عرض مدفوعات الطلب",
  "Confirm Order Payment": "تأكيد دفع الطلب",
  "Refund Order Payment": "استرجاع دفع الطلب",
  "Cancel Order": "إلغاء الطلب",
  "Assign Shipping": "تعيين الشحن",
  "Update Tracking": "تحديث التتبع",
  "Change Order Status Packaged": "تغيير حالة الطلب إلى تم التغليف",
  "Change Order Status Canceled": "تغيير حالة الطلب إلى ملغي",

  "Read Employees": "عرض الموظفين",
  "Create Employees": "إنشاء موظف",
  "Update Employees": "تحديث الموظف",
  "Delete Employees": "حذف الموظف",

  "Read Roles": "عرض الأدوار",
  "Create Roles": "إنشاء دور",
  "Update Roles": "تحديث الدور",
  "Delete Roles": "حذف الدور",

  "Read Jobtitles": "عرض المسميات الوظيفية",
  "Create Jobtitles": "إنشاء مسمى وظيفي",
  "Update Jobtitles": "تحديث المسمى الوظيفي",
  "Delete Jobtitles": "حذف المسمى الوظيفي",

  "Read Offices": "عرض المكاتب",
  "Create Offices": "إنشاء مكتب",
  "Update Offices": "تحديث المكتب",
  "Delete Offices": "حذف المكتب",

  "Read Departments": "عرض الأقسام",
  "Create Departments": "إنشاء قسم",
  "Update Departments": "تحديث القسم",
  "Delete Departments": "حذف القسم",

  "Read Workforms": "عرض أنماط العمل",
  "Create Workforms": "إنشاء نمط عمل",
  "Update Workforms": "تحديث نمط العمل",
  "Delete Workforms": "حذف نمط العمل",

  "Read Languages": "عرض اللغات",
  "Create Languages": "إنشاء لغة",
  "Update Languages": "تحديث اللغة",
  "Delete Languages": "حذف اللغة",

  "Read Currencies": "عرض العملات",
  "Create Currencies": "إنشاء عملة",
  "Update Currencies": "تحديث العملة",
  "Delete Currencies": "حذف العملة",

  "Read Shipping": "عرض الشحن",
  "Create Shipping": "إنشاء شحن",
  "Update Shipping": "تحديث الشحن",
  "Delete Shipping": "حذف الشحن",

  "Read Countries": "عرض الدول",
  "Create Countries": "إنشاء دولة",
  "Update Countries": "تحديث الدولة",
  "Delete Countries": "حذف الدولة",

  "Super Admin": "مدير عام",
  "User Management Access": "صلاحية إدارة المستخدمين",
  "Change Role": "تغيير الدور",
  PRODUCTS: "المنتجات",
  BOUTIQUES: "المتاجر",
  CATEGORIES: "الفئات",
  BRANDS: "العلامات التجارية",
  ORDERS: "الطلبات",
  EMPLOYEES: "الموظفون",
  ROLES: "الأدوار",
  JOBTITLES: "المسميات الوظيفية",
  OFFICES: "المكاتب",
  DEPARTMENTS: "الأقسام",
  WORKFORMS: "أنماط العمل",
  LANGUAGES: "اللغات",
  CURRENCIES: "العملات",
  SHIPPING: "الشحن",
  COUNTRIES: "الدول",
  ADMIN: "الإدارة",
  COMMENTS: "التعليقات",
  Permissions: "الأذونات",
  "Failed to update checklist": "فشل تحديث قائمة الرغبات",
  Stories: "القصص",
  "No stories yet": "لا توجد قصص بعد",
  "Upload your first story to get started": "ارفع قصتك الأولى للبدء",
  "Delete Story": "حذف القصة",
  "Are you sure you want to delete this story? This action cannot be undone.": "هل أنت متأكد أنك تريد حذف هذه القصة؟ لا يمكن التراجع عن هذا الإجراء.",
  "Select Product": "اختر منتجاً",
  "Link to Product": "ربط بمنتج",
  "Story uploaded successfully": "تم رفع القصة بنجاح",
  "Failed to load stories": "فشل تحميل القصص",
  "Failed to delete story": "فشل حذف القصة",
  "Linked to product": "مرتبط بمنتج",
  viewers: "مشاهدة",
  "Edit Shop Info": "تعديل معلومات المتجر",
  "Shop Name *": "* اسم المتجر",
  "Contact": "رقم التواصل",
  "* Country Code Must Like for AE 971": "* يجب أن يتضمن رمز الدولة مثل AE 971",
  "Address *": "* العنوان",
  "Upload Image": "رفع صورة",
  "Choose File": "اختر ملف",
  "Browse": "تصفح",
  "Upload Banner": "رفع غلاف",
  "Ratio .( 6:1 )": "بنسبة .( 6:1 )",
  "Cancel": "إلغاء",
  "Update": "تحديث",
  "Shop Name is required": "اسم المتجر مطلوب",
  "First Name is required": "الاسم الأول مطلوب",
  "Last Name is required": "اسم العائلة مطلوب",
  "Shop Address is required": "عنوان المتجر مطلوب",
  "Location Name is required": "اسم الموقع مطلوب",
  "Location Address is required": "عنوان الموقع مطلوب",
  "Contact is required": "رقم التواصل مطلوب",
  "Contact must contain valid numbers": "يجب أن يحتوي رقم التواصل على أرقام صحيحة",
  "Address is required": "العنوان مطلوب",
  "Shop Info Updated Successfully!": "تم تحديث معلومات المتجر بنجاح!",
  "Failed to update": "فشل التحديث",
  "Preview Photo": "معاينة الصورة",
  "Preview Video": "معاينة الفيديو",
  "Create Story": "إنشاء قصة",
  "Microphone not detected or permission denied": "لم يتم اكتشاف الميكروفون أو تم رفض الإذن",
  "Photo": "صورة",
  "Video": "فيديو",
  "Privacy Policy": "سياسة الخصوصية",
  "Contact Us": "اتصل بنا",
  "Compare": "المقارنة",
  "About TryDos": "عن ترايدوس",
  "TryDos is a multilingual live-shopping marketplace connecting boutiques and shoppers across the region with curated fashion, beauty and lifestyle products.":
    "ترايدوس سوق تسوّق مباشر متعدد اللغات يربط المتاجر والمتسوّقين في المنطقة بمنتجات منتقاة من الأزياء والجمال ونمط الحياة.",
  "TryDos is a multilingual live-shopping marketplace connecting boutiques and shoppers across the region. We bring curated fashion, beauty and lifestyle products from trusted sellers straight to your screen.":
    "ترايدوس سوق تسوّق مباشر متعدد اللغات يربط المتاجر والمتسوّقين في أنحاء المنطقة. نقدّم لك منتجات منتقاة من الأزياء والجمال ونمط الحياة من بائعين موثوقين مباشرةً إلى شاشتك.",
  "Our Mission": "مهمتنا",
  "We make discovering and buying from independent boutiques effortless — with live video shopping, secure checkout and fast delivery, all in your local language and currency.":
    "نجعل اكتشاف المتاجر المستقلة والشراء منها أمراً سهلاً — مع التسوّق المباشر بالفيديو والدفع الآمن والتوصيل السريع، وكل ذلك بلغتك وعملتك المحلية.",
  "Why Shop With TryDos": "لماذا تتسوّق مع ترايدوس",
  "Thousands of products from verified boutiques, real-time live sales, easy returns and dedicated support in Arabic, English, Turkish and Kurdish.":
    "آلاف المنتجات من متاجر موثوقة، وعروض بيع مباشرة، وإرجاع سهل، ودعم مخصّص بالعربية والإنجليزية والتركية والكردية.",
  "Read the TryDos Privacy Policy: what information we collect, how we use it, and the choices you have over your data.":
    "اقرأ سياسة الخصوصية في ترايدوس: ما المعلومات التي نجمعها، وكيف نستخدمها، والخيارات المتاحة لك بشأن بياناتك.",
  "Your privacy matters to us. This policy explains what information TryDos collects, how we use it, and the choices you have.":
    "خصوصيتك تهمّنا. توضّح هذه السياسة المعلومات التي يجمعها ترايدوس وكيفية استخدامها والخيارات المتاحة لك.",
  "Information We Collect": "المعلومات التي نجمعها",
  "We collect the details you provide when you create an account or place an order, along with basic device and usage data needed to operate and secure the service.":
    "نجمع التفاصيل التي تقدّمها عند إنشاء حساب أو تقديم طلب، إضافةً إلى بيانات الجهاز والاستخدام الأساسية اللازمة لتشغيل الخدمة وتأمينها.",
  "How We Use Your Data": "كيف نستخدم بياناتك",
  "Your data is used to process orders, personalise your experience, prevent fraud and improve TryDos. We never sell your personal information.":
    "تُستخدم بياناتك لمعالجة الطلبات وتخصيص تجربتك ومنع الاحتيال وتحسين ترايدوس. نحن لا نبيع معلوماتك الشخصية أبداً.",
  "The TryDos Terms & Conditions covering how to use the platform, orders, payments and your responsibilities as a shopper or seller.":
    "شروط وأحكام ترايدوس التي تغطي كيفية استخدام المنصّة والطلبات والمدفوعات ومسؤولياتك كمتسوّق أو بائع.",
  "By using TryDos you agree to these terms. Please read them carefully before shopping or selling on our platform.":
    "باستخدامك ترايدوس فإنك توافق على هذه الشروط. يُرجى قراءتها بعناية قبل التسوّق أو البيع على منصّتنا.",
  "Using TryDos": "استخدام ترايدوس",
  "You agree to use TryDos lawfully, to provide accurate account information, and to respect the rights of boutiques, sellers and other shoppers.":
    "أنت توافق على استخدام ترايدوس بشكل قانوني، وتقديم معلومات حساب دقيقة، واحترام حقوق المتاجر والبائعين وبقية المتسوّقين.",
  "Orders & Payments": "الطلبات والمدفوعات",
  "All orders are subject to availability and confirmation. Prices, fees and delivery times are shown at checkout and may vary by country.":
    "جميع الطلبات خاضعة للتوفّر والتأكيد. تظهر الأسعار والرسوم وأوقات التوصيل عند الدفع وقد تختلف حسب الدولة.",
  "Contact the TryDos team for help with your order, account, or to learn about selling on the platform as a boutique.":
    "تواصل مع فريق ترايدوس للحصول على مساعدة بشأن طلبك أو حسابك، أو لمعرفة المزيد عن البيع على المنصّة كمتجر.",
  "We're here to help. Reach the TryDos team with any question about your order, account or selling on the platform.":
    "نحن هنا لمساعدتك. تواصل مع فريق ترايدوس بأي سؤال عن طلبك أو حسابك أو البيع على المنصّة.",
  "Customer Support": "دعم العملاء",
  "For help with orders, returns or your account, contact our support team through the app or by email at support@trydos.com.":
    "للمساعدة بشأن الطلبات أو الإرجاع أو حسابك، تواصل مع فريق الدعم عبر التطبيق أو بالبريد الإلكتروني support@trydos.com.",
  "Business & Boutiques": "الأعمال والمتاجر",
  "Interested in selling on TryDos? Get in touch and our team will guide you through joining as a boutique.":
    "هل ترغب في البيع على ترايدوس؟ تواصل معنا وسيرشدك فريقنا خلال الانضمام كمتجر.",
  "Checkout": "الدفع",
  "Wallet Checkout": "الدفع بالمحفظة",
  "Select Payment Currency": "اختر عملة الدفع",
  "Available": "متاح",
  "Payment Summary": "ملخص الدفع",
  "Cart Total:": "إجمالي السلة:",
  "Available Balance:": "الرصيد المتاح:",
  "Amount to Pay:": "المبلغ المطلوب:",
  "Insufficient balance. Please add funds or select a different currency.": "الرصيد غير كافٍ. الرجاء إضافة رصيد أو اختيار عملة مختلفة.",
  "Confirm Payment": "تأكيد الدفع",
  "Microphone access denied": "تم رفض الوصول إلى الميكروفون",
  "Try again with clear voice": "حاول مرة أخرى بصوت واضح",
  "Failed to process audio": "فشلت معالجة الصوت",
  "Browser does not support this feature": "المتصفح لا يدعم هذه الميزة",
  "Story Uploaded": "تم رفع القصة",
  "Error Uploading Story": "حدث خطأ أثناء رفع القصة",
  "only 3 pinned chats allowed": "يُسمح بتثبيت 3 محادثات فقط",
  "Connection Lost": "انقطع الاتصال",
  "We are having trouble connecting to our servers. This might be due to:": "نواجه مشكلة في الاتصال بخوادمنا. قد يكون ذلك بسبب:",
  "Network connectivity issues": "مشاكل في اتصال الشبكة",
  "Temporary server maintenance": "صيانة مؤقتة للخادم",
  "Slow internet connection": "اتصال إنترنت بطيء",
  "Reload Page": "إعادة تحميل الصفحة",
  "Go Home": "الذهاب إلى الرئيسية",
  "Oops! Something went wrong": "عذراً! حدث خطأ ما",
  "We encountered an unexpected error. Dont worry, our team has been notified and is working on it.": "واجهنا خطأ غير متوقع. لا تقلق، تم إخطار فريقنا ويعمل على حله.",
  "Error ID:": "معرّف الخطأ:",
  "Item": "العنصر",
  "Collect Type": "نوع التحصيل",
  "Collect": "تحصيل",
  "Duration To Do Action": "المدة المتاحة لاتخاذ الإجراء",
  "Color:": "اللون:",
  "Size:": "المقاس:",
  "ID:": "المعرّف:",
  "Quantity:": "الكمية:",
  "Status:": "الحالة:",
  "Loading Call Information...": "جارٍ تحميل معلومات المكالمة...",
  "Copy Link": "نسخ الرابط",
  "Add a new contact": "إضافة جهة اتصال جديدة",
  "Contact Name": "اسم جهة الاتصال",
  "Already saved as": "محفوظ بالفعل باسم",
  "File": "ملف",
  "Image": "صورة",
  "Product": "منتج",
  "Audio": "صوت",
  "Incoming": "وارد",
  "Call": "مكالمة",
  "Crop Image": "قص الصورة",
  "Save": "حفظ",
  "Min": "الأدنى",
  "Max": "الأقصى",
  "Qty:": "الكمية:",
  "Price": "السعر",
  "Subtotal": "المجموع الفرعي",
  "Error:": "خطأ:",
  "Go Back": "العودة",
  "Search": "بحث",
  "Search...": "بحث...",
  "No matching options.": "لا توجد خيارات مطابقة.",
  "Forward Message": "إعادة توجيه الرسالة",
  "No more stories": "لا مزيد من القصص",
  "Name": "الاسم",
  "OR": "أو",
  "Session:": "الجلسة:",
  "Under Development": "قيد التطوير",
  "Browse Categories": "تصفح الفئات",
  "Popular Links:": "روابط شائعة:",
  "Boutiques": "المتاجر",
  "Load more": "تحميل المزيد",
  "Invalid cart data": "بيانات السلة غير صالحة",
  "Insufficient balance in your wallet.": "رصيد غير كافٍ في محفظتك.",
  "Payment successful!": "تمت عملية الدفع بنجاح!",
  "Payment failed. Please try again.": "فشلت عملية الدفع. الرجاء المحاولة مرة أخرى.",
  "Province": "المحافظة",
  "Town": "البلدة",
  "Suburb": "الحي",
  "Syncing...": "جارٍ المزامنة...",
  "Adding...": "جارٍ الإضافة...",
  "Confirm Add": "تأكيد الإضافة",
  "Send": "إرسال",
  "None": "لا شيء",
  "FILE": "ملف",
  "please select supported image format (jpeg, png, jpg, webp, svg, avif)": "الرجاء اختيار صيغة صورة مدعومة (jpeg, png, jpg, webp, svg, avif)",
  "No options found": "لا توجد خيارات",
  "Delivery Worker": "عامل التوصيل",
  "Unknown": "غير معروف",
  "Wrong Code": "رمز خاطئ",
  "Please Review Your Cart Info": "يرجى مراجعة معلومات سلة التسوق",
  "Product not found": "المنتج غير موجود",
  "Search request failed": "فشل طلب البحث",
  "Authentication required": "المصادقة مطلوبة",
  "Customer Info Error": "خطأ في معلومات العميل",
  "Failed": "فشل",
  "Permissions not granted for calls": "لم يتم منح الأذونات للمكالمات",
  "Errors:": "الأخطاء:",
  "Session for simulate user has ended": "انتهت جلسة محاكاة المستخدم",
  "Media server upload is not configured": "رفع خادم الوسائط غير مهيأ",
  "Media server upload failed": "فشل رفع الملف إلى خادم الوسائط",
  "Media server is not configured": "خادم الوسائط غير مهيأ",
  "No images were uploaded to the media server": "لم يتم رفع أي صور إلى خادم الوسائط",
  "Image upload failed": "فشل رفع الصورة",
  "Invalid upload response": "استجابة رفع غير صالحة",
  "Upload returned no file": "لم يُرجع الرفع أي ملف",
  "Upload returned no files": "لم يُرجع الرفع أي ملفات",
  "Buyer": "المشتري",
  "+(country_code)XXX": "+(country_code)XXX",
  "1 minutes video only": "فيديو بمدة دقيقة واحدة فقط",
  "Active": "نشط",
  "Add": "إضافة",
  "Adding User...": "جاري إضافة المستخدم...",
  "Allow Purchase": "السماح بالشراء",
  "Allow this product to be purchased?": "السماح بشراء هذا المنتج؟",
  "An English (en) name is required to enable the product.": "يلزم اسم بالإنجليزية (en) لتفعيل المنتج.",
  "Are you sure you want to delete the selected images? This action cannot be undone.": "هل أنت متأكد من حذف الصور المحددة؟ لا يمكن التراجع عن هذا الإجراء.",
  "Are you sure you want to delete this image? This action cannot be undone.": "هل أنت متأكد من حذف هذه الصورة؟ لا يمكن التراجع عن هذا الإجراء.",
  "Assign each uploaded image to a color. Every color needs at least one image, and every image must be assigned.": "عيّن كل صورة مرفوعة للون. يحتاج كل لون إلى صورة واحدة على الأقل، ويجب تعيين كل صورة.",
  "Barcode": "باركود",
  "Boutique": "متجر",
  "Boutiques created for this shop will appear here.": "ستظهر هنا المتاجر المصغّرة المنشأة لهذا المتجر.",
  "Brand": "العلامة التجارية",
  "Browse and manage your catalogue": "تصفّح وأدِر كتالوجك",
  "Bulk-import products by template": "استيراد المنتجات بالجملة عبر قالب",
  "Cannot enable yet — resolve these first:": "لا يمكن التفعيل بعد — عالِج ما يلي أولاً:",
  "Categories": "الفئات",
  "Change Banner": "تغيير اللافتة",
  "Change Image": "تغيير الصورة",
  "Change media": "تغيير الوسائط",
  "Change status": "تغيير الحالة",
  "Changes were submitted": "تم إرسال التغييرات",
  "Your Trusted Commerce Partner": "شريكك الموثوق في التجارة",
  "Changes were submitted and are pending admin approval — they go live once approved.": "تم إرسال التغييرات وهي قيد موافقة المشرف — ستُنشر بمجرد الموافقة عليها.",
  "Charge shipping per item ordered.": "احتساب الشحن لكل عنصر مطلوب.",
  "Check back later for comments from customers.": "عُد لاحقاً للاطلاع على تعليقات العملاء.",
  "Choose a section to get started.": "اختر قسماً للبدء.",
  "Collected": "تم الاستلام",
  "Color Images": "صور الألوان",
  "Confirm & Save": "تأكيد وحفظ",
  "Confirm & Start Packing": "تأكيد وبدء التغليف",
  "Confirm Upload": "تأكيد الرفع",
  "Confirm changes": "تأكيد التغييرات",
  "Confirmed": "مؤكَّد",
  "Copied!": "تم النسخ!",
  "Copy URL": "نسخ الرابط",
  "Could not change status": "تعذّر تغيير الحالة",
  "Country code must lead, e.g. AE = 971": "يجب أن يبدأ برمز الدولة، مثال: AE = 971",
  "Country of Origin": "بلد المنشأ",
  "Cover": "الغلاف",
  "Created At": "تاريخ الإنشاء",
  "Current Stock": "المخزون الحالي",
  "Customers will be able to buy this product once it passes activation checks.": "سيتمكن العملاء من شراء هذا المنتج بمجرد اجتيازه فحوصات التفعيل.",
  "Dashboard Menu": "قائمة لوحة التحكم",
  "Delete Image": "حذف الصورة",
  "Delete Reply": "حذف الرد",
  "Delete this reply?": "حذف هذا الرد؟",
  "Deleting...": "جاري الحذف...",
  "Deselect": "إلغاء التحديد",
  "Deselect all": "إلغاء تحديد الكل",
  "Disable": "تعطيل",
  "Disable purchasing?": "تعطيل الشراء؟",
  "Disabled": "معطَّل",
  "Discount": "خصم",
  "Discount Price": "سعر الخصم",
  "Download": "تنزيل",
  "Play": "تشغيل",
  "This video format is not supported. Send an MP4 video instead.": "صيغة الفيديو هذه غير مدعومة. أرسل فيديو بصيغة MP4 بدلاً من ذلك.",
  "This video cannot play here. Tap to download it.": "لا يمكن تشغيل هذا الفيديو هنا. اضغط لتنزيله.",
  "Drop images here": "أفلت الصور هنا",
  "Edit shop name, contact & media": "تعديل اسم المتجر وبيانات التواصل والوسائط",
  "Enter Dashboard": "الدخول إلى لوحة التحكم",
  "Extra": "إضافي",
  "Extra price": "سعر إضافي",
  "Failed to add user": "فشل إضافة المستخدم",
  "Failed to add user to shop": "فشل إضافة المستخدم إلى المتجر",
  "Failed to cancel order item": "فشل إلغاء عنصر الطلب",
  "Failed to confirm order item": "فشل تأكيد عنصر الطلب",
  "Failed to delete image": "فشل حذف الصورة",
  "Failed to delete images": "فشل حذف الصور",
  "Failed to delete user": "فشل حذف المستخدم",
  "Failed to load boutiques": "فشل تحميل المتاجر المصغّرة",
  "Failed to load images": "فشل تحميل الصور",
  "Failed to load orders": "فشل تحميل الطلبات",
  "Failed to load permissions": "فشل تحميل الأذونات",
  "Failed to load product": "فشل تحميل المنتج",
  "Failed to load products": "فشل تحميل المنتجات",
  "Failed to load roles": "فشل تحميل الأدوار",
  "Failed to load uploaded files": "فشل تحميل الملفات المرفوعة",
  "Failed to load users": "فشل تحميل المستخدمين",
  "Failed to pack order item": "فشل تغليف عنصر الطلب",
  "Failed to update order status": "فشل تحديث حالة الطلب",
  "Failed to update product": "فشل تحديث المنتج",
  "Failed to update user role": "فشل تحديث دور المستخدم",
  "Flat": "ثابت",
  "Gallery": "المعرض",
  "General": "عام",
  "ID": "المعرّف",
  "Identity & classification of the product.": "هوية المنتج وتصنيفه.",
  "Images": "الصور",
  "Inactive": "غير نشط",
  "Item is packed once an order is placed.": "يتم تغليف العنصر بمجرد تقديم الطلب.",
  "KB": "KB",
  "Keep": "إبقاء",
  "Labels": "التسميات",
  "Labels & Tags": "التسميات والوسوم",
  "Leave shop": "مغادرة المتجر",
  "Link": "رابط",
  "Linked product": "منتج مرتبط",
  "Loading files...": "جاري تحميل الملفات...",
  "Loading images...": "جاري تحميل الصور...",
  "Loading product…": "جاري تحميل المنتج…",
  "Loading stories...": "جاري تحميل القصص...",
  "Luck": "الحظ",
  "Luck Price": "سعر الحظ",
  "MB": "MB",
  "Manage team members and roles": "إدارة أعضاء الفريق والأدوار",
  "Max Allowed Qty": "الحد الأقصى المسموح",
  "Member": "عضو",
  "Meta Description": "وصف Meta",
  "Meta Image": "صورة Meta",
  "Meta Title": "عنوان Meta",
  "Model Number": "رقم الطراز",
  "Multiply Shipping × Quantity": "ضرب الشحن × الكمية",
  "Must be ≤ unit price": "يجب أن يكون ≤ سعر الوحدة",
  "Must stay unique across the marketplace": "يجب أن يبقى فريداً عبر السوق",
  "N/A": "N/A",
  "New video ready": "الفيديو الجديد جاهز",
  "Next": "التالي",
  "No Image": "لا توجد صورة",
  "No banner yet": "لا توجد لافتة بعد",
  "No changes to save.": "لا توجد تغييرات للحفظ.",
  "No comments found.": "لا توجد تعليقات.",
  "No description": "لا يوجد وصف",
  "No files uploaded yet.": "لم يتم رفع أي ملفات بعد.",
  "No image": "لا توجد صورة",
  "No images found": "لم يتم العثور على صور",
  "No items": "لا توجد عناصر",
  "No notes available.": "لا توجد ملاحظات.",
  "No options available for the current selection.": "لا توجد خيارات متاحة للتحديد الحالي.",
  "No order selected": "لم يتم اختيار طلب",
  "No per-country surcharges.": "لا توجد رسوم إضافية حسب الدولة.",
  "No permissions assigned": "لم يتم تعيين أي أذونات",
  "No remaining time": "لا يوجد وقت متبقٍ",
  "No roles available": "لا توجد أدوار متاحة",
  "No users found": "لم يتم العثور على مستخدمين",
  "No video attached.": "لا يوجد فيديو مرفق.",
  "Notes": "الملاحظات",
  "Optional. Upload a new product video or remove existing ones.": "اختياري. ارفع فيديو منتج جديداً أو احذف الفيديوهات الحالية.",
  "Origin & Countries": "المنشأ والدول",
  "Out of stock": "نفد المخزون",
  "Packed": "تم التغليف",
  "Packed After Ordering": "يُغلَّف بعد الطلب",
  "Page": "الصفحة",
  "Pending Approval": "قيد الموافقة",
  "Per-country Extra Price": "سعر إضافي حسب الدولة",
  "Percent": "نسبة مئوية",
  "Photo or video, up to": "صورة أو فيديو، حتى",
  "Pick a shop to open its dashboard.": "اختر متجراً لفتح لوحة تحكمه.",
  "Pick colors & sizes, then set price and stock for each combination.": "اختر الألوان والمقاسات، ثم حدّد السعر والمخزون لكل تركيبة.",
  "Pieces / Unit": "قطع / وحدة",
  "More problems were reported": "تم الإبلاغ عن مشاكل أخرى",
  "Please fix the highlighted fields before saving.": "يرجى تصحيح الحقول المظللة قبل الحفظ.",
  "Preparing your dashboard...": "جاري تجهيز لوحة تحكمك...",
  "Previous": "السابق",
  "Prices are in your display currency; converted server-side.": "الأسعار بعملة العرض لديك؛ ويتم تحويلها من جهة الخادم.",
  "Pricing & Stock": "التسعير والمخزون",
  "Product Name": "اسم المنتج",
  "Product updated successfully.": "تم تحديث المنتج بنجاح.",
  "Products added to this shop will appear here.": "ستظهر هنا المنتجات المضافة إلى هذا المتجر.",
  "Purchasable": "قابل للشراء",
  "Purchase Price": "سعر الشراء",
  "Questions": "الأسئلة",
  "Ratio 6:1": "نسبة 6:1",
  "Reactions": "التفاعلات",
  "Read only": "للقراءة فقط",
  "Ready To Collect": "جاهز للاستلام",
  "Refresh": "تحديث",
  "Reply to FQA Comment": "الرد على تعليق الأسئلة الشائعة",
  "Reply to reviews and FAQ": "الرد على التقييمات والأسئلة الشائعة",
  "Report Ref. Number": "الرقم المرجعي للبلاغ",
  "Required for pc / liter": "مطلوب للقطعة / اللتر",
  "Temporarily read-only": "للقراءة فقط مؤقتًا",
  "Must be a whole number between 1 and 100": "يجب أن يكون رقمًا صحيحًا بين 1 و 100",
  "Restricted Countries": "الدول المحظورة",
  "Review your access in this shop": "راجع صلاحيتك في هذا المتجر",
  "Review your changes before saving.": "راجع تغييراتك قبل الحفظ.",
  "Reviews": "التقييمات",
  "SEO / Meta": "SEO / Meta",
  "SKU": "SKU",
  "Save Changes": "حفظ التغييرات",
  "Saving...": "جاري الحفظ...",
  "Saving…": "جاري الحفظ…",
  "Select Files": "اختيار الملفات",
  "Select Folder": "اختيار المجلد",
  "Select a role": "اختر دوراً",
  "Select all": "تحديد الكل",
  "Select images to delete": "اختر الصور المراد حذفها",
  "Seller Product ID": "معرّف منتج البائع",
  "Share photo & video stories": "شارك قصص الصور والفيديو",
  "Shares": "المشاركات",
  "Shipping Days": "أيام الشحن",
  "Shop Banner": "لافتة المتجر",
  "Shop Logo": "شعار المتجر",
  "Shops you can manage will appear here.": "ستظهر هنا المتاجر التي يمكنك إدارتها.",
  "Show notes": "عرض الملاحظات",
  "Status": "الحالة",
  "Status updated.": "تم تحديث الحالة.",
  "Storefronts under this shop": "الواجهات ضمن هذا المتجر",
  "Sub-categories shown match the current parents (re-open the page after changing parents to load others).": "الفئات الفرعية المعروضة تطابق الفئات الرئيسية الحالية (أعد فتح الصفحة بعد تغيير الفئات الرئيسية لتحميل غيرها).",
  "Tags": "الوسوم",
  "Tax Type": "نوع الضريبة",
  "The first image is the cover. Drag order with the arrows.": "الصورة الأولى هي الغلاف. رتّب الترتيب بالأسهم.",
  "The product is restricted to the selected countries.": "المنتج محظور في الدول المحددة.",
  "The product will be hidden from purchase until you re-enable it.": "سيُخفى المنتج من الشراء حتى تعيد تفعيله.",
  "These fields will be updated": "سيتم تحديث هذه الحقول",
  "Seller Product ID is required": "معرّف منتج البائع مطلوب",
  "This Seller Product ID is already used": "معرّف منتج البائع هذا مستخدم بالفعل",
  "This will remove your access to the shop dashboard. You can be re-invited later.": "سيؤدي هذا إلى إزالة صلاحيتك للوصول إلى لوحة تحكم المتجر. يمكن إعادة دعوتك لاحقاً.",
  "Title": "العنوان",
  "Track and fulfil customer orders": "تتبّع طلبات العملاء وأنجزها",
  "Translations": "الترجمات",
  "USD": "USD",
  "Unit": "الوحدة",
  "Unit Price": "سعر الوحدة",
  "Unnamed Boutique": "متجر مصغّر بلا اسم",
  "Unnamed Product": "منتج بلا اسم",
  "Untitled Product": "منتج بلا عنوان",
  "Upload Failed Try Again": "فشل الرفع حاول مرة أخرى",
  "Upload Video": "رفع فيديو",
  "Upload and reuse product images": "ارفع صور المنتجات وأعد استخدامها",
  "Upload failed": "فشل الرفع",
  "Upload images first.": "ارفع الصور أولاً.",
  "Uploaded Excel Files": "ملفات إكسل المرفوعة",
  "Uploaded product images will appear here.": "ستظهر هنا صور المنتجات المرفوعة.",
  "Uploading…": "جاري الرفع…",
  "Variants": "الأنواع",
  "Video upload failed": "فشل رفع الفيديو",
  "View": "عرض",
  "View only": "للعرض فقط",
  "Height": "الطول",
  "Weight": "الوزن",
  "Welcome back": "مرحباً بعودتك",
  "You don't have permission to view boutiques": "ليس لديك إذن لعرض المتاجر المصغّرة",
  "You don't have permission to view or edit this product.": "ليس لديك إذن لعرض هذا المنتج أو تعديله.",
  "You need order viewing permissions to see this section": "تحتاج إلى أذونات عرض الطلبات لرؤية هذا القسم",
  "You're viewing this product. Tap Edit to make changes.": "أنت تعرض هذا المنتج. اضغط تعديل لإجراء تغييرات.",
  "Your Shops": "متاجرك",
  "Your role in this shop": "دورك في هذا المتجر",
  "Your shop": "متجرك",
  "files selected": "ملفات محددة",
  "images": "صور",
  "in stock": "متوفر",
  "max 3": "الحد الأقصى 3",
  "of": "من",
  "or choose files / folder": "أو اختر الملفات / المجلد",
  "permissions": "أذونات",
  "selected": "محدد",
  "this shop": "هذا المتجر",
  "Product Quality": "جودة المنتج",
  "Damaged": "تالف",
  "Not as described": "غير مطابق للوصف",
  "Poor material / quality": "جودة / مواد رديئة",
  "Wrong item received": "تم استلام عنصر خاطئ",
  "Expired / spoiled": "منتهي الصلاحية / تالف",
  "Delivery Time": "وقت التوصيل",
  "Arrived too late": "وصل متأخراً جداً",
  "Missed the delivery window": "تجاوز نافذة التوصيل",
  "No clear ETA": "لا يوجد وقت وصول متوقع واضح",
  "Faster than expected": "أسرع من المتوقع",
  "Delivery Worker": "مندوب التوصيل",
  "Rude behavior": "سلوك غير مهذب",
  "Unprofessional": "غير احترافي",
  "Did not show up": "لم يحضر",
  "Asked for an extra fee": "طلب رسوماً إضافية",
  "Polite & helpful": "مهذب ومتعاون",
  "Delivery Car": "سيارة التوصيل",
  "Dirty vehicle": "مركبة غير نظيفة",
  "No cooling / improper temperature": "لا يوجد تبريد / درجة حرارة غير مناسبة",
  "Unsafe handling": "تعامل غير آمن",
  "No proper vehicle": "لا توجد مركبة مناسبة",
  // Hidden Orders widget
  "Action About Your Orders": "إجراءات على طلباتك",
  "Hidden Orders": "الطلبات المخفية",
  "See orders and products you hid": "اعرض الطلبات والمنتجات التي أخفيتها",
  "These are orders and products you hid from your history. Tap the eye to restore any of them back to your list.": "هذه هي الطلبات والمنتجات التي أخفيتها من سجلك. اضغط على العين لاستعادة أي منها إلى قائمتك.",
  "You have no hidden orders.": "لا توجد لديك طلبات مخفية.",
  "Restore This Order": "استعادة هذا الطلب",
  "Restore This Product": "استعادة هذا المنتج",
  "Are you sure you want to restore this order to your list?": "هل أنت متأكد أنك تريد استعادة هذا الطلب إلى قائمتك؟",
  "Are you sure you want to restore this product to your list?": "هل أنت متأكد أنك تريد استعادة هذا المنتج إلى قائمتك؟",
  "Boutique details": "تفاصيل المتجر",
  "Shown across the storefront.": "تظهر في جميع أنحاء المتجر.",
  "Boutique name": "اسم المتجر",
  Availability: "الإتاحة",
  Web: "الويب",
  Mobile: "الجوال",
  "Web + Mobile": "الويب والجوال",
  "Choose where this boutique is available.": "اختر أين يكون هذا المتجر متاحًا.",
  "Boutique icon": "أيقونة المتجر",
  "Upload icon": "رفع أيقونة",
  "Available in countries": "متاح في الدول",
  "Leave empty to make the boutique available everywhere.": "اتركه فارغًا لجعل المتجر متاحًا في كل مكان.",
  "Name, description and bio per language. English is required.": "الاسم والوصف والنبذة لكل لغة. الإنجليزية مطلوبة.",
  Bio: "نبذة",
  Banner: "لافتة",
  Banners: "اللافتات",
  "Add banner": "إضافة لافتة",
  "Move left": "تحريك لليسار",
  "Move right": "تحريك لليمين",
  "Loading boutique…": "جارٍ تحميل المتجر…",
  "Boutique not found.": "المتجر غير موجود.",
  "Failed to load boutique.": "فشل تحميل المتجر.",
  "You don't have permission to view or edit this boutique.": "ليس لديك صلاحية لعرض أو تعديل هذا المتجر.",
  "Set active": "تفعيل",
  "Set inactive": "إلغاء التفعيل",
  "Boutique updated successfully.": "تم تحديث المتجر بنجاح.",
  "Failed to update boutique.": "فشل تحديث المتجر.",
  "Could not change status.": "تعذر تغيير الحالة.",
  "Your changes were saved, but the status could not be updated.": "تم حفظ تغييراتك، لكن تعذّر تحديث الحالة.",
  "Status could not be changed:": "تعذر تغيير الحالة:",
  "Image upload failed.": "فشل رفع الصورة.",
  "Upload returned no file.": "لم يُرجع الرفع أي ملف.",
  "Name is required.": "الاسم مطلوب.",
  "Icon is required.": "الأيقونة مطلوبة.",
  "English name is required.": "الاسم بالإنجليزية مطلوب.",
  "Choose an availability.": "اختر الإتاحة.",
  "Please choose an image file.": "يرجى اختيار ملف صورة.",
  "Banner image must be 10 MB or smaller.": "يجب ألا يتجاوز حجم اللافتة 10 ميجابايت.",
  "Icon image must be 10 MB or smaller.": "يجب ألا يتجاوز حجم الأيقونة 10 ميجابايت.",
  "This banner may not display well": "قد لا تظهر هذه اللافتة بشكل جيد",
  "For best results use a banner of": "للحصول على أفضل نتيجة استخدم لافتة بمقاس",
  "Keep important content centered — the storefront may crop the top and bottom.": "أبقِ المحتوى المهم في الوسط — قد يقتطع المتجر الأعلى والأسفل.",
  "Your image is": "صورتك بمقاس",
  "Ignore & upload": "تجاهل وارفع",
  "Recommended size": "الحجم المُوصى به",
  "Used across all languages.": "يُستخدم في جميع اللغات.",
  "New Boutique": "متجر جديد",
  "Add Boutique": "إضافة متجر",
  "Add your first boutique": "أضف متجرك الأول",
  "Fill in the details and create your boutique.": "املأ التفاصيل وأنشئ متجرك.",
  "Create Boutique": "إنشاء المتجر",
  "Boutique created successfully.": "تم إنشاء المتجر بنجاح.",
  "Failed to create boutique.": "فشل إنشاء المتجر.",
  "Delete boutique": "حذف المتجر",
  "Delete this boutique?": "حذف هذا المتجر؟",
  "This boutique will be permanently removed from your shop and can't be undone.":
    "سيُحذف هذا المتجر نهائيًا من متجرك ولا يمكن التراجع عن ذلك.",
  "Boutique deleted successfully.": "تم حذف المتجر بنجاح.",
  "Failed to delete boutique.": "فشل حذف المتجر.",
  "Failed to load boutique form.": "فشل تحميل نموذج المتجر.",
  Translations: "الترجمات",
  "Name, description, bio, icon and banners per language. English is required.":
    "الاسم والوصف والنبذة والأيقونة واللافتات لكل لغة. الإنجليزية مطلوبة.",
  "Copy from": "نسخ من",
  "Copy from another language": "نسخ من لغة أخرى",
  "No countries available.": "لا توجد دول متاحة.",
  "Description is required.": "الوصف مطلوب.",
  "Bio is required.": "النبذة مطلوبة.",
  "At least one banner is required.": "يلزم لافتة واحدة على الأقل.",
  "Restricted countries": "الدول المقيّدة",
  "The boutique is restricted to the selected countries. Leave empty to make it available everywhere.":
    "المتجر مقيّد بالدول المحددة. اتركه فارغًا لجعله متاحًا في كل مكان.",
  "Name, description, bio, icon and at least one banner are required for every language.":
    "الاسم والوصف والنبذة والأيقونة ولافتة واحدة على الأقل مطلوبة لكل لغة.",
  // Seller product editor
  "Add Image": "إضافة صورة",
  "Add selected": "إضافة المحدد",
  "An English (en) name is required": "مطلوب اسم بالإنجليزية (en)",
  "At least one product image is required": "مطلوب صورة واحدة للمنتج على الأقل",
  "At most 3 labels allowed": "يُسمح بثلاث علامات كحد أقصى",
  "Attributes": "الخصائص",
  "Choose from gallery": "اختيار من المعرض",
  "Color → Image Assignment": "تخصيص اللون ← الصورة",
  "Colors": "الألوان",
  "Confirm new product": "تأكيد المنتج الجديد",
  "Country": "الدولة",
  "Description": "الوصف",
  "Descriptors": "الواصفات",
  "Discount must be ≤ unit price": "يجب أن يكون الخصم ≤ سعر الوحدة",
  "Edit": "تعديل",
  "edited": "مُعدّل",
  "Enter a valid discount price": "أدخل سعر خصم صالح",
  "Enter a valid purchase price": "أدخل سعر شراء صالح",
  "Enter a valid stock": "أدخل مخزونًا صالحًا",
  "Enter a valid weight": "أدخل وزنًا صالحًا",
  "Enter a valid unit price": "أدخل سعر وحدة صالح",
  "Every color needs at least one image": "يحتاج كل لون إلى صورة واحدة على الأقل",
  "Every image must be assigned to a color": "يجب تخصيص كل صورة للون",
  "Every variant needs a location": "كل خيار يحتاج إلى موقع",
  "Every variant needs a quantity": "يحتاج كل خيار إلى كمية",
  "Every variant needs an SKU": "يحتاج كل خيار إلى رمز SKU",
  "Failed to create product": "فشل إنشاء المنتج",
  "Failed to load gallery": "فشل تحميل المعرض",
  "Fill in the details and create your product.": "املأ التفاصيل وأنشئ منتجك.",
  "image(s)": "صورة/صور",
  "item(s)": "عنصر/عناصر",
  "Label": "علامة",
  "language(s)": "لغة/لغات",
  "Loading…": "جارٍ التحميل…",
  "Location": "الموقع",
  "Location is required": "الموقع مطلوب",
  "Unit price must be greater than discount price": "يجب أن يكون سعر الوحدة أكبر من سعر الخصم",
  "Discount price must be greater than purchase price": "يجب أن يكون سعر الخصم أكبر من سعر الشراء",
  "Unit price must be greater than purchase price": "يجب أن يكون سعر الوحدة أكبر من سعر الشراء",
  "Main Categories": "الفئات الرئيسية",
  "missing": "مفقود",
  "Multiply Shipping × Qty": "ضرب الشحن × الكمية",
  "New Product": "منتج جديد",
  "new upload": "رفع جديد",
  "No images in your gallery yet.": "لا توجد صور في معرضك بعد.",
  "Off": "معطّل",
  "On": "مفعّل",
  "Origin Country": "بلد المنشأ",
  "Product created successfully.": "تم إنشاء المنتج بنجاح.",
  "Product Images": "صور المنتج",
  "Product name is required": "اسم المنتج مطلوب",
  "Brand is required": "العلامة التجارية مطلوبة",
  "Qty": "الكمية",
  "Remove": "إزالة",
  "removed": "مُزال",
  "Report Ref.": "مرجع التقرير",
  "Retry": "إعادة المحاولة",
  "Select": "اختيار",
  "Select a category to see its attributes.": "اختر فئة لعرض خصائصها.",
  "Select a valid unit": "اختر وحدة صالحة",
  "Select an image.": "اختر صورة.",
  "Select one or more images.": "اختر صورة واحدة أو أكثر.",
  "Seller product ID is required": "معرّف منتج البائع مطلوب",
  "set": "مُحدد",
  "Set attribute values for the selected categories. All optional.": "عيّن قيم الخصائص للفئات المحددة. الكل اختياري.",
  "Sizes": "المقاسات",
  "Stock": "المخزون",
  "Sub Categories": "الفئات الفرعية",
  "Sub-sub Categories": "الفئات الفرعية الثانوية",
  "Tax": "الضريبة",
  "These details will be saved": "سيتم حفظ هذه التفاصيل",
  "unassigned": "غير مخصص",
  "updated": "مُحدّث",
  "Upload from device": "رفع من الجهاز",
  "Variant Pricing / Stock": "تسعير/مخزون الخيارات",
  "variant(s)": "خيار/خيارات",
  "Weight is required for pc / liter units": "الوزن مطلوب لوحدات القطعة/اللتر",
  // ---- i18n audit fixes ----
  "Failed to load wallet currencies": "فشل تحميل عملات المحفظة",
  "Failed to load wallet data": "فشل تحميل بيانات المحفظة",
  "Failed to load wallet balance": "فشل تحميل رصيد المحفظة",
  "Insufficient wallet balance": "رصيد المحفظة غير كافٍ",
  "Wallet payment failed. Please try again": "فشل الدفع من المحفظة. يرجى المحاولة مرة أخرى",
  "Wallet Payment": "الدفع من المحفظة",
  "Select Currency": "اختر العملة",
  "Amount to Pay": "المبلغ المطلوب دفعه",
  "Insufficient wallet balance for this payment": "رصيد المحفظة غير كافٍ لإتمام هذا الدفع",
  "Confirm Wallet Payment": "تأكيد الدفع من المحفظة",
  "Back to your wallet": "العودة إلى محفظتك",
  "Order processing timed out. Please check your orders": "انتهت مهلة معالجة الطلب. يرجى التحقق من طلباتك",
  "Add To Bag Within": "أضف إلى الحقيبة خلال",
  "Seconds": "ثوانٍ",
  "Only": "فقط",
  "Soon": "قريباً",
  "Province | District | Town | Street": "المحافظة | القضاء | البلدة | الشارع",
  "Search Province | District | Town | Street": "ابحث عن المحافظة | القضاء | البلدة | الشارع",
  "Select From List": "اختر من القائمة",
  "Error getting your location": "حدث خطأ أثناء تحديد موقعك",
  "Geolocation is not supported by your browser": "متصفحك لا يدعم تحديد الموقع الجغرافي",
  "Your Address List Is Empty": "قائمة عناوينك فارغة",
  "Select Your Region": "اختر منطقتك",
  "Switching Country...": "جارٍ تغيير الدولة...",
  "Couldn't load your request": "تعذّر تحميل طلبك",
  "We couldn't load your seller request status. Please try again.": "تعذّر تحميل حالة طلب البائع الخاص بك. يرجى المحاولة مرة أخرى.",
  "Your seller request is under review": "طلب البائع الخاص بك قيد المراجعة",
  "We've received your seller request and it's currently being reviewed. We'll let you know once it's processed.": "لقد استلمنا طلب البائع الخاص بك وتجري مراجعته حالياً. سنعلمك بمجرد معالجته.",
  "Your seller request was rejected": "تم رفض طلب البائع الخاص بك",
  "Unfortunately, your seller request was not approved.": "للأسف، لم تتم الموافقة على طلب البائع الخاص بك.",
  "Your seller request was approved": "تمت الموافقة على طلب البائع الخاص بك",
  "Your seller request has been approved. You can now start selling on Trydos.": "تمت الموافقة على طلب البائع الخاص بك. يمكنك الآن البدء بالبيع على Trydos.",
  "Sales": "المبيعات",
  "Add your first product": "أضف أول منتج لك",
  "Add Product": "إضافة منتج",
  "Boutique not found": "لم يتم العثور على البوتيك",
  "Position your product here": "ضع منتجك هنا",
  "Please Verify Your Phone Number": "يرجى تأكيد رقم هاتفك",
  "User Name": "اسم المستخدم",
  "Enter Full User Name": "أدخل اسم المستخدم كاملاً",
  "Full name is required": "الاسم الكامل مطلوب",
  "Please select your gender": "يرجى تحديد الجنس",
  // ---- i18n audit fixes ----
  "cancel": "إلغاء",
  "Your Message": "رسالتك",
  "Edt": "تعديل",
  "Link copied to clipboard!": "تم نسخ الرابط!",
  "Telegram": "تيليجرام",
  "Failed to Upload file": "فشل رفع الملف",
  "Failed to send message": "فشل إرسال الرسالة",
  "Please enable camera permissions to use camera features": "يرجى تفعيل أذونات الكاميرا لاستخدام ميزات الكاميرا",
  "Failed to Upload audio": "فشل رفع الملف الصوتي",
  "No available Microphone": "لا يوجد ميكروفون متاح",
  "Cookies Is Not Enabled": "ملفات تعريف الارتباط غير مُفعّلة",
  "Show OTP Statics": "عرض إحصائيات OTP",
  "failed to search with image": "فشل البحث بالصورة",
  "Take photo with camera": "التقط صورة بالكاميرا",
  "Choose file from device": "اختر ملفاً من الجهاز",
  "a device": "جهاز",
  "Log in on this device?": "تسجيل الدخول على هذا الجهاز؟",
  "Deny": "رفض",
  "Approve": "موافقة",
  "Camera unavailable — allow camera access to scan": "الكاميرا غير متاحة — اسمح بالوصول إلى الكاميرا للمسح",
  "Scan to sign in": "امسح للدخول",
  "Point at the QR on your other screen": "وجّه الكاميرا نحو رمز QR على شاشتك الأخرى",
  "Failed To Upload Image..Try Again": "فشل رفع الصورة.. حاول مرة أخرى",
  "Please Login First": "يرجى تسجيل الدخول أولاً",
  "Removed from checklist": "تمت الإزالة من قائمة التحقق",
  "Failed to remove from checklist": "فشلت الإزالة من قائمة التحقق",
  "Failed to process image": "فشلت معالجة الصورة",
  "This device has only one camera": "هذا الجهاز يحتوي على كاميرا واحدة فقط",
  "Camera constraints not supported, trying fallback settings": "قيود الكاميرا غير مدعومة، جارٍ تجربة إعدادات بديلة",
  "Camera access denied": "تم رفض الوصول إلى الكاميرا",
  "No camera found": "لم يتم العثور على كاميرا",
  "Camera error occurred": "حدث خطأ في الكاميرا",
  "Close camera": "إغلاق الكاميرا",
  "Photo captured!": "تم التقاط الصورة!",
  "Retake": "إعادة الالتقاط",
  "Use Photo": "استخدام الصورة",
  "Switch camera": "تبديل الكاميرا",
  "Capture photo": "التقاط صورة",
  "Comment input": "حقل إدخال التعليق",
  "Quick Video": "فيديو سريع",
  "Please choose a JPEG, PNG or WebP image": "يرجى اختيار صورة بصيغة JPEG أو PNG أو WebP",
  "The photo must be 4 MB or less": "يجب ألا يتجاوز حجم الصورة 4 ميغابايت",
  "Add a photo": "إضافة صورة",
  "Report photo": "الإبلاغ عن الصورة",
  "Scan QR to sign in": "امسح رمز QR للدخول",
  "Only image or PDF files are allowed": "يُسمح فقط بملفات الصور أو PDF",
  "Only image and video files are allowed": "يُسمح فقط بملفات الصور أو الفيديو",
  "Notification is Denied": "تم رفض الإشعارات",
  "User in Another Call": "المستخدم في مكالمة أخرى",
  "New message": "رسالة جديدة",
  "Loading": "جارٍ التحميل",
  "Failed to update profile Info": "فشل تحديث معلومات الملف الشخصي",
  "Failed to update name": "فشل تحديث الاسم",
  "Failed to send verification code": "فشل إرسال رمز التحقق",
  "Failed to Initialize Call": "فشل بدء المكالمة",
  "Contacts API not supported on this browser": "واجهة جهات الاتصال غير مدعومة في هذا المتصفح",
  "Add Contact Manually": "إضافة جهة اتصال يدوياً",
  // ---- i18n audit fixes ----
  "Within": "خلال",
  "You Have New Messages From Delivery Worker..click for more": "لديك رسائل جديدة من عامل التوصيل..انقر للمزيد",
  "Alternative Phone Number": "رقم الهاتف البديل",
  "Enter Recipient Phone Number": "أدخل رقم هاتف المستلم",
  "Enter Alternative Recipient Phone Number": "أدخل رقم الهاتف البديل للمستلم",
  "Find Offers": "ابحث عن العروض",
  "Quantity Running Out.": "الكمية تنفد.",
  "Hurry Up! Quantity Running Out.": "أسرع! الكمية تنفد.",
  "Safe Payment": "دفع آمن",
  "Secure Payment": "دفع آمن",
  "Order": "طلب",
  "Order ID": "معرف الطلب",
  "Order Placed Successfully": "تم تقديم الطلب بنجاح",
  "Order Placed Failed": "فشل تقديم الطلب",
  "Order Placed Failed Please Try Again": "فشل تقديم الطلب، يرجى المحاولة مرة أخرى",
  "Order Placed Successfully Please Wait For Confirmation": "تم تقديم الطلب بنجاح، يرجى انتظار التأكيد",
  "Ship To You Accepted ": "تم قبول الشحن إليك في",
  "You Can Change Size, Color, Qty Of The Product Without Any Conditions According To The Change Policy": "يمكنك تغيير المقاس واللون والكمية للمنتج دون أي شروط وفقاً لسياسة الاستبدال",
  "To": "إلى",
  "Choose The Verification Method, Receive Code Via": "اختر طريقة التحقق، استلم الرمز عبر",
  "Tap \" Agree & Continue\" To Accept trydos": "انقر على \" موافق ومتابعة\" لقبول ترايدوس",
  "Enabled Notifications Topic": "مواضيع الإشعارات المُفعّلة",
  "Disabled Notifications Topic": "مواضيع الإشعارات المُعطّلة",
  "No topics subscribed": "لا توجد مواضيع مشترك بها",
  "notifications subscription": "اشتراك الإشعارات",
  "Pick Your Delivery Location Inside Your Country": "اختر موقع التوصيل داخل بلدك",
  // ---- i18n audit fixes ----
  "minutes": "دقائق",
  "Errors": "الأخطاء",
  "Edit Shipping Address": "تعديل عنوان الشحن",
  "Verify": "تحقق",
  "Change ": "تغيير ",
  "Found on a device — approve on your phone":
    "تم العثور على جهاز — وافق من هاتفك",
  "Approved (demo)": "تمت الموافقة (تجريبي)",
  "Request declined": "تم رفض الطلب",
  "Code expired — reopen to refresh":
    "انتهت صلاحية الرمز — أعد الفتح للتحديث",
  "Simulate scan (dev)": "محاكاة المسح (تجريبي)",
  "Too many verification requests, please try again later":
    "طلبات تحقق كثيرة جداً، يرجى المحاولة لاحقاً",
  "We Recommend That You Complete Your Profile To Make The Most Of The App’s Features, Such As Shopping, Chatting, Stories, Taking Advantage Of Offers, Interests, And Much More":
    "نوصيك بإكمال ملفك الشخصي للاستفادة القصوى من ميزات التطبيق، مثل التسوق والدردشة والقصص والاستفادة من العروض والاهتمامات وأكثر من ذلك بكثير",
  "Added to checklist": "أُضيف إلى القائمة",
  "Removed From Compare": "تمت الإزالة من المقارنة",
  "please select one contact at least": "يرجى اختيار جهة اتصال واحدة على الأقل",
  "Boutique is required": "المتجر مطلوب",
  "Select at least one category": "اختر فئة واحدة على الأقل",
  "Description is required": "الوصف مطلوب",
  "Please check this field": "يرجى مراجعة هذا الحقل",
  "Check the product images and their order": "تحقق من صور المنتج وترتيبها",
  "Every color needs at least one image, and every image must be assigned": "كل لون يحتاج صورة واحدة على الأقل، ويجب تعيين كل صورة للون",
  "Attributes are not editable yet. This section is a preview and nothing here is saved.": "الخصائص غير قابلة للتعديل بعد. هذا القسم للعرض فقط ولا يتم حفظ أي شيء هنا.",
  "Share": "مشاركة",
  "SKU must be unique within the product": "يجب أن يكون رمز SKU فريداً ضمن المنتج",
  "SKU must be unique": "رمز SKU يجب أن يكون فريداً",
  "Variation SKUs must be unique within the product": "يجب أن تكون رموز SKU للخيارات فريدة ضمن المنتج",
  "Save Draft": "حفظ كمسودة",
  "Load Draft": "تحميل المسودة",
  "Draft saved successfully.": "تم حفظ المسودة بنجاح.",
  "No saved draft found.": "لم يتم العثور على مسودة محفوظة.",
  "Draft loaded successfully.": "تم تحميل المسودة بنجاح.",
  "Failed to save draft.": "فشل حفظ المسودة.",
  "Failed to load draft.": "فشل تحميل المسودة.",
  "Variant quantity cannot be negative": "لا يمكن أن تكون كمية الخيار سالبة",
  "Variant price cannot be negative": "لا يمكن أن يكون سعر الخيار سالباً",
  "Variant discount price cannot be negative": "لا يمكن أن يكون سعر خصم الخيار سالباً",
  "Variant luck price cannot be negative": "لا يمكن أن يكون سعر الحظ للخيار سالباً",
  "Luck price cannot be greater than unit price": "لا يمكن أن يكون سعر الحظ أكبر من سعر الوحدة",
  "Luck price cannot be greater than discount price": "لا يمكن أن يكون سعر الحظ أكبر من سعر الخصم",
  "Variant luck price cannot be greater than unit price": "لا يمكن أن يكون سعر الحظ للخيار أكبر من سعر الوحدة",
  "Variant luck price cannot be greater than discount price": "لا يمكن أن يكون سعر الحظ للخيار أكبر من سعر الخصم",
  "Product name is required for": "اسم المنتج مطلوب للغة",
  "Description is required for": "الوصف مطلوب للغة",
  "original": "الأصلي",
  "Must be 0 or greater": "يجب أن تكون القيمة 0 أو أكبر",
  "Auto-calculated from variations": "محسوب تلقائياً من الخيارات",
  "Default Language": "اللغة الافتراضية",
  "Select default language": "اختر اللغة الافتراضية",
  "Select the primary language for this product. You can enter details for this language now and other languages will be translated automatically.": "اختر اللغة الأساسية لهذا المنتج. يمكنك إدخال التفاصيل بهذه اللغة الآن وسيتم ترجمة اللغات الأخرى تلقائياً.",
  "Similar Words": "الكلمات المشابهة",
  "Type a word and press Enter": "اكتب كلمة واضغط على Enter",
  "Add": "إضافة",
  "Attribute": "الخاصية",
  "Product updated, but attributes failed to save.": "تم تحديث المنتج، لكن تعذّر حفظ الخصائص.",
  // Seller dashboard — Locations section
  "Locations": "المواقع",
  "Add Location": "إضافة موقع",
  "Edit Location": "تعديل الموقع",
  "Address": "العنوان",
  "Latitude": "خط العرض",
  "Longitude": "خط الطول",
  "No locations found": "لا توجد مواقع",
  "Locations added to this shop will appear here.": "ستظهر هنا المواقع المضافة إلى هذا المتجر.",
  "Add your first location": "أضف موقعك الأول",
  "Warehouses and pickup points for this shop": "المستودعات ونقاط الاستلام لهذا المتجر",
  "You don't have permission to view locations": "ليس لديك صلاحية لعرض المواقع",
  "Failed to load locations": "تعذّر تحميل المواقع",
  "Failed to save location": "تعذّر حفظ الموقع",
  "Failed to change location status": "تعذّر تغيير حالة الموقع",
  "Location created successfully": "تم إنشاء الموقع بنجاح",
  "Location updated successfully": "تم تحديث الموقع بنجاح",
  "Status changed successfully": "تم تغيير الحالة بنجاح",
  "Pick the position on the map": "حدد الموقع على الخريطة",
  "Use my current location": "استخدم موقعي الحالي",
  "Location name": "اسم الموقع",
  "All statuses": "كل الحالات",
  "All countries": "كل الدول",
  "Deactivate": "إلغاء التفعيل",
  "Activate": "تفعيل",
  "Name is required": "الاسم مطلوب",
  "Country is required": "الدولة مطلوبة",
  "Latitude must be between -90 and 90": "يجب أن يكون خط العرض بين -90 و 90",
  "Longitude must be between -180 and 180": "يجب أن يكون خط الطول بين -180 و 180",
  "Map is unavailable": "الخريطة غير متاحة",
  "Coordinates are optional": "الإحداثيات اختيارية",
  // Product Editor & Changes Widget
  "Removed": "تمت الإزالة",
  "Updated": "مُحَدَّث",
  "New Images Added": "تمت إضافة صور جديدة",
  "Removed Images": "الصور المزالة",
  "Updated Gallery Preview": "معاينة المعرض المُحَدَّث",
  "Original Colors": "الألوان الأصلية",
  "New Selection": "التحديد الجديد",
  "languages": "لغات",
  "variants": "أنواع",
  "countries": "دول",
  "Attributes / Descriptors": "الخصائص / الواصفات",
  "Stock Qty": "كمية المخزون",
  "Variant Pricing & Stock": "أسعار ومخزون الأنواع",
  "Keywords": "الكلمات المفتاحية",
  "Keywords / Search Terms": "الكلمات المفتاحية / مصطلحات البحث",
  "Default Language Code": "رمز اللغة الافتراضية",
  "New video file uploaded": "تم رفع ملف فيديو جديد",
  "Removed Videos": "الفيديوهات المزالة",
  "video(s)": "فيديو",
  "Select default language for product creation. Other languages will be translated automatically.": "اختر اللغة الافتراضية لإنشاء المنتج. سيتم ترجمة اللغات الأخرى تلقائياً.",
  "Notifications Not Supported": "الإشعارات غير مدعومة",
  "Push notifications are not supported by your current browser or mode. To access features that depend on notifications, such as order updates and live chat, please switch to a supported browser like Chrome, Edge, or Safari.": "الإشعارات الفورية غير مدعومة في متصفحك الحالي أو الوضع الحالي. للاستفادة من الميزات التي تعتمد على الإشعارات، مثل تحديثات الطلبات والمحادثة المباشرة، يرجى الانتقال إلى متصفح مدعوم مثل Chrome أو Edge أو Safari.",
  "Got it": "حسناً",
  "Enter a Valid Value for Quantity": "أدخل قيمة صحيحة للكمية",
  "Enter a Valid Value for Quantity In Variants Table": "أدخل قيمة صحيحة للكمية في جدول المتغيرات",
  "Enter a valid Count of pieces": "أدخل عدد قطع صالح",
  "Select a valid origin country": "اختر بلد منشأ صالح",
  "Changes were saved": "تم حفظ التغييرات",
  // RDB Authentication Widget
  "Get Started !": "البدء !",
  "Experience fast and secure digital banking.": "تجربة مصرفية رقمية سريعة وآمنة.",
  "I Have an Account": "لدي حساب بالفعل",
  "New Customer": "عميل جديد",
  "Maybe Later": "ربما لاحقاً",
  "To create an account, you must agree to our Terms of Service": "لإنشاء حساب، يجب عليك الموافقة على شروط الخدمة الخاصة بنا",
  "Terms of Service": "شروط الخدمة",
  "Agree and Continue": "الموافقة والمتابعة",
  "Sign Up": "إنشاء حساب",
  "Sign In": "تسجيل الدخول",
  "Enter your mobile number to continue": "أدخل رقم هاتفك المحمول للمتابعة",
  "Verification code will be sent to this number": "سيتم إرسال رمز التحقق إلى هذا الرقم",
  "Your details are protected with bank-grade encryption": "بياناتك محمية بتشفير عالي الأمان",
  "Enter Phone Number": "أدخل رقم الهاتف",
  "Select your verification method": "اختر طريقة التحقق الخاصة بك",
  "Edit": "تعديل",
  "WhatsApp": "واتساب",
  "SMS": "رسالة نصية SMS",
  "Enter the 6-digit verification code sent via": "أدخل رمز التحقق المكون من 6 أرقام المرسل عبر",
  "Resend code in": "إعادة إرسال الرمز خلال",
  "Didn't receive code?": "لم تصلك الرسالة؟",
  "Resend Code": "إعادة إرسال الرمز",
  "or": "أو",
  "Change Number": "تغيير الرقم",
  "Change Method": "تغيير الطريقة",
  "Verify Your Number !": "تأكيد رقمك !",
  "Change Your Number !": "تغيير رقمك !",
  "Verification code has expired": "انتهت صلاحية رمز التحقق",
  "Enter Your Name !": "أدخل اسمك !",
  "Last Step And Enjoy Our Services": "الخطوة الأخيرة للتمتع بخدماتنا",
  "Ensure Greater Security And Protect Your Funds": "ضمان أمان أعلى وحماية بياناتك",
  "Enter Your Name Exact ID": "أدخل اسمك كما هو في الهوية",
  "Already Registered": "مسجل بالفعل",
  "This phone number is already registered with an account.": "رقم الهاتف هذا مسجل بالفعل بحساب آخر.",
  "Log In and Continue": "تسجيل الدخول والمتابعة",
  "Not Registered": "غير مسجل",
  "No account found with this phone number.": "لم نجد حساباً مرتبطاً برقم الهاتف هذا.",
  "Create New Account": "إنشاء حساب جديد",
  // RDB Auth Exact Copy Strings
  "Get Started !": "البدء !",
  "To Take Advantage Of All The Advantages Of The Application, Please Join Us In Quick And Easy Steps And For Just One Time": "للاستفادة من جميع مزايا التطبيق، يرجى الانضمام إلينا بخطوات سريعة وسهلة ولمرة واحدة فقط",
  "I Have Already Account": "لدي حساب بالفعل",
  "Later, Take A Look At The App": "لاحقاً، إلقاء نظرة على التطبيق",
  "To Create New Account Tap “Agree & Continue” To Accept": "لإنشاء حساب جديد انقر على \"الموافقة والمتابعة\" للقبول",
  "Terms Of Services": "شروط الخدمة",
  "Agree & Continue": "الموافقة والمتابعة",
  "Sign Up !": "إنشاء حساب !",
  "Login !": "تسجيل الدخول !",
  "Login": "تسجيل الدخول",
  "Enter Your Phone Number Registered With Us": "أدخل رقم هاتفك المسجل لدينا",
  "We Will Send A Verification Code To The Number": "سنقوم بإرسال رمز التحقق إلى الرقم",
  "Your Privacy Is Completely Safe": "خصوصيتك آمنة تماماً",
  "Enter Your Phone Number": "أدخل رقم هاتفك المحمول",
  "Choose Verification Method": "اختر طريقة التحقق",
  "Send WhatsApp": "إرسال عبر واتساب",
  "Send SMS": "إرسال عبر SMS",
  "Enter Verification Code Sent To Your ": "أدخل رمز التحقق المرسل إلى ",
  "Resend After - ": "إعادة الإرسال بعد - ",
  "Didn't You Receive A Code?": "ألم يصلك الرمز؟",
  "Method": "الطريقة",
  "Or": "أو",
  "The Code Sent Has Expired": "انتهت صلاحية الرمز المرسل",
  "Already Registered !": "مسجل بالفعل !",
  "This Number Already Registered With Us": "هذا الرقم مسجل لدينا بالفعل",
  "Login & Continue": "تسجيل الدخول والمتابعة",
  "Cancel & Take A Look At The App": "إلغاء وإلقاء نظرة على التطبيق",
  "Not Registered !": "غير مسجل !",
  "Sorry, This Number Is Not Registered With Us !": "عذراً، هذا الرقم غير مسجل لدينا !",
  "Create New Account & Continue": "إنشاء حساب جديد والمتابعة",
  // Auth flow — sign in / sign up
  "Sign in to your account": "سجّل الدخول إلى حسابك",
  "terms of services": "شروط الخدمة",
  "Enter Verification Code Sent To Your": "أدخل رمز التحقق المرسل عبر",
  "Whatsapp": "واتساب",
  "Resend After -": "إعادة الإرسال بعد -",
  "Login Successfully !": "تم تسجيل الدخول بنجاح !",
  "Welcome Back, Enjoy Our Services": "مرحباً بعودتك، استمتع بخدماتنا",
  "Sign Up Successfully !": "تم إنشاء الحساب بنجاح !",
  "Session limit reached. Try again later.": "تم بلوغ حد الجلسة. حاول مرة أخرى لاحقاً.",
  // Auth flow — QR login
  "Login with QR": "تسجيل الدخول برمز QR",
  "Show a new code": "عرض رمز جديد",
  "Login was declined on your phone": "تم رفض تسجيل الدخول من هاتفك",
  "Scanned — confirm on your phone to continue": "تم المسح — أكّد من هاتفك للمتابعة",
  "On your phone, open Settings → Linked Devices → Scan, then point the camera at this code.": "من هاتفك، افتح الإعدادات ← الأجهزة المرتبطة ← مسح، ثم وجّه الكاميرا نحو هذا الرمز.",
  "This device appears to be in a different city": "يبدو أن هذا الجهاز في مدينة مختلفة",
  // Auth flow — accessibility labels
  "Scan QR Code": "مسح رمز QR",
  "Send phone number": "إرسال رقم الهاتف",
  "Continue": "متابعة",

  // The login flow, worded exactly as the XD design file has it.
  "login !": "تسجيل الدخول !",
  "Sign up !": "إنشاء حساب !",
  "already registered !": "مسجل بالفعل !",
  "not registered !": "غير مسجل !",
  "enter your name !": "أدخل اسمك !",
  "Sign up successfully !": "تم إنشاء الحساب بنجاح !",
  "Enter your phone number registered with us": "أدخل رقم هاتفك المسجل لدينا",
  "Your privacy is completely safe": "خصوصيتك آمنة تماماً",
  "We will send a verification code to the number": "سنقوم بإرسال رمز التحقق إلى الرقم",
  "We have sent a verification code to the number": "لقد أرسلنا رمز التحقق إلى الرقم",
  "Choose verification method": "اختر طريقة التحقق",
  "this number already registered with us": "هذا الرقم مسجل لدينا بالفعل",
  "Sorry, this number is not registered with us !": "عذراً، هذا الرقم غير مسجل لدينا !",
  "Last step and enjoy our services": "الخطوة الأخيرة للتمتع بخدماتنا",
  "ensure greater security and protect your funds": "ضمان أمان أعلى وحماية بياناتك",
  "Enjoy with our services": "استمتع بخدماتنا",
  "Enter your Name exact id": "أدخل اسمك كما هو في الهوية",
  "Enter your phone number": "أدخل رقم هاتفك المحمول",
  "I have already account": "لدي حساب بالفعل",
  "New customer": "عميل جديد",
  "Later, take a look at the app": "لاحقاً، إلقاء نظرة على التطبيق",
  "Cancel & take a look at the app": "إلغاء وإلقاء نظرة على التطبيق",
  "login & Continue": "تسجيل الدخول والمتابعة",
  "Create new account & Continue": "إنشاء حساب جديد والمتابعة",
  "agree & continue": "الموافقة والمتابعة",
  "Terms of services": "شروط الخدمة",
  ". quick preview .": ". نظرة سريعة .",
  "send WhatsApp": "إرسال عبر واتساب",
  "Send Sms": "إرسال عبر SMS",
  "edit": "تعديل",
  "resend after -": "إعادة الإرسال بعد -",
  "Didn’t you receive a code?": "ألم يصلك الرمز؟",
  "resend code": "إعادة إرسال الرمز",
  "method": "الطريقة",
  "enter verification code sent to your": "أدخل رمز التحقق المرسل عبر",
  "Switch From your App": "التبديل من تطبيقك",
  "You can use your account on the web securely and easily.": "يمكنك استخدام حسابك على الويب بأمان وسهولة.",
  "Open your Trydos application": "افتح تطبيق Trydos",
  "choose Switch web": "اختر التبديل إلى الويب",
  "read this code from opposite side Camera": "امسح هذا الرمز بالكاميرا من الجهة المقابلة",
  "To Create new account Tap “agree & Continue” to accept": "لإنشاء حساب جديد انقر على \"الموافقة والمتابعة\" للقبول",
  "The code sent has expired": "انتهت صلاحية الرمز المرسل",
  "We Could Not Add More Of This Product": "لم نتمكن من إضافة المزيد من هذا المنتج",
  "Do You Want Us To Notify You When It Is Available?": "هل تريد أن نعلمك عندما يتوفر؟",
  "Scan this code in the RDB app": "امسح هذا الرمز في تطبيق RDB",
  "Payment code": "رمز الدفع",
  "Open the RDB app": "افتح تطبيق RDB",
  "Cancel payment": "إلغاء الدفع",
  "Waiting for your payment": "بانتظار الدفع",
  "Payment expired": "انتهت مهلة الدفع",
  "Payment cancelled": "تم إلغاء الدفع",
  "Payment failed": "فشل الدفع",
  "Payment received": "تم استلام الدفع",
  "Time left": "الوقت المتبقي",
  "Amount to pay": "المبلغ المطلوب",
  "You have a payment in progress": "لديك عملية دفع جارية",
  "Continue payment": "متابعة الدفع",
  "Your cart is locked until you finish or cancel the payment": "سلتك مقفلة حتى تكمل الدفع أو تلغيه",
  "Could not start the payment. Please try again": "تعذر بدء الدفع. حاول مرة أخرى",
  "Could not cancel the payment. Please try again": "تعذر إلغاء الدفع. حاول مرة أخرى",
  "This payment is already paid": "تم دفع هذه العملية بالفعل",
  "Hide menu": "إخفاء القائمة",
  "Device and browser info": "معلومات الجهاز والمتصفح",
  "Main": "الرئيسية",
  "Cart": "السلة",
  "Chat": "المحادثة",
  "Women": "نساء",
  "Children": "أطفال",
  "Electronic": "إلكترونيات",
  "Search anything…": "ابحث عن أي شيء…",
  "Voice search": "البحث الصوتي",
  "Scan": "مسح",
  "For you": "لك",
  "Empty": "فارغ",
  "Your cart is empty": "سلتك فارغة",
  "Items you add to your cart will show here": "المنتجات التي تضيفها إلى سلتك ستظهر هنا",
  "No conversations yet": "لا توجد محادثات بعد",
  "Messages with sellers will show here": "ستظهر رسائلك مع البائعين هنا",
  "lock": "قفل",
  "client ID": "معرّف العميل",
  "Enter Address !": "أدخل العنوان !",
  "Add photo": "إضافة صورة",
  "Setup your size profile": "جهّز ملف مقاساتك",
  "To get smart recommendations that fit you.": "لتحصل على اقتراحات ذكية تناسبك.",
  "Setup size now": "جهّز المقاسات الآن",
  "Unverified account": "حساب غير موثّق",
  "Tap verify, skip the line, and enjoy exclusive offers.": "اضغط توثيق، تجاوز الانتظار، واستمتع بعروض حصرية.",
  "Verify now": "وثّق الآن",
  "Get trydos Business": "احصل على trydos للأعمال",
  "Start Your Business now": "ابدأ عملك الآن",
  "action": "إجراء",
  "Login history": "سجل الدخول",
  "logout": "تسجيل الخروج",
  "Profile Photo": "الصورة الشخصية",
  "Add & save": "إضافة وحفظ",
  "Choose": "اختيار",
  "Uploading profile photo …": "جارٍ رفع الصورة الشخصية …",
  "copy": "نسخ",
  "download": "تنزيل",
  "share": "مشاركة",
  "client name": "اسم العميل",
  "client Phone Number": "رقم هاتف العميل",
  "client Info": "معلومات العميل",
  "Body measurements": "قياسات الجسم",
  "client Information": "معلومات العميل",
  "client Status": "حالة العميل",
  "client since": "عميل منذ",
  "client type": "نوع العميل",
  "client Verified": "توثيق العميل",
  "days": "أيام",
  "Personal": "شخصي",
  "Change my Phone Number Request": "طلب تغيير رقم هاتفي",
  "Delete my account Request": "طلب حذف حسابي",
  "full Name": "الاسم الكامل",
  "enter Email Address": "أدخل البريد الإلكتروني",
  "verification Email !": "توثيق البريد الإلكتروني !",
  "enter verification code sent to your Email": "أدخل رمز التحقق المرسل إلى بريدك الإلكتروني",
  "resend after": "إعادة الإرسال بعد",
  "How tall are you?": "كم طولك؟",
  "What is your weight?": "كم وزنك؟",
  "What is your Foot size?": "ما مقاس قدمك؟",
  "Add Shipping Address": "إضافة عنوان شحن",
  "Your address list is empty": "قائمة عناوينك فارغة",
  "You can also create multiple addresses to use": "يمكنك أيضًا إنشاء عدة عناوين لاستخدامها",
  "Delete below address ?": "حذف العنوان أدناه ؟",
  "Sure, Delete": "نعم، احذف",
  "Turkiye": "تركيا",
  "Select from list": "اختر من القائمة",
  "Detailed address": "العنوان التفصيلي",
  "Street address, building, Flat, Door, unit.": "الشارع، المبنى، الشقة، الباب، الوحدة.",
  "Address title": "اسم العنوان",
  "Ex: Home, my office, 2 home ect.": "مثال: المنزل، مكتبي، المنزل 2 إلخ.",
  "your location on map": "موقعك على الخريطة",
  "Locate your location on map": "حدد موقعك على الخريطة",
  "location is accurate, making it easy to receive shipments": "الموقع الدقيق يسهّل استلام الشحنات",
  "District": "المنطقة",
  "Street": "الشارع",
  "Entering your information correctly allows us to provide you with better service and benefit from all services.": "إدخال معلوماتك بشكل صحيح يتيح لنا تقديم خدمة أفضل لك والاستفادة من جميع الخدمات.",
  "Why add a address?": "لماذا تضيف عنوانًا؟",
  "Accurate address details ensure faster delivery of your correct products and show you the best options in your area.": "تفاصيل العنوان الدقيقة تضمن توصيلًا أسرع لمنتجاتك الصحيحة وتعرض لك أفضل الخيارات في منطقتك.",
  "Learn more": "اعرف المزيد",
  // Chat: message edit, tags, reminders, archive and unread.
  "Forwarded": "مُعاد توجيهها",
  "Edited": "مُعدّلة",
  "Edit message": "تعديل الرسالة",
  "Tag message": "وسم الرسالة",
  "Remind me": "ذكّرني",
  "Reminder set for": "التذكير مضبوط على",
  "Cancel reminder": "إلغاء التذكير",
  "In 20 minutes": "بعد 20 دقيقة",
  "In 1 hour": "بعد ساعة",
  "In 3 hours": "بعد 3 ساعات",
  "Tomorrow morning": "صباح الغد",
  "Pick a date and time": "اختر التاريخ والوقت",
  "Set": "ضبط",
  "Choose a time in the future": "اختر وقتاً في المستقبل",
  "Choose an earlier time": "اختر وقتاً أقرب",
  "Urgent": "عاجل",
  "Important": "مهم",
  "To do": "للتنفيذ",
  "Unarchive": "إلغاء الأرشفة",
  "Archived": "المؤرشفة",
  "Reminders": "التذكيرات",
  "No archived chats": "لا توجد محادثات مؤرشفة",
  "No reminders": "لا توجد تذكيرات",
  "Message": "رسالة",
  "Tagged messages": "الرسائل الموسومة",
  "Could not load the messages": "تعذّر تحميل الرسائل",
  "No messages with this tag": "لا توجد رسائل بهذا الوسم",
  "Failed to edit the message": "تعذّر تعديل الرسالة",
  "Failed to update the tag": "تعذّر تحديث الوسم",
  "Failed to set the reminder": "تعذّر ضبط التذكير",
  "Failed to cancel the reminder": "تعذّر إلغاء التذكير",
  "Failed to archive the chat": "تعذّرت أرشفة المحادثة",
  "Failed to unarchive the chat": "تعذّر إلغاء أرشفة المحادثة",
  "Failed to mark the chat as unread": "تعذّر تحديد المحادثة كغير مقروءة",
};

export default translations;


