const translations = {
  "Typing...": "Yazıyor...",
  "Employees": "Çalışanlar",
  "Roles": "Roller",
  "Job Titles": "İş Unvanları",
  "Offices": "Ofisler",
  "Departments": "Departmanlar",
  "Work Forms": "Çalışma Türleri",
  "Currencies": "Para Birimleri",
  "Countries": "Ülkeler",
  "Comments": "Yorumlar",
  "Admin": "Yönetim",
  "Read Locations": "Konumları Görüntüle",
  "Create Location": "Konum Oluştur",
  "Update Location": "Konumu Güncelle",
  "Change Location Status": "Konum Durumunu Değiştir",
  "Read Shop Info": "Mağaza Bilgilerini Görüntüle",
  "Update Shop Info": "Mağaza Bilgilerini Güncelle",
  "Read Product Images": "Ürün Resimlerini Görüntüle",
  "Upload Product Images": "Ürün Resimleri Yükle",
  "Delete Product Images": "Ürün Resimlerini Sil",
  "Read Story": "Hikayeleri Görüntüle",
  "Read Comments": "Yorumları Görüntüle",
  "Reply Comment": "Yoruma Yanıt Ver",
  "Tries left": "Kalan deneme",
  "Too many wrong codes. Ask for a new code.": "Çok fazla yanlış kod. Yeni bir kod isteyin.",
  "This product has pending changes awaiting admin approval. The form below shows your submitted changes; the live product still shows the previous values until approval.": "Bu üründe yönetici onayı bekleyen değişiklikler var. Aşağıdaki form gönderdiğiniz değişiklikleri gösterir; yayındaki ürün onaylanana kadar önceki değerleri göstermeye devam eder.",
  "Your last changes to this product were denied. The live product still shows the previous values.": "Bu üründe yaptığınız son değişiklikler reddedildi. Yayındaki ürün hâlâ önceki değerleri gösteriyor.",
  "Couldn't load your shop details. Product creation is unavailable until they load.": "Mağaza bilgileriniz yüklenemedi. Bilgiler yüklenene kadar ürün oluşturulamaz.",
  "Adding a product needs permission to view shop info. Ask a shop admin to grant you that permission, then try again.": "Ürün eklemek için mağaza bilgilerini görüntüleme izni gerekir. Bir mağaza yöneticisinden bu izni vermesini isteyin, sonra tekrar deneyin.",
  "Couldn't load your shop details. Editing this product is unavailable until they load.": "Mağaza bilgileriniz yüklenemedi. Bilgiler yüklenene kadar bu ürün düzenlenemez.",
  "Opening a product needs permission to view shop info. Ask a shop admin to grant you that permission, then try again.": "Bir ürünü açmak için mağaza bilgilerini görüntüleme izni gerekir. Bir mağaza yöneticisinden bu izni vermesini isteyin, sonra tekrar deneyin.",
  "Bold": "Kalın",
  "Italic": "İtalik",
  "Underline": "Altı çizili",
  "Heading": "Başlık",
  // Listing share widget
  "Share this page": "Bu sayfayı paylaş",
  "Send this page to your favourite apps": "Bu sayfayı favori uygulamalarınıza gönderin",
  "Link Copied to Clipboard": "Bağlantı panoya kopyalandı",
  // Listing sort widget
  "Sort products": "Ürünleri sırala",
  "Choose how products are ordered": "Ürünlerin nasıl sıralanacağını seçin",
  Default: "Varsayılan",
  Recommended: "Önerilen",
  "Best match for your search": "Aramanız için en iyi eşleşme",
  "Best sellers": "Çok satanlar",
  "Most bought right now": "Şu anda en çok alınanlar",
  "New arrivals": "Yeni gelenler",
  "By date added": "Eklenme tarihine göre",
  Newest: "En yeni",
  Oldest: "En eski",
  "By product price": "Ürün fiyatına göre",
  "Low to High": "Düşükten yükseğe",
  "High to Low": "Yüksekten düşüğe",
  Alphabetical: "Alfabetik",
  "A to Z": "A'dan Z'ye",
  "Z to A": "Z'den A'ya",
  // Become Seller modal
  "Tell us about you and your shop to start selling":
    "Satışa başlamak için kendinizden ve mağazanızdan bahsedin",
  Password: "Şifre",
  "Select document type": "Belge türünü seçin",
  John: "Ahmet",
  Doe: "Yılmaz",
  "example@mail.com": "example@mail.com",
  "+1 234...": "+90 5XX...",
  "My Shop": "Mağazam",
  "Street 1": "1. Sokak",
  Warehouse: "Depo",
  "Full Address": "Tam Adres",
  Identity: "Kimlik",
  Passport: "Pasaport",
  "Commercial License": "Ticari Lisans",
  "Tax Certificate": "Vergi Sertifikası",
  "Bank Statement": "Banka Hesap Özeti",
  "Address Proof": "Adres Belgesi",
  "Authorization Letter": "Yetki Belgesi",
  "Please select a document type": "Lütfen bir belge türü seçin",
  "Something went wrong": "Bir şeyler ters gitti",
  "We could not load this part of the page": "Sayfanın bu bölümünü yükleyemedik",
  "Upload URL not found": "Yükleme bağlantısı bulunamadı",
  "Failed to add contact": "Kişi eklenemedi",
  "Failed to upload document": "Belge yüklenemedi",
  // Report Story flow
  "Inappropriate Content": "Uygunsuz İçerik",
  "Harassment or Hate Speech": "Taciz veya Nefret Söylemi",
  "Spam or Scam": "Spam veya Dolandırıcılık",
  "Intellectual Property Violation": "Fikri Mülkiyet İhlali",
  "Violence or Dangerous Content": "Şiddet veya Tehlikeli İçerik",
  Other: "Diğer",
  "Report Reason": "Bildirme Nedeni",
  "Details (optional)": "Detaylar (isteğe bağlı)",
  "Details": "Detaylar",
  "Write details here...": "Detayları buraya yazın...",
  "Submit Report": "Bildirimi Gönder",
  "Tell us what went wrong so we can improve your experience":
    "Deneyiminizi iyileştirebilmemiz için neyin yanlış gittiğini bize bildirin",
  "Additional notes": "Ek notlar",
  Optional: "İsteğe bağlı",
  "Write more details here": "Buraya daha fazla ayrıntı yazın",
  "We received your report. Thanks for your thoughts":
    "Bildiriminizi aldık. Görüşleriniz için teşekkürler",
  "Could not submit your report. Please try again":
    "Bildiriminiz gönderilemedi. Lütfen tekrar deneyin",
  "We received your report": "Bildiriminizi aldık",
  "Thanks for your thoughts": "Görüşleriniz için teşekkürler",
  "Please select at least one reason or provide details.":
    "Lütfen en az bir neden seçin veya detay belirtin.",
  "Copy Data": "Verileri Kopyala",
  "copy reason of verification success!":
    "Doğrulama nedeni başarıyla kopyalandı!",
  Type: "Yaz",
  "Pick a category, download its template, fill it in, then upload it.":
    "Bir kategori seçin, şablonunu indirin, doldurun ve ardından yükleyin.",
  Category: "Kategori",
  "Loading categories...": "Kategoriler yükleniyor...",
  "Select a category": "Bir kategori seçin",
  "Failed to load categories": "Kategoriler yüklenemedi",
  "Please select a category first": "Lütfen önce bir kategori seçin",
  "Download Template": "Şablonu İndir",
  "Preparing template...": "Şablon hazırlanıyor...",
  "Template is not available": "Şablon mevcut değil",
  "Failed to download template": "Şablon indirilemedi",
  "Supports .xlsx, .xls, .xlsm, .xlsb": ".xlsx, .xls, .xlsm, .xlsb destekler",
  "Please upload a valid Excel file (.xlsx, .xls, .xlsm, .xlsb)":
    "Lütfen geçerli bir Excel dosyası yükleyin (.xlsx, .xls, .xlsm, .xlsb)",
  "File uploaded and processed successfully!":
    "Dosya başarıyla yüklendi ve işlendi!",
  "Select Shop": "Mağaza Seçimi",
  "Customers Comments": "Müşteri Yorumları",
  "Upload Excel File": "Excel Dosyası Yükle",
  "FAQ": "SSS",
  "Reviewing": "Değerlendirmeler",
  "Comment ID": "Yorum Kimliği",
  "Customer": "Müşteri",
  "Comment": "Yorum",
  "Rating": "Puanlama",
  "Variant": "Varyant",
  "Date": "Tarih",
  "Reply Text": "Yanıt Metni",
  "Reply": "Yanıtla",
  "Edit Reply": "Yanıtı Düzenle",
  "Write a reply...": "Bir yanıt yazın...",
  "Submit Reply": "Yanıtı Gönder",
  "Drag & drop Excel file here, or click to select": "Excel dosyasını buraya sürükleyip bırakın veya seçmek için tıklayın",
  "Select Excel File": "Excel Dosyası Seç",
  "Upload Excel": "Excel Yükle",
  "Uploading...": "Yükleniyor...",
  "File uploaded successfully!": "Dosya başarıyla yüklendi!",
  "File upload failed.": "Dosya yükleme başarısız oldu.",
  "No file selected": "Dosya seçilmedi",
  "Wallet Deposits": "Cüzdan Yatırımları",
  "View / Create": "Görüntüle / Oluştur",
  "Product Not Found": "Ürün Bulunamadı",
  "Buyer Rate": "Alıcı Puanı",
  "Shipping Company": "Kargo Şirketi",
  "Shop Info":"Mağaza Bilgisi",
  Recommendation: "Tavsiye",
  "Confirm Returning Items": "Öğeleri iade etmeyi onayla",
  "Cancel All Return Requests": "Tüm iade taleplerini iptal et",
  "Update Return Request For This Product":
    "Bu ürün için iade talebini güncelle",
  Complaint: "Şikayet",
  Visitors: "Ziyaretçiler",
  "And Not Necessarily From Customers Who Have Purchased The Product Before. These Are Pre-Purchase Questions, And They Are Answered Directly By The Seller":
    "Ve daha önce ürünü satın almış müşterilerden olması gerekmiyor. Bunlar satın alma öncesi sorulardır ve satıcı tarafından doğrudan cevaplanır",
  "All The Questions Below Are From": "Aşağıdaki tüm sorular şunlardan",
  Quality: "Kalite",
  "There is No Comments Yet..": "Henüz yorum yok..",
  "Product Price When Returned": "Ürün İade Edildiğinde Fiyatı",
  Full: "Tam",
  "Buyers Product Rate": "Alıcıların Ürün Puanı",
  "All Reviews Are Genuine From Customers Who Purchased And Actually Received The Product Through":
    "Tüm yorumlar, ürünü satın alıp gerçekten teslim alan müşterilerden gelmektedir",
  "All Comments Are Genuine From Customers Who Purchased And Actually Received The Product Through":
    "Tüm yorumlar, ürünü satın alıp gerçekten teslim alan müşterilerden gelmektedir",
  "Overall Good Quality": "Genel İyi Kalite",
  "Recommend It By": "Tavsiye Eden",
  "Buyers Product Recommend To Buy": "Alıcıların Ürünü Satın Alma Önerisi",
  "All Recommendations Are Genuine From Customers Who Purchased And Actually Received The Product Through":
    "Tüm öneriler, ürünü satın alıp gerçekten teslim alan müşterilerden gelmektedir",
  "Buyers Comment": "Alıcı Yorumu",
  "Refund Processing": "İade İşleniyor",
  "Very Bad": "Çok Kötü",
  Bad: "Kötü",
  Normal: "Normal",
  Good: "İyi",
  "Very Good": "Çok İyi",
  "get the Amount back": "Miktarı Geri Al",
  "Good Quality": "İyi Kalite",
  "Views Product": "Ürün Görüntülemeleri",
  "True Size": "Gerçek Beden",
  "Recommend It": "Tavsiye Ediyorum",
  "If Delivered After": "Eğer şu tarihten sonra teslim edilirse",
  "You Will Get 25 USD To Your Wallet Automatically":
    "Cüzdanınıza otomatik olarak 25 USD yüklenecek",
  Within: "İçinde",
  "Days After Receiving The Product, You Can Return It Without Conditions Or Reasons With Complete Ease And Get The Amount Back":
    "Ürünü teslim aldıktan sonraki günler içinde, hiçbir koşul veya gerekçe olmadan kolayca iade edebilir ve paranızı geri alabilirsiniz",
  "Days After Receiving The Product, You Can Return It Without Conditions Or Reasons With Complete Ease And":
    "Ürünü teslim aldıktan sonra günler içinde, herhangi bir şart veya sebep olmadan kolayca iade edebilirsiniz ve",
  "Dont Recommend It": "Tavsiye Etmiyorum",
  "FAQ Buyer & Seller": "SSS Alıcı & Satıcı",
  "Expected Shipping & delivery Date": "Beklenen Kargo ve Teslimat Tarihi",
  "Based On The Previous Delivery Statistics Below To Your Area, We Conclude That The Expected Delivery Time For Your Product Is":
    "Aşağıdaki bölgenize ait önceki teslimat istatistiklerine göre, ürününüz için tahmini teslimat süresinin şu olduğunu belirledik:",
  "Get a": "Al",
  "Refund Of The Product Price If Shipping Is Delayed":
    "Kargo gecikirse ürün fiyatı iadesi al",
  January: "Ocak",
  February: "Şubat",
  March: "Mart",
  April: "Nisan",
  May: "Mayıs",
  June: "Haziran",
  July: "Temmuz",
  August: "Ağustos",
  September: "Eylül",
  October: "Ekim",
  November: "Kasım",
  December: "Aralık",
  Dear: "Sevgili",
  "Sizes Available": "Mevcut Bedenler",
  "Out For Return": "Yolda",
  "Buyers Reviews On Product Sizing":
    "Alıcıların Ürün Bedenleri Hakkındaki Yorumları",
  "Ask Seller Your Question About This Product …":
    "Satıcıya bu ürün hakkında sorunuzu sorun…",
  "Product Return Has Been Requested": "Ürün iadesi talep edildi",
  Translate: "Çevir",
  "Show Original": "Orijinali Göster",
  "Waiting…": "Bekleniyor…",
  Wait: "Bekleyin",
  "Wait": "Bekleyin",
  "s": "sn",
  "before trying again": "tekrar denemeden önce",
  "Product Return Request Approve": "Ürün iade talebini onayla",
  "Product Return Needs Confirm": "Ürün iade talebi onay gerektiriyor",
  "Notification is Blocked in This Browser Please Enable Notification premission and refresh":
    "Bildirimler bu tarayıcıda engellenmiş, lütfen bildirim iznini etkinleştirip sayfayı yenileyin",
  "Notification is Blocked in This Browser Please Enable Notification premission and refresh":
    "ئاگادارکردنەوە لەم وێبگەڕەدا قەدەغە کراوە، تکایە مۆڵەتی ئاگادارکردنەوە چالاک بکە و پەڕەکە نوێ بکەرەوە",
  "You Need To Confirm Your Request": "Talebinizi onaylamanız gerekiyor",
  "Product Return Request Approved": "Ürün iade talebi onaylandı",
  "Product Collection Within 1 Day": "Ürün 1 gün içinde teslim alınacak",
  "Product Has Been Returned Successfully": "Ürün başarıyla iade edildi",
  "Back To Your Wallet": "Cüzdanınıza geri dönecek",
  "Product Has Been Resolved Successfully": "Ürün sorunu başarıyla çözüldü",
  "Cancel Return Request & Get": "İade talebini iptal et ve al",
  "Sorry, we couldn't load this product. It might have been removed or is temporarily unavailable.":
    "Üzgünüz, bu ürünü yükleyemedik. Kaldırılmış olabilir veya geçici olarak mevcut değil.",
  "Don't worry, you can try again or browse our other amazing products.":
    "Endişelenmeyin, tekrar deneyebilir veya diğer harika ürünlerimizi keşfedebilirsiniz.",
  "Back to Home": "Ana Sayfaya Dön",
  "Try Again": "Tekrar Dene",
  "Retrying...": "Yeniden deneniyor...",
  "You Can Return The Product Without Any Conditions According To The Return Policy And Get A Refund":
    "Ürünü iade koşulu olmadan iade edebilir ve tam ücret iadesi alabilirsiniz",
  "Add Photo": "Fotoğraf Ekle",
  "Add Story": "Hikaye Ekle",
  "Failed To Remove Image..Try Again":
    "سڕینەوەی وێنە سەرکەوتوو نەبوو.. دووبارە هەوڵ بدە",
  "Please Add Photos Of The Product You Received So That We Can Provide You With The Best Service To Avoid This Issue.":
    "Lütfen aldığınız ürünün fotoğraflarını ekleyin, böylece size en iyi hizmeti sunarak bu sorunun tekrar yaşanmasını önleyebiliriz.",

  "Failed to update like status. Please try again.":
    "Beğeni durumu güncellenemedi. Lütfen tekrar deneyin",
  "Cart Loading Error": "Sepet Yükleme Hatası",
  "Failed To Retrive Results Please Try Again":
    "Sonuçlar alınamadı, lütfen tekrar deneyin",
  "We encountered an error while loading your cart. Don't worry, we'll fix this issue for you.":
    "Sepetinizi yüklerken bir hata oluştu. Endişelenmeyin, bu sorunu sizin için çözeceğiz.",
  "View Product": "Ürünü Görüntüle",
  "Try Again": "Tekrar Dene",
  "More From": "Daha Fazlası",
  "Tap “Agree & Continue” To Accept trydos":
    "Trydos'u kabul etmek için “Kabul Et ve Devam Et”e dokunun",
  "Cancel Return Request": " İade talebini iptal et",
  "Return Request": "İade Talebi",
  "Initializing Return": "İade başlatılıyor",
  Size: "Beden",
  "You Need To Confirm Your Request": "Talebinizi onaylamanız gerekiyor",
  "Product Return Needs Confirm": "Ürün iadesi onay gerektiriyor",
  "return this product is not allowed": "Destpêkirina vegerandinê",
  "Failed To Load Return Details Try again":
    "İade detayları yüklenemedi, tekrar deneyin",
  "You are about to leave our site": "Sitemizden ayrılmak üzeresiniz",
  "External Link Warning": "Harici Bağlantı Uyarısı",
  "Notification Permission:": "Bildirim İzni:",
  "Preparing Your Experience": "Deneyiminiz hazırlanıyor",
  "this option dosent have enough quantity":
    "Bu seçenek yeterli miktara sahip değil.",
  "Story hidden successfully.": "Hikaye başarıyla gizlendi",
  "Failed to report story.": "Hikayeyi raporlama başarısız oldu",
  "Story reported successfully.": "Hikaye başarıyla raporlandı",
  "Report Story": "Hikayeyi Rapor Et",
  "Are you sure you want to report this story?":
    "Bu hikayeyi rapor etmek istediğinizden emin misiniz ?",
  "Reset Redeemed Products": "Kullanılmış Ürünleri Sıfırla",
  "Request Log": "İstek Kaydı",
  s: "s",
  "Make Stories Token Expired": "Hikayeler Tokenini Süresi Dolmuş Yap",
  "Make Chat Token Expired": "Sohbet Tokenini Süresi Dolmuş Yap",
  "I Read And Agree To": "Okudum ve kabul ediyorum",
  "Confirm the Changes First": "Önce değişiklikleri onaylayın",
  "Yes, I Agree": "Evet, kabul ediyorum",
  "We Will Ignore The First Size And Send Your Order With The New Size.":
    "İlk bedeni dikkate almayacağız ve siparişinizi yeni bedenle göndereceğiz.",
  "We Will Ignore The First Color And Send Your Order With The New Color.":
    "İlk rengi dikkate almayacağız ve siparişinizi yeni renkle göndereceğiz.",
  "You Can Add Up To 5 Photos Only": "En fazla 5 fotoğraf ekleyebilirsiniz",
  "Each Photo Must Be 5 MB Or Less": "Her fotoğraf en fazla 5 MB olmalıdır",
  "The Change Size Terms.": "Beden değişikliği koşulları.",
  "The Change Color Terms.": "Renk değişikliği koşulları.",
  "To New Size": "Yeni bedene",
  "To New Color": "Yeni renge",
  "Change Below Size": "Aşağıdaki bedeni değiştir",
  "Change Below Color": "Aşağıdaki rengi değiştir",
  "Chat with delivery worker": "Teslimat görevlisiyle sohbet et",
  "Take Photo": "Fotoğraf Çek",
  "Upload Photo/Video": "Fotoğraf/Video Yükle",
  "Failed To Load Filters Try Again":
    "Filtreler yüklenemedi, lütfen tekrar deneyin",
  "Failed To Load Products Retring in 3 seconds":
    "Ürünler yüklenemedi, 3 saniye sonra tekrar denenecek",
  "Failed To Load Products After 3 Attempts":
    "Ürünler yüklenemedi, 3 denemeden sonra tekrar denenecek",
  "No media selected": "Medya seçilmedi",
  "Add link...": "Bağlantı ekle...",
  "Rate & Get Money": "Puan Ver & Para Kazan",
  "Your Accurate Rating Of The Products You Receive Helps Us Improve Quality And Helps Other Customers Purchase.":
    "Aldığınız ürünlere yaptığınız doğru puanlama, kaliteyi iyileştirmemize ve diğer müşterilerin alışveriş yapmasına yardımcı olur.",
  "Premium Shopping": "Premium Alışveriş",
  Experience: "Deneyim",
  "Featured Products": "Öne Çıkan Ürünler",
  "Share Story": "Hikayeyi Paylaş",
  "You Have New Messages From Delivery Worker..click for more":
    "Teslimat Elemanından Yeni Mesajlarınız Var..daha fazlası için tıklayın",
  "Flash Deal": "Hızlı Fırsatlar",
  "Premium Brands": "Premium Markalar",
  "Show More": "Daha Fazla Göster",
  "TryDos - Premium Shopping Experience": "TryDos - Premium Alışveriş Deneyimi",
  "Discover premium products on TryDos":
    "TryDos'ta premium ürünleri keşfedin - Öne çıkan ürünler, hızlı fırsatlar ve butik koleksiyonlarla nihai alışveriş hedefiniz.",
  "Featured Products - TryDos":
    "Öne Çıkan Ürünler - TryDos | Premium Seçilmiş Koleksiyon",
  "Flash Deals - TryDos": "Hızlı Fırsatlar - TryDos | Sınırlı Süreli Teklifler",
  "Compare Products - TryDos":
    "Ürün Karşılaştırma - TryDos | Ürün Karşılaştırma Aracı",
  "Categories - TryDos": "Kategoriler - TryDos | Kategoriye Göre Gözat",
  "Settings - TryDos": "Ayarlar - TryDos | Hesap Ayarları",
  "Premium Curated Collection": "Premium Seçilmiş Koleksiyon",
  "Limited Time Offers": "Sınırlı Süreli Teklifler",
  "Product Comparison Tool": "Ürün Karşılaştırma Aracı",
  "Browse by Category": "Kategoriye Göre Gözat",
  "Account Settings": "Hesap Ayarları",
  "Your ultimate shopping destination": "Nihai alışveriş hedefiniz",
  "Curated selection of premium featured products":
    "Premium öne çıkan ürünlerin seçilmiş koleksiyonu",
  "Limited-time offers with great discounts":
    "Harika indirimlerle sınırlı süreli teklifler",
  "Find and compare your favorite products":
    "Favori ürünlerinizi bulun ve karşılaştırın",
  "Browse products by categories": "Ürünleri kategorilere göre gözatın",
  "Manage your account preferences": "Hesap tercihlerinizi yönetin",
  shopping: "alışveriş",
  "online shopping": "online alışveriş",
  "premium products": "premium ürünler",
  "best deals": "en iyi fırsatlar",
  "shopping mall": "alışveriş merkezi",
  boutiques: "butikler",
  brands: "markalar",
  fashion: "moda",
  electronics: "elektronik",
  "home garden": "ev bahçe",
  "Syncing contacts...": "İletişim bilgileriniz eşitleniyor...",
  "Please select a reason for canceling this order":
    "Lütfen bu siparişi iptal etmek için bir neden seçin",
  "Please Review Your Cart Some Products Not Available":
    "Lütfen Sepetinizi İnceleyin Bazı Ürünler Mevcut Değil",
  "Please Verify Your Phone Number And Try Again":
    "Lütfen Telefon Numaranızı Doğrulayın ve Tekrar Deneyin",
  "Please verify your phone number to continue":
    "Devam etmek için lütfen telefon numaranızı doğrulayın",
  "Close": "Kapat",
  "You Can Change Your Shipping Address And Delivery Notes":
    "Teslimat Adresinizi ve Teslimat Notlarınızı Kolayca Değiştirebilirsiniz",
  "Shipping Cost": "Kargo Ücreti",
  "Failed to sync contacts": "İletişim bilgileriniz eşitlenemedi",
  "Modify Order": "Siparişi Düzenle",
  "Manage Your Order": "Siparişinizi Yönetin",
  "Change Delivery Address & Note": "Teslimat Adresini ve Notu Değiştirin",
  "Cancel Order": "Siparişi İptal Et",
  "You Can Cancel & Back Your Money":
    "Siparişi İptal Edip Para İade Edilebilir",
  "We Have Other Solutions Instead Of Cancellation.":
    "Diğer Çözümlerimiz İptal Etmekten Farklıdır.",
  "Learn More Tips.": "Daha Fazla İpucu Öğrenin",
  "Why Was The Order Cancelled?": "Sipariş Neden İptal Edildi?",
  "I Changed My Mind": "Siparişi İptal Etmek İstemedim",
  "I Fear Quality": "Ürünün Kalitesi İptal Etmek İstemedim",
  "I Fear The Delivery Time": "Teslimat Süresi İptal Etmek İstemedim",
  "I'm Afraid Of Sizes": "Boyutlar İptal Etmek İstemedim",
  "I Saw A Better Price": "Daha Uygun Fiyat İptal Etmek İstemedim",
  "Information About Your Order Modify Or Cancel":
    "Siparişinizin Düzenlenmesi veya İptal Edilmesi Hakkında Bilgiler",
  "Add Contacts": "İletişim bilgilerini ekle",
  "Get from your contacts": "Kişilerinizden alın",
  "Enter name": "Adı ve Soyadı",
  "Enter phone number": "Telefon Numarası",
  "Contacts API not supported in this browser":
    "Bu tarayıcıda API kişileri desteklenmiyor",
  Invite: "Davet",
  "The Purchase Was Completed Successfully": "Satın Alma Başarıyla Tamamlandı",
  "Please Select an Address": "Lütfen Bir Adres Seçiniz",
  Reschedule: "Yeniden Planla",
  "Session Expired..please Login again..Reloading...":
    "Oturum Süresi Doldu..Lütfen Tekrar Giriş Yapın",
  "Please Be Accurate and select your Location":
    "Lütfen doğru ve konumunuzu seçin",
  "Your Current Location is Not belong to Country Bounds":
    "Mevcut Konumunuz Seçilen Ülke Sınırları İçinde Değil",
  "Request submitted successfully": "Talebiniz başarıyla gönderildi",
  "Request already submitted": "Talep zaten gönderildi",
  "You have already submitted a seller request. We are processing it and will get back to you soon.":
    "Zaten bir satıcı talebi gönderdiniz. Talebinizi işleme alıyoruz ve en kısa sürede size geri döneceğiz.",
  "Please fill in all fields": "Lütfen tüm alanları doldurun",
  "Personal Details": "Kişisel Bilgiler",
  "First Name": "Ad",
  "Last Name": "Soyad",
  "Repeat Password": "Parolayı Tekrarla",
  "Passwords do not match": "Şifreler eşleşmiyor",
  "Enter a valid email address": "Geçerli bir e-posta adresi girin",
  "Password is required": "Şifre gereklidir",
  "Min 8 characters, including a letter and a number":
    "En az 8 karakter, bir harf ve bir rakam içermeli",
  "Please repeat your password": "Lütfen şifrenizi tekrar girin",
  "Shop Information": "Mağaza Bilgileri",
  "Shop Address": "Mağaza Adresi",
  "Location Details": "Konum Bilgileri",
  "Location Name": "Konum Adı",
  "Location Address": "Konum Adresi",
  Submit: "Gönder",
  Documents: "Belgeler",
  Remove: "Kaldır",
  "No documents uploaded": "Herhangi bir belge yüklenmedi",
  "Document Type": "Belge Türü",
  "Choose File": "Dosya Seç",
  "e.g., Trade License": "Örn., Ticaret Ruhsatı",
  "Upload Document": "Belge Yükle",
  Uploading: "Yükleniyor",
  "Please select a file": "Lütfen bir dosya seçin",
  "Please upload at least one document": "Lütfen en az bir belge yükleyin",
  "000 CM": "000 CM",
  "Failed to recognize your voice": "Sesinizi tanımlamada sorun oluştu",
  "Discount Coupon": "İndirim Kuponu",
  "Wallet Already Selected": "Cüzdan Zaten Seçildi",
  "You will be notified for this product already":
    "Bu Ürün Hakkında Bildirim Alındı",
  "your TryDos Wallet balance is empty":
    "TryDos Cüzdanınızda bakiye bulunmamaktadır",
  "Pick Your Deleivery Location Inside Your Country":
    "Ülkeniz İçindeki Teslimat Konumunuzu Seçin",
  "Item Status": "Ürün Durumu",
  "Composed Of": "İçerik",
  Recipient: "Alıcı",
  "Order Number has been Copied": "Sipariş Numarası Kopyalandı",
  "Recipient Phone": "Alıcı Telefon Numarası",
  "Loading...": "Yükleniyor...",
  Buying: "Satın Alındı",
  Pieces: "Adet",
  "No orders found for this status.": "Bu Duruma Ait Sipariş Bulunamadı",
  "Order Not Found": "Sipariş Bulunamadı",
  "This order doesn't exist or the link is incorrect.":
    "Bu sipariş mevcut değil veya bağlantı hatalı.",
  "Go to My Orders": "Siparişlerime Git",
  "No more orders": "Daha fazla sipariş yok",
  "Orders Details": "Sipariş Detayları",
  "Expected Delivery Date": "Teslimat Tarihi",
  "Order Status": "Sipariş Durumu",
  "Work Days": "Çalışma Günleri",
  "Order Invoice": "Sipariş Faturası",
  "Order Number": "Sipariş Numarası",
  "Your Size Info": "Boyut Bilgileriniz",
  "Height is required": "Boy zorunludur",
  "Weight is required": "Kilo zorunludur",
  "Height must be between 110 and 250 cm":
    "Boy 110 ile 250 cm arasında olmalıdır",
  "Weight must be between 40 and 180 kg":
    "Kilo 40 ile 180 kg arasında olmalıdır",
  "Profile | Countries": "Profil | Ülkeler",
  "Available Countries": "Mevcut Ülkeler",
  "Your Address Info": "Adres Bilgileriniz",
  "What Is Your Weight?": "Kilonuz kaç?",
  "000 KG": "000 KG",
  "How Tall Are You?": "Boyunuz kaç?",
  "Enter Email": "E-posta Girin",
  "Profile | Size Info": "Profil | Boyut Bilgileri",
  "Profile | Address Info": "Profil | Adres Bilgileri",
  "Profile | Bank Cards": "Profil | Banka Kartları",
  "Your Bank Cards Info": "Banka Kartları Bilgileriniz",
  All: "Tümü",
  Pending: "Beklemede",
  Processing: "İşleniyor",
  "Ready to Shipping": "Kargo Hazırlanıyor",
  Shipped: "Kargo Gönderildi",
  "Out for Delivery": "Kargo Teslim Ediliyor",
  Delivered: "Kargo Teslim Edildi",
  "Partial Return": "Kısmi İade",
  Returned: "İade Edildi",
  Failed: "Başarısız",
  Canceled: "İptal Edildi",
  "Canceled Archived": "İptal Edildi ve Arşivlendi",
  Man: "Erkek",
  Woman: "Kadın",
  Other: "Diğer",
  Gender: "Cinsiyet",
  Email: "E-posta",
  "Enter Alternative Phone": "Alternatif Telefon Numarasını Girin",
  Phone: "Telefon",
  "Enter Full Name": "Tam Adınızı Girin",
  "Enter Phone": "Telefon Numaranızı Girin",
  "Full Name": "Tam Adınız",
  "Change Photo": "Fotoğrafı Değiştir",
  "Profile | Address": "Profil | Adres",
  "Profile | Languages": "Profil | Dil",
  "Profile | Personal Info": "Profil | Kişisel Bilgiler",
  "Choose From Library": "Dosyadan Seç",
  "take photo": "Fotoğraf Çek",
  "About Us": "Hakkımızda",
  "Bank Cards": "Banka Kartları",
  "Personal Info": "Kişisel Bilgiler",
  "Add Profile Picture": "Profil Fotoğrafı Ekle",
  "Uploading Profile Photo …": "Profil Fotoğrafı Yükleniyor...",
  "Change Profile Photo": "Profil Fotoğrafını Değiştir",
  Address: "Adres",
  Action: "İşlem",
  "Quantity Running Out. ": "Miktar Bitiyor. ",
  "Time Running Out. ": "Zaman Bitiyor. ",
  "Share App": "Uygulamayı Paylaş",
  "Legal Information": "Hukukî Bilgiler",
  "Add Size": "Ekle",
  "Terms & Conditions": "Şartlar ve Koşullar",
  Verified: "Doğrulandı",
  "Verify Now": "Doğrula",
  "Only Allowed To Pay through TryDos Wallet":
    "Bu Ürünü Satın Almak İçin Sadece TryDos Cüzdanına Ödeme Yapılabilir",
  "Product Not available": "Bu Ürün Mevcut Değildir",
  "Product Restricted in This Country": "Bu Ürün Bu Ülkeye Uygun Değildir",
  "Already Added To Compare!": "Zaten Karşılaştırmaya Eklendi!",
  "Added To Compare": "Karşılaştırmaya Eklendi",
  "Added To Compare! Click To Go To Compare Page":
    "Karşılaştırmaya Eklendi! Karşılaştırma Sayfasına Gitmek İçin Tıklayın",
  Details: "Detaylar",
  "One of the products was not found. Please try searching for a different product.":
    "Bir veya daha fazla ürün bulunamadı. Lütfen farklı bir ürün arayın.",
  "Confirm Shipping & Payment": "Siparişi Onayla ve Ödeme Yap",
  "Bag Shipping & Delivery Address": "Kargo ve Teslimat Adresi",
  "Your Order Number": "Sipariş Numaranız",
  "You Can Track The Status Of Your Order Through ":
    "Siparişinizin Durumunu Takip Edin",
  "Are you sure you want to delete this comment?":
    "Bu yorumu silmek istediğinizden emin misiniz?",
  "Delete Comment": "Yorumu Sil",
  Cancel: "İptal",
  Done: "Tamamlandı",
  "My Account / My Orders": "Hesabım / Siparişlerim",
  Select: "Seç",
  "Shipping & Payment": "Kargo ve Ödeme",
  "Write The Address Clearly, Including The Street Address, Building, Flat, Door, Unit.":
    "Adresi Açıkça Yazın, Sokak Adresini, Bina, Daire, Kapı, Birimi İçerecektir.",
  "Address Info": "Adres Bilgileri",
  and: "ve",
  "I read and agree to the": "Okudum ve kabul ediyorum",
  policies: "politikalar",
  terms: "şartlar",
  "Click To Show All Discount": "Tüm İndirimleri Görüntüle",
  "Total Discount": "Toplam İndirim",
  Item: "Ürün",
  "Your Address List": "Adres Listesi",
  "Delete Below Address?": "Aşağıdaki Adresi Sil?",
  "Yes Delete": "Evet, Sil",
  Price: "Fiyat",
  "Normal Price": "Normal Fiyat",
  "Please Enter Coupon Information": "Kupon Bilgilerini Girin",
  "Change From List": "Listeden Değiştir",
  "Contact Info": "İletişim Bilgileri",
  "Total Items": "Toplam Ürün",
  "Recipient Name": "Alıcı Adı",
  "Add & Save": "Ekle ve Kaydet",
  "Edit & Save": "Düzenle ve Kaydet",
  "Locate Your Location On Map": "Konumunuzu Haritada Bulun",
  "Alternative Phone Number": "Alternatif Telefon Numarası",
  "(Optional)": "(İsteğe Bağlı)",
  "Enter Recipient Phone Number": "Alıcı Telefon Numarasını Girin",
  "Enter Alternative Recipient Phone Number":
    "Alternatif Alıcı Telefon Numarasını Girin",
  "Address Title": "Adres Başlığı",
  "Detailed Address & Note": "Detaylı Adres ve Not",
  "Entering The Information Below Clearly And Completely Will Ensure That Your Order Arrives Without Problems And Faster.":
    "Aşağıdaki Bilgileri Açıkça ve Tamamen Girerek, Siparişinizin Sorun Yoksa ve Hızlıca İletilmesini Sağlayacaktır.",
  "Applying...": "Uygulanıyor...",
  "Country | Region": "Ülke | Bölge",
  "Location Is Accurate, Making It Easy To Receive Shipments":
    "Konum Doğru, Teslimat Almak Çok Kolay",
  Apply: "Tamam",
  "You Can Also Create Multiple Addresses To Use":
    "Ayrıca Birden Çok Adres Oluşturabilirsiniz",
  "Payment Method": "Ödeme Yöntemi",
  "I Have a Discount Coupon": "Bir İndirim Kodumu Var",
  "Please Choose Your Payment Method About Your Bag":
    "Lütfen Çantalarınız İçin Ödeme Yönteminizi Seçin",
  "Add New Shipping Address": "Kargo Adresi Ekle",
  Delivery: "Kargo",
  "Earn Money With This Order": "Bu Siparişle Para Kazan",
  "Purshase Protection": "Satın Alma Koruma",
  "Safe & Easy Payment": "Güvenli ve Kolay Ödeme",
  "More Options": "Daha Fazla Seçenek",
  "Notify Me About The Product When": "Bu Ürün Hakkında Bildirim Al",
  "Add To My Checklist": "Bu Ürünü Listeme Ekle",
  "My Checklist": "Listem",
  "Add To Compare": "Bu Ürünü Karşılaştırmaya Ekle",
  "Before Stock Out": "Stok Bitmeden Önce",
  "Change In Price": "Fiyat Değişti",
  "Follow Comments": "Yorumları Takip Et",
  Discounts: "İndirim",
  Offers: "Teklifler",
  "Share This Product With": "Bu Ürünü Paylaş",
  "Notify Me When Product Is Available": "Bildirim Al, Boyut Mevcut Olduğunda",
  "Agree & Change": "Kabul Et ve Değiştir",
  "enter number of pieces you want to return":
    "İade etmek istediğiniz parça sayısını girin",
  Recommended: "Önerilen",
  "Comment About This Product": "Bu Ürün Hakkında Yorum",
  "We Will Inform You When this Is Available":
    "Bu Ürün Mevcut Olduğunda Size Bildireceğiz",
  "change address request": "Adres değiştirme talebi",
  "Change Below Address": "Aşağıdaki adresi değiştir",
  "To New Address": "Yeni adrese",
  "The Change Addres Terms.": "Adres değiştirme şartları.",
  "We Will Ignore The First Address And Send Your Order To The New Address.":
    "İlk adresi yok sayacağız ve siparişinizi yeni adrese göndereceğiz.",
  "For You": "Sana",
  Last: "Son",
  "Not Available Now, Stock Is Sold Out": "Şu an mevcut değil, stok satıldı",
  "Need Help Finding Your Size?": "Size Bulma Yardımı İstiyor musunuz?",
  "Within 3 Days After Receiving The Product, You Can Return It Without Conditions Or Reasons With Complete Ease And Get The Amount Back":
    "Ürünü aldıktan 3 gün içinde, herhangi bir şey söylemeden veya sebeplerden dolayı iade edebilirsiniz ve para iadesi alabilirsiniz",
  "Search History": "Arama Geçmişi",
  "Popular Search": "Popüler Arama",
  "Total Products:": "Ürünlerin Toplamı:",
  "No Results Found": "Sonuç bulunamadı",
  Reset: "Sıfırla",
  "Clear All": "Tümünü Temizle",
  pending: "Bekliyor",
  "Find Products": "Ürünleri Bul",
  "Find Brands": "Markaları Bul",
  "Find Categories": "Kategorileri Bul",
  "Find Offers": "Teklifleri Bul",
  "Find Boutiques": "Boutikleri Bul",
  "Cart is Empty": "Sepetiniz Boş",
  ready_to_shipping: "Hazırlanıyor",
  out_for_delivery: "Kargoda",
  delivered: "Teslim Edildi",
  partial_return: "Kısmen İade Edildi",
  returned: "Tamamen İade Edildi",
  canceled_archived: "İptal Edildi",
  failed: "İptal Edildi",
  paid: "Ödendi",
  unpaid: "Ödenmedi",
  Buy: "Satın Al",
  "Confirm & Continue": "Onayla ve Devam Et",
  Saved: "Kaydedildi",
  Shipping: "Kargo",
  "Please log in first": "Lütfen önce giriş yapın",
  "Load More": "Daha Fazla Yükle",
  "Shipping Address": "Kargo Adresi",
  "Back To HomePage": "Ana Sayfaya Geri Dön",
  "Composed Of:": "İçerisinde:",
  items: "ürün",
  "Hurry Up!": "Hızlıca Alın!",
  "Quantity Running Out.": "Miktar Bitiyor.",
  "Hurry Up! Quantity Running Out.": "Hızlıca Alın! Miktar Bitiyor.",
  "Return Guarantee": "İade Garantisi",
  within: "içinde",
  Days: "Gün",
  "Delivery Guarantee": "Teslimat Garantisi",
  "Secure Privacy": "Güvenli Gizlilik",
  "Safe Payment": "Güvenli Ödeme",
  "Secure Payment": "Güvenli Ödeme",
  Total: "Toplam",
  Order: "Sipariş",
  "Order Details": "Sipariş Detayları",
  "Order ID": "Sipariş ID",
  "Order Date": "Sipariş Tarihi",
  "Place Order": "Sipariş Ver",
  "Order Placed Successfully": "Sipariş Başarıyla Verildi",
  "Order Placed Failed": "Sipariş Verilirken Hata Oluştu",
  "Order Placed Failed Please Try Again":
    "Sipariş Verilirken Hata Oluştu, Lütfen Tekrar Deneyin",
  "Order Placed Successfully Please Wait For Confirmation":
    "Sipariş Başarıyla Verildi, Lütfen Bekleyiniz",
  Save: "Kaydet",
  Country: "Ülke",
  name: "İsim",
  phone: "Telefon",
  email: "E-posta",
  Profile: "Profil",
  Notifications: "Bildirimler",
  "Click to view details": "Detayları Görüntüle",
  Language: "Dil",
  Orders: "Siparişler",
  Settings: "Ayarlar",
  Logout: "Çıkış Yap",
  "Shipping & Delivery Address": "Teslimat ve Gönderim Adresi",
  "No Address Selected": "Adres Listeniz Boş",
  "Expected Delivery": "Tahmini Teslimat",
  "Delivery Note": "Teslimat Notu",
  "Show Address List": "Adres Listesini Göster",
  "Please Enter Shipping Address To Receive Your Bag":
    "Çantanızı Alabilmeniz İçin Lütfen Teslimat Adresinizi Girin",
  "Shipment Will Be Sent To The Address Below":
    "Gönderi Aşağıdaki Adrese Gönderilecektir",
  "Your Shopping Bag": "Alışveriş Çantanız",
  "Shopping Bag": "Alışveriş Çantası",
  "Name Should be atleast 8 characters": "İsim En Az 8 Karakter Olmalıdır",
  Items: "Ürünler",
  "stock is limited": "stok sınırlıdır",
  "Please verify the number first.": "Lütfen önce numarayı doğrulayın.",
  "your session has expired please login again or continue as guest":
    "oturumunuz sona erdi, lütfen tekrar giriş yapın veya misafir olarak devam edin",
  "Your session has expired": "Oturumunuz sona erdi",
  "Please login again to get back to your account, or continue browsing as a guest.":
    "Hesabınıza geri dönmek için lütfen tekrar giriş yapın veya misafir olarak gezinmeye devam edin.",
  "Continue as Guest": "Misafir Olarak Devam Et",
  "Enter Your Phone Number To Complete Order":
    "Siparişi Tamamlamak İçin Telefon Numaranızı Girin",
  "Please Select The Appropriate": "Lütfen Uygun Olanı Seçin",
  Size: "Beden",
  "Add To Bag": "Çantaya Ekle",
  "Free Shipping": "Ücretsiz Kargo",
  "Shipping Is Completely Free Without Any Extras":
    "Kargo Tamamen Ücretsiz, Ekstra Ücret Yok",
  "Reach End": "Sonuca Ulaş",
  "You've reached the end": "Sona ulaştınız",
  "You've seen everything in this list.": "Bu listedeki her şeyi gördünüz.",
  "No products found": "Ürün bulunamadı",
  "Try changing or clearing your filters.": "Filtrelerinizi değiştirmeyi veya temizlemeyi deneyin.",
  "Clear filters": "Filtreleri temizle",
  "Good Quality Product": "İyi Kaliteli Ürün",
  "Verified by trydos": "trydos tarafından doğrulandı",
  "Made In Turkey": "Türkiye'de Üretilmiştir",
  "Made In {country}": "{country} Üretimi",
  "You May Also Like": "Bunları da Beğenebilirsin",
  "Read More": "Daha Fazla Oku",
  "Read Less": "Daha Az Oku",
  "All Inclusive Without Additions": "Her Şey Dahil Ekstra Yok",
  "Ship To You Accepted ": "Size Gönderim Kabul Edildi",
  "Buyers Camera": "Alıcı Kamerası",
  "Product Story": "Ürün Hikayesi",
  "Free Return": "Ücretsiz İade",
  "Product Shipping & Delivery": "Ürün Gönderimi ve Teslimatı",
  Days: "Gün",
  "Fast Packing & Start Shipping": "Hızlı Paketleme & Gönderime Başlama",
  "Expected Within": "İçerisinde Beklenen",
  Lebanon: "Lübnan",
  "At Your Address In": "Adresinizde",
  "Same Day Packing & Ship If Buy Before":
    "Aynı Gün Paketleme & Gönderim, Eğer Satın Alırsanız",
  Today: "Bugün",
  "You Will Get A": "Bir Alacaksınız",
  "Time Is Expected, It May Take More Or Less Than 2 Days":
    "Beklenen Süre, 2 Günden Az veya Fazla Sürebilir",
  Shot: "Çekim",
  Refund: "İade",
  brands: "Markalar",
  colors: "Renkler",
  sizes: "Bedenler",
  "Confirm Country Change": "Ülke değişikliğini onayla",
  "Are you sure you want to change your country?":
    "Ülkenizi değiştirmek istediğinizden emin misiniz?",
  Current: "Mevcut",
  New: "Yeni",
  "Specify Your Address To Calculate The Delivery Time":
    "Teslimat Süresini Hesaplamak İçin Adresinizi Belirtin",
  "In Your Address": "Adresinizde",
  "Available ": "Mevcut",
  "Of The Product Price If Shipping Is Delayed":
    "Ürün Fiyatından Kargo Gecikirse",
  Color: "Renk",
  "in one word what is in this photo?": "Bu fotoğrafta bir kelimeyle ne var?",
  Brands: "Markalar",
  "this message was deleted": "bu mesaj silindi",
  "No Answer": "Cevap Yok",
  "Initialize Call please wait..": "Arama Başlatılıyor, lütfen bekleyin..",
  "Call Answered from another account":
    "Arama başka bir hesap tarafından cevaplandı",
  "Calling ...": "Aranıyor...",
  "Best Offers From Brands": "Markalardan En İyi Teklifler",
  "failed to send otp code please try again":
    "otp kodu gönderilemedi, lütfen tekrar deneyin",
  Offer: "Teklif",
  Sale: "Satış",
  "user not found": "kullanıcı bulunamadı",
  More: "Daha Fazla",
  Second: "Saniye",
  "This Offer Is For Only": "Bu Teklif Sadece İçin",
  Hours: "Saatler",
  Remaining: "Kalan",
  "Buy 1 gift 1": "1 Al, 1 Hediye",
  "Mango Famous Turkish Brand Best Discounts":
    "Mango Ünlü Türk Markası En İyi İndirimler",
  "Mango Famous Turkish Brand Best Offers":
    "Mango Ünlü Türk Markası En İyi Teklifler",
  "Quick Offer": "Hızlı Teklif",
  "Products With Great Fast And Limited Offers":
    "Harika Hızlı ve Sınırlı Tekliflere Sahip Ürünler",
  "Products With Great Offers": "Harika Tekliflere Sahip Ürünler",
  Category: "Kategori",
  "Enjoy Shopping From All Categories & Products From Various Brands":
    "Tüm Kategorilerden ve Çeşitli Markalardan Alışverişin Tadını Çıkarın",
  Kids: "Çocuk",
  Home: "Ev",
  Electrical: "Elektrikli",
  Shop: "Mağaza",
  Welcome: "Hoşgeldiniz",
  Hello: "Merhaba",
  "Why We Know You ?": "Sizi Neden Tanıyoruz?",
  "Can We Know You ?": "Sizi Tanıyabilir Miyiz?",
  Login: "Giriş Yap",
  Search: "Ara",
  "By Mobile Phone Number": "Mobil Telefon Numarasıyla",
  "By Scan Qr From Trydos App": "Trydos Uygulamasından QR Taratarak",
  "Scan This Qr Code From You Trydos App In Your Phone":
    "Telefonunuzdaki Trydos Uygulamasından Bu QR Kodu Taratın",
  "Enter Your Phone Number Registered With Us":
    "Bizimle Kayıtlı Telefon Numaranızı Girin",
  "Don't Have Account?": "Hesabınız Yok Mu?",
  "Create New Account": "Yeni Hesap Oluştur",
  "Choose The Verification Method": "Doğrulama Yöntemini Seçin",
  "Send Verification Code To": "Doğrulama Kodunu Gönder",
  SMS: "SMS",
  WhatsApp: "WhatsApp",
  "Please Enter The Verification Code Sent To Your Phone":
    "Telefonunuza Gönderilen Doğrulama Kodunu Girin",
  "You Can Resend The Code After":
    "Kodu Şu Süre Sonra Yeniden Gönderebilirsiniz",
  "Resend Code": "Kodu Yeniden Gönder",
  "Sorry, This Number Is Not Registered With Us !":
    "Üzgünüz, Bu Numara Bizimle Kayıtlı Değil",
  "Register With Us In A Few Simple Steps":
    "Birkaç Basit Adımla Bizimle Kayıt Olun",
  "Please Enter The Correct Code Sent To Your Phone":
    "Telefonunuza Gönderilen Doğru Kodu Girin",
  "You Have": "Sizin Var",
  Attempts: "Denemeler",
  "Change The Method Of Receiving": "Alım Yöntemini Değiştir",
  Change: "Değiştir",
  "Search, Chat, Contact, Start New Chat":
    "Ara, Sohbet Et, İletişime Geç, Yeni Sohbet Başlat",
  "Contacts List": "İletişim Listesi",
  Copy: "Kopyala",
  OK: "Tamam",
  Reply: "Yanıtla",
  Forward: "İlet",
  CategoryMessage: "Kategori",
  Reminder: "Hatırlatma",
  Edit: "Düzenle",
  Delete: "Sil",
  Archive: "Arşivle",
  Unmute: "Ses Aç",
  Mute: "Sessize Al",
  Pin: "Sabitle",
  Unpin: "Sabitlemeden Kaldır",
  Read: "Okundu",
  Unread: "Okunmamış",
  Sunday: "Pazar",
  Monday: "Pazartesi",
  Tuesday: "Salı",
  Wednesday: "Çarşamba",
  Thursday: "Perşembe",
  Friday: "Cuma",
  Saturday: "Cumartesi",
  Yesterday: "Dün",
  Image: "Görsel",
  Video: "Video",
  File: "Dosya",
  Audio: "Ses",
  "Incoming Voice Call..": "Gelen Sesli Arama..",
  "Incoming Video Call..": "Gelen Görüntülü Arama..",
  "Voice Call": "Sesli Arama",
  "Video Call": "Görüntülü Arama",
  Calling: "Aranıyor...",
  "End Call": "Aramayı Sonlandır",
  "User declined": "Kullanıcı reddetti",
  "Missed Call": "Gözden Kaçırılan Arama",
  "Incoming Call": "Gelen Arama",
  "Outgoing Call": "Giden Arama",
  "Please Enable Notification to use Chat":
    "Sohbeti Kullanmak İçin Lütfen Bildirimleri Etkinleştirin",
  "No Contacts": "İletişim Yok",
  "Log in Through our App to access contacts":
    "İletişimlere Erişmek İçin Uygulamamızla Giriş Yapın",
  "Cancel & Take A Look At The Site": "İptal Et ve Uygulamaya Göz At",
  "Create New Account & Continue": "Yeni Hesap Oluştur & Devam Et",
  "Register & Create New Account With Us In A Few Simple Steps":
    "Birkaç Basit Adımla Bizimle Kayıt Olun ve Yeni Hesap Oluşturun",
  "Login & Continue": "Giriş Yap ve Devam Et",
  "You Can Log In Now": "Şimdi Giriş Yapabilirsiniz",
  "This Number Already Registered With Us !":
    "Bu Numara Zaten Bizimle Kayıtlı!",
  "Enter Your Name": "Adınızı Girin",
  "The Number Verified Successfully !": "Numara Başarıyla Doğrulandı!",
  "Last Step And Enjoy Our Services":
    "Son Adım ve Hizmetlerimizin Tadını Çıkarın",
  "Your Privacy Is Completely Safe, We Not Share Your Information With Anyone":
    "Gizliliğiniz Tamamen Güvende, Bilgilerinizi Hiçbir Kimseyle Paylaşmayız",
  "We Will Send A Verification Code To The Number":
    "Numaraya Doğrulama Kodu Göndereceğiz",
  "Please Enter The Verification Code Sent To Your Whatsapp":
    "Whatsapp'a Gönderilen Doğrulama Kodunu Girin",
  "Didn't You Receive A Code?": "Kod Almadınız mı?",
  "the Method Of Receiving": "Alım Yöntemi",
  "The Code Sent Has Expired": "Gönderilen Kodun Süresi Dolmuş",
  "Later, Take A Look At The Site": "Sonra, Uygulamaya Göz At",
  "Invalid Phone Number": "Geçersiz Telefon Numarası",
  "Enter Your Phone Number To Login":
    "Giriş Yapmak İçin Telefon Numaranızı Girin",
  "To ": "Şuraya",
  "Create New Account ": "Yeni Hesap Oluştur",
  'Tap "Agree & Continue" To Accept trydos':
    'trydos\'u Kabul Etmek İçin "Kabul Et ve Devam Et"e Dokunun',
  "Terms Of Services": "Hizmet Şartları",
  "Agree & Continue": "Kabul Et ve Devam Et",
  "Choose The Verification Method, Receive Code Via:":
    "Doğrulama Yöntemini Seçin, Kodu Şu Yöntemle Alın:",
  "Complete My Profile": "Profilimi Tamamla",
  "Skip For Now": "Şimdi Atla",
  "We Recommend That You Complete Your Profile To Make The Most Of The App's Features, Such As Shopping, Chatting, Stories, Taking Advantage Of Offers, Interests, And Much More":
    "Uygulamanın Özelliklerinden En İyi Şekilde Yararlanmak İçin Profilinizi Tamamlamanızı Tavsiye Ederiz. Alışveriş, Sohbet, Hikayeler, Tekliflerden Yararlanma, İlgi Alanları ve Daha Fazlası İçin.",
  "Enjoy With Our Services": "Hizmetlerimizin Tadını Çıkarın",
  "Logged In Successfully !": "Başarıyla Giriş Yapıldı!",
  "Hello,": "Merhaba,",
  "To Take Advantage Of All The Advantages Of The Application, Please Join Us In Quick And Easy Steps And For Just One Time":
    "Uygulamanın Tüm Avantajlarından Yararlanmak İçin, Lütfen Hızlı ve Kolay Adımlarla Sadece Bir Defa Bize Katılın",
  "I have Already Account": "Zaten Hesabım Var",
  "Terms Of Services ": "Hizmet Şartları",
  "Notifications Settings": "Bildirim Ayarları",
  "Notification Test": "Bildirim Testi",
  "Enabled Notifications Topic:": "Etkin Bildirim Konuları:",
  "Disabled Notifications Topic:": "Devre Dışı Bildirim Konuları:",
  "Unsubscribing...": "Abonelikten Çıkılıyor...",
  Unsubscribe: "Abonelikten Çık",
  "Loading Topics...": "Konular Yükleniyor...",
  "No topics subscribed.": "Abone olunan konu yok.",
  "Subscribing...": "Abone Olunuyor...",
  Subscribe: "Abone Ol",
  "notifications subscription:": "bildirim aboneliği:",
  "Enable Email Notifications": "E-posta Bildirimlerini Etkinleştir",
  "Enable FireBase Notifications": "FireBase Bildirimlerini Etkinleştir",
  "Enable WhatsApp Notifications": "WhatsApp Bildirimlerini Etkinleştir",
  "notifications Receiving Preference:": "bildirim alma tercihi:",
  "Select An Option": "Bir Seçenek Seçin",
  daily: "günlük",
  weekly: "haftalık",
  monthly: "aylık",
  "Continue with": "ile devam et",
  OR: "VEYA",
  "Select Your Country": "Ülkenizi Seçin",
  uk: "İngiltere",
  uae: "BAE",
  "Name is required": "İsim gereklidir",
  "Name must be at least 2 characters": "İsim en az 2 karakter olmalıdır",
  "Name must be less than 50 characters": "İsim 50 karakterden az olmalıdır",
  "Must not exceed 10 characters": "10 karakteri geçmemelidir",
  "Email is required": "E-posta gereklidir",
  "Please enter a valid email address":
    "Lütfen geçerli bir e-posta adresi girin",
  "Phone number is required": "Telefon numarası gereklidir",
  "Please enter a valid phone number":
    "Lütfen geçerli bir telefon numarası girin",
  "Upload Photo": "Fotoğraf Yükle",
  "Remove Photo": "Fotoğrafı Kaldır",
  "From Files": "Dosyalardan",
  from: "itibaren",
  "From Camera": "Kameradan",
  "JPG, PNG or GIF (max. 5MB)": "JPG, PNG veya GIF (maks. 5MB)",
  CheckList: "İstek Listesi",
  "Your CheckList is empty": "İstek listeniz boş",
  "Compare Products": "Ürünleri Karşılaştır",
  Compare: "Karşılaştır",
  "type a comment": "Yorum yaz",
  "Search for Product 1": "1. Ürünü Ara",
  "Search for Product 2": "2. Ürünü Ara",
  Name: "İsim",
  Colors: "Renkler",
  Sizes: "Bedenler",
  "Offer Price": "Teklif Fiyatı",
  "Product Comparison": "Ürün Karşılaştırma",
  "Loading your selected products for a detailed side-by-side comparison. This will help you make an informed decision by comparing features, prices, and specifications.":
    "Seçtiğiniz ürünler detaylı bir yan yana karşılaştırma için yükleniyor. Bu, özellikleri, fiyatları ve teknik özellikleri karşılaştırarak bilinçli bir karar vermenize yardımcı olacaktır.",
  "Filter By brands": "Markalara göre filtrele",
  "Filter By categories": "Kategorilere göre filtrele",
  "Filter By colors": "Renklere göre filtrele",
  "Filter By sizes": "Bedenlere göre filtrele",
  "Filter By boutiques": "Mağazalara göre filtrele",
  "Filter By Prices": "Fiyatlara göre filtrele",
  "Filter By search_text": "Filtrele göre",
  "Filter By prices": "Fiyatlara göre filtrele",
  Preparing: "Hazırlanıyor",
  "Action About Your Order": "Siparişiniz Hakkında İşlemler",
  "Change Delivery Note": "Teslimat Notunu Değiştir",
  "Add delivery instructions...": "Teslimat talimatlarını ekle...",
  "Action About This Product": "Bu Ürün Hakkında İşlemler",
  "And Back Your Money": "Ve Paranızı İade Edin",
  Hours: "Saatler",
  "Return This Product": "Bu Ürünü İade Et",
  "Change Size, Color, Other": "Beden, Renk ve Diğerini Değiştir",
  "Change Product Request": "Ürün Değişim Talebi",
  "You Can Change Delivery Address": "Teslimat Adresini Değiştirebilirsiniz",
  "Delivery Address & Note": "Teslimat Adresi ve Not",
  "Delivery Address": "Teslimat Adresi",
  "Delivery Note": "Teslimat Notu",
  Close: "Kapat",
  "Hide This Product From My List": "Bu Ürünü Listemden Gizle",
  "Hide This Product": "Bu Ürünü Gizle",
  "Cancel This Order": "Bu Siparişi İptal Et",
  "Cancel This Product": "Bu Ürünü İptal Et",
  "You Can Cancel This Order And Back Your Money":
    "Bu Siparişi İptal Edip Paranızı İade Edebilirsiniz",
  "You Can Change Size, Color, Qty Of The Product Without Any Conditions According To The Change Policy":
    "Değişim Politikasına Göre Koşulsuz Olarak Ürünün Bedenini, Rengini ve Adedini Değiştirebilirsiniz",
  "Change Qty": "Adedi Değiştir",
  "Change Size": "Bedeni Değiştir",
  "Change Color": "Rengi Değiştir",
  "Change From": "İtibaren",
  "To New Color?": "Yeni renge mi?",
  Clarification: "Açıklama",
  "About Return Your Product": "Ürününüzü İade Hakkında",
  "You Will Not Be Charged Any Fees.": "Hiçbir Ücret Alınmayacaktır.",
  "You Will Receive Your Refund Within 12 Hours.":
    "12 Saat İçinde Tam İadenizi Alacaksınız.",
  "Repeated Cancellations Will Affect Your Rating, Which Will Affect Your Ability To Receive New Offers Or Opportunities From Us.":
    "Tekrarlanan İptaller Puanınızı Etkileyecek ve Bu da Bizden Yeni Teklifler veya Fırsatlar Alabilme Yeteneğinizi Etkileyecektir.",
  "Repeated Returns Will Affect Your Rating, Which Will Affect Your Ability To Receive New Offers Or Opportunities From Us.":
    "Tekrarlanan İadeler Puanınızı Etkileyecek ve Bu da Bizden Yeni Teklifler veya Fırsatlar Alabilme Yeteneğinizi Etkileyecektir.",
  "Terms Of Return": "İade Şartları",
  "Return Terms.": "İade Şartları.",
  "I Agree & Cancel": "Kabul Ediyor ve İptal Ediyorum",
  "I Disagree": "Katılmıyorum",
  "Terms Of Cancellation Terms": "İptal Şartları",
  "About Change Request Product": "Ürün Değişim Talebi Hakkında",
  "Cancel Request": "İptal Talebi",
  "Return This Product In": "Bu Ürünü İçinde İade Et",
  "Cancellation Terms": "İptal Şartları",
  "I Read And Agree To The": "Okudum ve kabul ediyorum",
  "Change Request": "Değişim Talebi",
  "About Cancel Your Product": "Ürün İptal Talebi Hakkında",
  "Cancel This Product In": "Bu Ürünü İptal Edin",
  "You Can Cancel The Product Without Any Conditions According To The Cancel Policy And Get A Full Refund":
    "İptal politikası kapsamında hiçbir koşul olmaksızın ürünü iptal edebilir ve tam para iadesi alabilirsiniz",
  "To Your Account": "Hesabınıza",
  "Why Was The Product Return?": "Ürün neden iade edildi?",
  "About Cancel Your Order": "Siparişinizi İptal Etme Hakkında",
  // Alt attributes
  sy: "Suriye",
  "info icon": "bilgi simgesi",
  login: "giriş yap",
  offer: "teklif",
  avatar: "avatar",
  story: "hikaye",
  user: "kullanıcı",
  imgs: "resimler",
  Preview: "Önizleme",
  "TryDos Logo": "TryDos Logosu",
  truck: "kamyon",
  deliveryman: "teslimatçı",
  Omar: "Omar",
  image: "resim",
  "Please enable notification permissions to use camera features":
    "Kamera özelliklerini kullanmak için lütfen bildirim izinlerini etkinleştirin",
  "buy Button": "satın al butonu",
  Upload: "Yükle",
  TryDos: "TryDos",
  // Console messages
  "Server cookie API failed:": "Sunucu çerez API'si başarısız oldu:",
  "Navigation blocked - already in progress":
    "Gezinme engellendi - zaten devam ediyor",
  "Navigating to:": "Şuraya gidiliyor:",
  "FCM token is older than 1 day. Refreshing...":
    "FCM jetonu 1 günden eski. Yenileniyor...",
  "Notification permission denied or dismissed.":
    "Bildirim izni reddedildi veya kapatıldı.",
  "Do not close this window": "Bu pencereyi kapatmayın",
  "Preparing your experience...": "Deneyiminiz hazırlanıyor...",
  // Date
  "2 June": "2 Haziran",
  // Messages
  "User In Another Call": "Kullanıcı Başka Bir Aramada",
  "Initializing...": "Başlatılıyor...",
  "Loading countries...": "Ülkeler yükleniyor...",
  "Preparing options...": "Seçenekler hazırlanıyor...",
  "Almost ready...": "Neredeyse hazır...",
  "Ready!": "Hazır!",
  "Please wait while we set up your country selection":
    "Ülke seçiminizi ayarlarken lütfen bekleyin",
  "You previously visited from": "Daha önce şuradan ziyaret ettiniz",
  "but now accessing from": "ama şimdi şuradan erişiyorsunuz",
  "Choose your country to get the best experience":
    "En iyi deneyimi elde etmek için ülkenizi seçin",
  "Loading...": "Yükleniyor...",
  "No more notifications": "Başka bildirim yok",
  "Currency Request": "Para Birimi Talebi",
  January: "Ocak",
  February: "Şubat",
  March: "Mart",
  April: "Nisan",
  May: "Mayıs",
  June: "Haziran",
  July: "Temmuz",
  August: "Ağustos",
  September: "Eylül",
  October: "Ekim",
  November: "Kasım",
  December: "Aralık",
  M: "M",
  K: "B",
  "Info Message": "Bilgi Mesajı",
  "Entering The Information Below Clearly And Completely Will Ensure That Your Order Arrives Without Problems And Faster.":
    "Aşağıdaki bilgileri açık ve eksiksiz olarak girmeniz, siparişinizin sorunsuz ve daha hızlı ulaşmasını sağlayacaktır.",
  Enabled: "Etkin",
  "Not Enabled": "Etkin Değil",
  "FireBase Supported": "Firebase Desteği",
  Supported: "Destekleniyor",
  "Not Supported": "Desteklenmiyor",
  "User ID:": "Kullanıcı Kimliği:",
  "FCM Token:": "FCM Jetonu:",
  "FCM Error:": "FCM Hatası:",
  "Notification Settings": "Bildirim Ayarları",
  Product: "Ürün",
  "this message was deleted": "bu mesaj silindi",
  "You will be notified for this product already":
    "Bu ürün için zaten bildirim almayı etkinleştirdiniz",
  "Session Expired..please Verify Your Phone Number":
    "Oturum süresi doldu.. Lütfen telefon numaranızı doğrulayın",
  Cancel: "İptal",
  "For All": "Hepsi İçin",
  "For Me": "Benim İçin",
  Imagee: "Resim",
  Video: "Video",
  Audio: "Ses Kaydı",
  File: "Dosya",
  "The number was copied successfully": "Numara başarıyla kopyalandı",
  "Number copy failed": "Numara kopyalanamadı",
  "Are you sure you want to delete this story?":
    "Bu hikayeyi silmek istediğinizden emin misiniz ?",
  Confirm: "Onayla",
  "Delete Story": "Hikaye Silinsin mi",
  "Failed to delete story.": "Hikaye silinemedi.",
  "Story deleted successfully.": "Hikaye başarıyla silindi.",
  Pack: "Paket",
  "Add Comment…": "Yorum Ekle…",
  "Report This Product": "Bu ürünü bildir",
  "Delivery Time, Delivery Man, Delivery Car":
    "Teslim Süresi, Kurye, Teslimat Aracı",
  Qty: "Adet",
  "You Can't Change Qty To Higher Than The Current Qty":
    "Adedi mevcut adetten daha yüksek seçemezsiniz",
  "Select quantity": "Miktarı seçin",
  "Cancellation Terms.": "İptal Şartları.",
  "Hide This Pack": "Bu Paketi Gizle",
  "Hide This Pack From My List": "Bu Paketi Listemden Gizle",
  "Are you sure you want to hide this order?": "Bu siparişi gizlemek istediğinizden emin misiniz?",
  "Are you sure you want to hide this product?": "Bu ürünü gizlemek istediğinizden emin misiniz?",
  "This is the only product in this order, so hiding it will hide the whole order.":
    "Bu, bu siparişteki tek üründür, bu nedenle onu gizlemek tüm siparişi gizleyecektir.",
  "You Can Cancel This Pack And Back Your Money":
    "Bu Paketi İptal Edip Paranızı Geri Alabilirsiniz",
  "Cancel This Pack": "Bu Paketi İptal Et",
  "I’m Afraid Of Sizes": "Bedenlerden Korkarım",
  "are allowed.": "izin verilir.",
  "Please enter a valid URL (e.g., example.com or www.example.com)":
    "Lütfen geçerli bir URL girin (örn. example.com veya www.example.com)",
  "Only links from": "Yalnızca şu kaynaktan gelen bağlantılar",
  "Trydos — Flash Deals, Boutique Finds & Featured Products":
    "Trydos — Flaş Fırsatlar, Butik Keşifler ve Öne Çıkan Ürünler",
  Trydos: "Trydos",
  "Flash Deals": "Flaş Fırsatlar",
  Bar: "Bar",
  Boutiques: "Butikler",
  "Online Shopping": "Çevrimiçi Alışveriş",
  TryDos: "Trydos",
  "online shopping": "çevrimiçi alışveriş",
  "premium products": "premium ürünler",
  "flash deals": "flaş fırsatlar",
  "best deals": "en iyi fırsatlar",
  "shopping mall": "alışveriş merkezi",
  boutiques: "butikler",
  brands: "markalar",
  fashion: "moda",
  electronics: "elektronik",
  "home garden": "ev ve bahçe",
  "Trydos — Flash Deals & Featured Boutique Picks":
    "Trydos — Flaş Fırsatlar ve Öne Çıkan Butik Seçimleri",
  "Your destination for exclusive bar items, boutique fashion, and daily flash deals.":
    "Özel bar ürünleri, butik moda ve günlük flaş fırsatlar için adresiniz.",
  "Trydos — Shop Boutique Products & Flash Deals":
    "Trydos — Butik Ürünler ve Flaş Fırsatları Satın Al",
  "Explore Trydos for curated categories like Bar, Boutiques, and limited-time Flash Deals.":
    "Trydos'u Bar, Butikler ve sınırlı süreli flaş fırsatlar gibi seçilmiş kategoriler için keşfedin.",
  "Discover Trydos: your online store for bar items, boutique products, and flash deals.":
    "Trydos'u keşfedin: bar ürünleri, butik ürünler ve flaş fırsatlar için çevrimiçi mağazanız.",
  "our top categories": "en iyi kategorilerimiz",
  "trusted brands": "güvenilir markalar",
  "all standard sizes": "tüm standart bedenler",
  "all colors": "tüm renkler",
  "Search results for ": "Arama sonuçları için ",
  Shop: "Alışveriş",
  From: "Başlangıç",
  "Discover a curated selection of": "Seçilmiş bir koleksiyonu keşfedin",
  Featuring: "İçerir",
  "and prices from": "ve fiyatlar",
  to: "ile",
  on: "üzerinde",
  "Search results for": "Arama sonuçları için",
  "Trydos -": "Trydos -",
  "Discover New ": "Yeni keşfet ",
  "Tags: ": "Etiketler: ",
  "Boutiques: ": "Butikler: ",
  "Categories: ": "Kategoriler: ",
  "Brands: ": "Markalar: ",
  "Colors: ": "Renkler: ",
  "Sizes: ": "Bedenler: ",
  "Products on Trydos": "Trydos'taki ürünler",
  Trydos: "Trydos",
  "View More": "Daha Fazla Gör",
  "Only This Piece": "Sadece Bu Parça",
  "SVG Images Not Allowed": "SVG resimlerine izin verilmiyor",
  "File size should not exceed 10 MB": "Dosya boyutu 10 MB'ı aşmamalıdır",
  "File size should not exceed 25 MB": "Dosya boyutu 25 MB'ı aşmamalıdır",
  "Best Price": "En İyi Fiyat",
  "Last 3 Days!": "Son 3 Gün!",
  Trend: "Trend",
  "Color !": "Renk!",
  "Best Sell": "En Çok Satan",
  "Last Week !": "Geçen Hafta!",
  "Fast Packing": "Hızlı Paketleme",
  "& Today Shipping If Buy Before 13:00 Today":
    "Bugün 13:00'ten Önce Alırsanız Bugün Kargo",
  "Open Try-On": "Sanal Deneme Aç",
  "Choose how to add your photo": "Fotoğrafınızı nasıl ekleyeceğinizi seçin",
  "Take a new photo with your camera": "Kameranızla yeni bir fotoğraf çekin",
  "Select from your gallery": "Galerinizden seçin",
  "Your photo is ready!": "Fotoğrafınız hazır!",
  "Click 'Try On' to see how this product looks on you":
    "Bu ürünün üzerinizde nasıl göründüğünü görmek için 'Dene'ye tıklayın",
  "Processing your virtual try-on...": "Sanal denemeniz işleniyor...",
  "This may take a few moments": "Bu birkaç dakika sürebilir",
  "Try again": "Tekrar dene",
  "Capture Photo": "Fotoğraf Çek",
  "Processing…": "İşleniyor…",
  "Try On": "Dene",
  "Not now": "Şimdi değil",
  Allow: "İzin ver",
  "Back-in-stock and quantity alerts": "Stok yenileme ve miktar uyarıları",
  "New boutiques & categories": "Yeni butik ve kategoriler",
  "Order updates": "Sipariş güncellemeleri",
  "Chat messages": "Sohbet mesajları",
  "Enable notifications for a more effortless shopping experience:":
    "Daha zahmetsiz bir alışveriş deneyimi için bildirimleri etkinleştirin:",
  "Stay in the loop": "Güncel kal",
  "Luck!": "Şans!",
  "Add To Bag Within ": "Sepete Ekle İçinde",
  seconds: "saniye",
  "stroke of luck": "Şans eseri",
  "Today Shipping": "Bugün Kargo",
  "If Buy Before": "Satın alırsanız",
  Syria: "Suriye",
  Iraq: "Irak",
  Lebanon: "Lübnan",
  Turkey: "Türkiye",
  "Selected Color": "Seçilen Renk",
  "Select Color": "Seçilen Renk",
  "Select Your Required Size": "Gerekli Bedeninizi Seçin",
  "Added To Your Bag": "Sepetinize Eklendi",
  "Removed From Your Bag": "Sepetinizden Çıkarıldı",
  Added: "Eklendi",
  "To Your Bag": "Sepetine",
  "Take A Look At Other Colors": "Diğer Renkleri İncele",
  "Notify Me When Variant Is Available":
    "Ürün mevcut olduğunda beni bilgilendir",
  "We Will Inform You When Variant Is Available":
    "Ürün mevcut olduğunda sizi bilgilendireceğiz",
  "Rate Your Experience": "Deneyiminizi Değerlendirin",
  "Share your thoughts about this product":
    "Bu ürün hakkındaki düşüncelerinizi paylaşın",
  "See Our Many Offers And Take Advantage Of Discounts And Special Products":
    "Birçok teklifimizi görün ve indirimlerden ve özel ürünlerden yararlanın",
  "Notification Is Not Enabled! please Allow Notification Access":
    "Bildirimler etkin değil! Lütfen bildirim erişimine izin verin",
  "Your Balance Not meet purchase value":
    "Bakiyeniz satın alma tutarını karşılamıyor",
  "New Price": "Yeni Fiyat",
  "Please Login so You Can Add a comment":
    "Yorum ekleyebilmek için lütfen giriş yapın",
  "Please Login so You Can Add a comment":
    "تکایە بچۆ ژوورەوە بۆ ئەوەی بتوانیت لێدوانێک زیاد بکەیت",
  "Update Rating": "Değerlendirmeyi Güncelle",
  "Edit Your Question": "Sorunuzu Düzenleyin",
  "Update your question about this product":
    "Bu ürünle ilgili sorunuzu güncelleyin",
  "Update Question": "Soruyu Güncelle",
  "Your Comment": "Yorumunuz",
  Edit: "Düzenle",
  "Add your comment": "Yorumunuzu ekleyin",
  // Wallet Transactions
  "Wallet Transactions": "Cüzdan İşlemleri",
  Wallet: "Cüzdan",
  Back: "Geri",
  Date: "Tarih",
  Amount: "Tutar",
  Type: "Tür",
  Description: "Açıklama",
  Inflow: "Giriş",
  Outflow: "Çıkış",
  "Order Refund": "Sipariş İadesi",
  "Purchase Payment": "Satın Alma Ödemesi",
  "Promo Credit": "Promosyon Kredisi",
  "Order Payment": "Sipariş Ödemesi",
  "Submit Rating": "Değerlendirmeyi Gönder",
  "Do you want to delete this message?": "Bu mesajı silmek istiyor musunuz?",
  Last: "Kalan",
  "Waiting Seller Reply...": "Satıcı Yanıtını Bekliyor...",
  "Add More to Your Bag": " Sepetinize Daha Fazla Ekle",
  "Please enable camera permissions to use camera feature":
    "Kamera özelliğini kullanmak için lütfen kamera izinlerini etkinleştirin",
  Small: "küçük",
  Large: "büyük",
  True: "gerçek",
  "Max Allowed Quantity Reached": "İzin verilen maksimum miktara ulaşıldı",
  "I Agree & Return": "Kabul Ediyorum ve İade Ediyorum",
  "Delay Confirmation. I want to Return more product":
    "Onayı ertele. Daha fazla ürün iade etmek istiyorum",
  "chosse an image or video from camera or files":
    "Kameradan veya dosyalardan bir fotoğraf ya da video seçin",
  camera: "Kamera",
  files: "Dosyalar",
  "Terms Of Change Address": " Adres Değişikliği Şartları",
  "You cannot send messages or calls to this user":
    "Bu kullanıcıya mesaj veya arama gönderemezsiniz",
  Block: "Engelle",
  UnBlock: "Engeli kaldır",
  "Delete Chat": "Sohbeti sil",
  "Are you sure you want to delete this chat?": "Bu sohbeti silmek istediğinizden emin misiniz?",
  "Save To Gallery": "Galeride kaydet",
  Never: "Asla",
  "Media & Files": "Medya ve dosyalar",
  Search: "Ara",
  "Video as مكالمة فيديو": "Görüntülü arama",
  Call: "Ara",
  "Copy Invite": " Davetiyeyi Kopyala",
  "Choose how to send the invitation":
    " Davetiyeyi nasıl göndereceğinizi seçin",
  "Shared a product": " Bir ürün paylaştı",
  "Confirm Return Request": " İade Talebini Onayla",
  "Cancel Return Request": "İade Talebini İptal Et",
  "Go to Seller Dashboard": " Satıcı Kontrol Paneline Git",
  "Become A Seller At Trydos": "Trydos’ta Satıcı Olun",
  now: "şimdi",
  image: "resim gönderdi",
  video: "video gönderdi",
  file: "dosya gönderdi",
  "voice message": "sesli mesaj gönderdi",
  message: "mesaj gönderdi",
  "Please enable  permissions (camera,mic) to use calls features":
    "Arama özelliklerini kullanmak için lütfen izinleri (kamera, mikrofon) etkinleştirin",
  Online: "Çevrimiçi",
  "last Seen ": "Son görülme",
  "Outgoing Voice Call": "Giden sesli arama",
  "Outgoing Video Call": "Giden görüntülü arama",
  "Incoming Voice Call": "Gelen sesli arama",
  "Incoming Video Call": "Gelen görüntülü arama",
  "Missed Voice Call At": "Cevapsız sesli arama saat",
  "Missed Video Call At": "Cevapsız görüntülü arama saat",
  "Missed Voice Call": "Cevapsız sesli arama",
  "Missed Video Call": "Cevapsız görüntülü arama",
  "Loading products...": "Berhem tên barkirin...",
  "Loading boutiques...": "Dikan tên barkirin...",
  "Loading orders...": "Siparîş tên barkirin...",
  "Loading permissions...": "Destûr tên barkirin...",
  "Loading users...": "Bikarhêner tên barkirin...",
  "Checking permissions...": "Destûr tên kontrolkirin...",
  Retry: "Dîsa biceribîne",
  "Access Denied": "Destûr nayê dayîn",
  "You don't have permission to view products":
    "Tu destûra dîtina berheman nîn e",
  "You don't have permission to manage users":
    "Tu destûra rêvebirina bikarhêneran nîn e",
  "You don't have permission to view permissions":
    "Tu destûra dîtina destûran nîn e",
  Users: "Bikarhêner",
  "Add User": "Bikarhêner zêde bike",
  "Add User to Shop": "Bikarhêner li dikanê zêde bike",
  "User added successfully!": "Bikarhêner bi serkeftî hate zêdekirin",
  Delete: "Jê bibe",
  "Leave Shop": "Ji dikanê derkeve",
  "Search roles...": "Rol bigere...",
  "No roles found": "Rol nehat dîtin",
  "Load more roles": "Rolên din bar bike",
  "No orders found": "Siparîş nehat dîtin",
  Update: "Rojane bike",
  "Updating...": "Tê rojanekirin...",
  "Change status:": "Rewş biguherîne:",
  Products: "Ürünler",
  Boutiques: "Dikan",
  "No products found": "Berhem nehat dîtin",
  "No boutiques found": "Dikan nehat dîtin",
  "please Select a shop to continue": "Lütfen bir mağaza seçin",
  "Loading...": "Yükleniyor...",
  "No shops available": "Kullanılabilir mağaza yok",
  "Shop Name": "Mağaza Adı",
  "Seller ID": "Satıcı ID",
  Actions: "İşlemler",
  Enter: "Giriş",
  Leave: "Ayrıl",
  "Confirm leave shop": "Mağazadan ayrılmayı onayla",
  "Are you sure you want to leave this shop? This will remove your access to the shop dashboard.":
    "Bu mağazadan ayrılmak istediğinizden emin misiniz? Mağaza kontrol paneline erişiminiz kaldırılacaktır.",
  Cancel: "İptal",
  "Leaving...": "Ayrılıyor...",
  "Failed to leave shop": "Mağazadan ayrılma başarısız oldu",
  "Access Denied": "Erişim reddedildi",
  "You don't have permission to view products":
    "Ürünleri görüntüleme yetkiniz yok",
  Permissions: "Yetkiler",
  "Name / Phone": "İsim / Telefon",
  Role: "Rol",
  Actions: "İşlemler",
  "Seller ID": "Satıcı ID",
  "Searching roles...": "Roller aranıyor...",
  "Loading roles...": "Roller yükleniyor...",
  "Phone Number": "Telefon Numarası",
  "Format: +(country_code)XXX (e.g., +9611234567)":
    "Format: +(ülke_kodu)XXX (örn. +9611234567)",
  "Seller Dashboard": "Satıcı Paneli",
  "Access Denied": "Destûr nayê dayîn",
  "You don't have permission to view products":
    "Tu destûra dîtina berheman nîn e",
  Permissions: "Destûr",
  "Name / Phone": "Nav / Telefon",
  Role: "Rol",
  Actions: "Çalakî",
  "Seller ID": "ID ya firoşkar",
  "Searching roles...": "Li rolan digere...",
  "Loading roles...": "Rol tên barkirin...",
  "Phone Number": "Hejmara telefonê",
  "Format: +(country_code)XXX (e.g., +9611234567)":
    "Format: +(koda welatê)XXX (mînak: +9611234567)",
  "Seller Dashboard": "Panela firoşkar",
  "Seller ID:": "Satıcı ID:",
  "please Select a shop to continue": "Lütfen bir mağaza seçin",
  "No shops available": "Mevcut mağaza yok",
  "Super Admin": "Süper Yönetici",
  "You have full access to all features": "Tüm özelliklere tam erişiminiz var",
  "Recommended Products": ' "Önerilen Ürünler",',
  "To New Size?": "Bo qebarey nû?",

  "Settings - TryDos": "Ayarlar - TryDos",
  "Manage your account settings and preferences.":
    "Hesap ayarlarınızı ve tercihlerinizi yönetin.",
  "Compare products side by side on TryDos - Make informed purchasing decisions.":
    "TryDos'ta ürünleri yan yana karşılaştırın - Bilinçli satın alma kararları verin.",
  "Discover premium products on TryDos - Your ultimate shopping destination with featured products, flash deals, and boutique collections.":
    "TryDos'ta premium ürünleri keşfedin - Öne çıkan ürünler, hızlı fırsatlar ve butik koleksiyonlarla nihai alışveriş hedefiniz.",
  "TryDos - Boutique & Product Listing": "TryDos - Butik ve Ürün Listesi",
  "Browse TryDos boutiques and products with powerful filters for brand, category, color, size, and more.":
    "TryDos butiklerini ve ürünlerini marka, kategori, renk, beden ve daha fazlası için güçlü filtrelerle göz atın.",
  "RDB Wallet": "RDB Cüzdanı",
  "Retry fetching wallet balance": "Cüzdan bakiyesi yükleniyor...",
  "Submit Deposit": "Yatırımı Gönder",
  "Processing...": "İşleniyor...",
  "Click to upload": "Yüklemek için tıklayın",
  "Proof of Transfer": "Transfer Onayı",
  Tax: "Vergi",
  "You Receive": "Alırsınız",
  Fee: "Ücret",
  "No banks support this currency for deposit":
    "Bu para birimi için yatırım yapan banka yok",
  "Select a bank": "Banka Seçin",
  "No deposits found": "Yatırım bulunamadı",
  "Create Deposit": "Yatırım Oluştur",
  Deposits: "Yatırımlar",
  "Related Categories": "İlgili Kategoriler",
  minute: "dakika",
  "Call End in": "Arama Sonu",
  "Read Products": "Ürünleri Görüntüle",
  "Create Product": "Ürün Oluştur",
  "Update Product": "Ürünü Güncelle",
  "Change Product Status": "Ürün Durumunu Değiştir",

  "Read Butiks": "Butikleri Görüntüle",
  "Create Butiks": "Butik Oluştur",
  "Update Butiks": "Butiği Güncelle",
  "Delete Butiks": "Butiği Sil",
  "Change Boutique Status": "Butik Durumunu Değiştir",

  "Read Categories": "Kategorileri Görüntüle",
  "Create Categories": "Kategori Oluştur",
  "Update Categories": "Kategoriyi Güncelle",
  "Delete Categories": "Kategoriyi Sil",
  "Change Category Status": "Kategori Durumunu Değiştir",

  "Read Brands": "Markaları Görüntüle",
  "Create Brands": "Marka Oluştur",
  "Update Brands": "Markayı Güncelle",
  "Delete Brands": "Markayı Sil",
  "Change Brand Status": "Marka Durumunu Değiştir",

  "Read Orders": "Siparişleri Görüntüle",
  "Update Order Info": "Sipariş Bilgilerini Güncelle",
  "Change Order Status": "Sipariş Durumunu Değiştir",
  "Read Order Payments": "Sipariş Ödemelerini Görüntüle",
  "Confirm Order Payment": "Ödemeyi Onayla",
  "Refund Order Payment": "Ödemeyi İade Et",
  "Cancel Order": "Siparişi İptal Et",
  "Assign Shipping": "Kargo Ata",
  "Update Tracking": "Takibi Güncelle",
  "Change Order Status Packaged": "Siparişi Paketlendi Olarak Değiştir",
  "Change Order Status Canceled": "Siparişi İptal Edildi Olarak Değiştir",

  "Read Employees": "Çalışanları Görüntüle",
  "Create Employees": "Çalışan Oluştur",
  "Update Employees": "Çalışanı Güncelle",
  "Delete Employees": "Çalışanı Sil",

  "Read Roles": "Rolleri Görüntüle",
  "Create Roles": "Rol Oluştur",
  "Update Roles": "Rolü Güncelle",
  "Delete Roles": "Rolü Sil",

  "Read Jobtitles": "İş Unvanlarını Görüntüle",
  "Create Jobtitles": "İş Unvanı Oluştur",
  "Update Jobtitles": "İş Unvanını Güncelle",
  "Delete Jobtitles": "İş Unvanını Sil",

  "Read Offices": "Ofisleri Görüntüle",
  "Create Offices": "Ofis Oluştur",
  "Update Offices": "Ofisi Güncelle",
  "Delete Offices": "Ofisi Sil",

  "Read Departments": "Departmanları Görüntüle",
  "Create Departments": "Departman Oluştur",
  "Update Departments": "Departmanı Güncelle",
  "Delete Departments": "Departmanı Sil",

  "Read Workforms": "Çalışma Türlerini Görüntüle",
  "Create Workforms": "Çalışma Türü Oluştur",
  "Update Workforms": "Çalışma Türünü Güncelle",
  "Delete Workforms": "Çalışma Türünü Sil",

  "Read Languages": "Dilleri Görüntüle",
  "Create Languages": "Dil Oluştur",
  "Update Languages": "Dili Güncelle",
  "Delete Languages": "Dili Sil",

  "Read Currencies": "Para Birimlerini Görüntüle",
  "Create Currencies": "Para Birimi Oluştur",
  "Update Currencies": "Para Birimini Güncelle",
  "Delete Currencies": "Para Birimini Sil",

  "Read Shipping": "Kargoyu Görüntüle",
  "Create Shipping": "Kargo Oluştur",
  "Update Shipping": "Kargoyu Güncelle",
  "Delete Shipping": "Kargoyu Sil",

  "Read Countries": "Ülkeleri Görüntüle",
  "Create Countries": "Ülke Oluştur",
  "Update Countries": "Ülkeyi Güncelle",
  "Delete Countries": "Ülkeyi Sil",

  "Super Admin": "Süper Yönetici",
  "User Management Access": "Kullanıcı Yönetimi Erişimi",
  "Change Role": "Rolü Değiştir",
  PRODUCTS: "Ürünler",
  BOUTIQUES: "Butikler",
  CATEGORIES: "Kategoriler",
  BRANDS: "Markalar",
  ORDERS: "Siparişler",
  EMPLOYEES: "Çalışanlar",
  ROLES: "Roller",
  JOBTITLES: "İş Unvanları",
  OFFICES: "Ofisler",
  DEPARTMENTS: "Departmanlar",
  WORKFORMS: "Çalışma Türleri",
  LANGUAGES: "Diller",
  CURRENCIES: "Para Birimleri",
  SHIPPING: "Kargo",
  COUNTRIES: "Ülkeler",
  ADMIN: "Yönetim",
  COMMENTS: "Yorumlar",
  Permissions: "İzinler",
  "Failed to update checklist": "Kontrol listesini güncelleme başarısız oldu",
  Stories: "Hikayeler",
  "No stories yet": "Henüz hikaye yok",
  "Upload your first story to get started": "Başlamak için ilk hikayeni yükle",
  "Delete Story": "Hikayeyi Sil",
  "Are you sure you want to delete this story? This action cannot be undone.": "Bu hikayeyi silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.",
  "Select Product": "Ürün Seç",
  "Link to Product": "Ürüne Bağla",
  "Story uploaded successfully": "Hikaye başarıyla yüklendi",
  "Failed to load stories": "Hikayeler yüklenemedi",
  "Failed to delete story": "Hikaye silinemedi",
  "Linked to product": "Ürüne bağlı",
  viewers: "görüntülenme",
  "Edit Shop Info": "Mağaza Bilgilerini Düzenle",
    "Shop Name *": "Mağaza Adı *",
    "Contact": "İletişim",
    "* Country Code Must Like for AE 971": "* Ülke Kodu AE 971 gibi olmalıdır",
    "Address *": "Adres *",
    "Upload Image": "Resim Yükle",
    "Choose File": "Dosya Seç",
    "Browse": "Gözat",
    "Upload Banner": "Afiş Yükle",
    "Ratio .( 6:1 )": "Oran .( 6:1 )",
    "Cancel": "İptal",
    "Update": "Güncelle",
    "Shop Name is required": "Mağaza adı gereklidir",
    "First Name is required": "Ad gereklidir",
    "Last Name is required": "Soyad gereklidir",
    "Shop Address is required": "Mağaza adresi gereklidir",
    "Location Name is required": "Konum adı gereklidir",
    "Location Address is required": "Konum adresi gereklidir",
    "Contact is required": "İletişim numarası gereklidir",
    "Contact must contain valid numbers": "İletişim numarası sadece rakamlardan oluşmalıdır",
    "Address is required": "Adres gereklidir",
    "Shop Info Updated Successfully!": "Mağaza Bilgileri Başarıyla Güncellendi!",
    "Failed to update": "Güncelleme başarısız oldu",
    "Preview Photo": "Fotoğraf Önizleme",
    "Preview Video": "Video Önizleme",
    "Create Story": "Hikaye Oluştur",
    "Microphone not detected or permission denied": "Mikrofon algılanmadı veya izin reddedildi",
    "Photo": "Fotoğraf",
    "Video": "Video",
    "Privacy Policy": "Gizlilik Politikası",
    "Contact Us": "Bize Ulaşın",
    "Compare": "Karşılaştır",
    "About TryDos": "TryDos Hakkında",
    "TryDos is a multilingual live-shopping marketplace connecting boutiques and shoppers across the region with curated fashion, beauty and lifestyle products.":
      "TryDos, bölgedeki butikleri ve alışverişçileri seçkin moda, güzellik ve yaşam tarzı ürünleriyle buluşturan çok dilli bir canlı alışveriş pazar yeridir.",
    "TryDos is a multilingual live-shopping marketplace connecting boutiques and shoppers across the region. We bring curated fashion, beauty and lifestyle products from trusted sellers straight to your screen.":
      "TryDos, bölge genelinde butikleri ve alışverişçileri buluşturan çok dilli bir canlı alışveriş pazar yeridir. Güvenilir satıcılardan seçkin moda, güzellik ve yaşam tarzı ürünlerini doğrudan ekranınıza getiriyoruz.",
    "Our Mission": "Misyonumuz",
    "We make discovering and buying from independent boutiques effortless — with live video shopping, secure checkout and fast delivery, all in your local language and currency.":
      "Bağımsız butikleri keşfetmeyi ve onlardan alışveriş yapmayı zahmetsiz hale getiriyoruz — canlı video alışverişi, güvenli ödeme ve hızlı teslimatla, hepsi kendi dilinizde ve para biriminizde.",
    "Why Shop With TryDos": "Neden TryDos'tan Alışveriş Yapmalısınız",
    "Thousands of products from verified boutiques, real-time live sales, easy returns and dedicated support in Arabic, English, Turkish and Kurdish.":
      "Doğrulanmış butiklerden binlerce ürün, gerçek zamanlı canlı satışlar, kolay iade ve Arapça, İngilizce, Türkçe ve Kürtçe özel destek.",
    "Read the TryDos Privacy Policy: what information we collect, how we use it, and the choices you have over your data.":
      "TryDos Gizlilik Politikasını okuyun: hangi bilgileri topladığımız, bunları nasıl kullandığımız ve verileriniz üzerindeki seçenekleriniz.",
    "Your privacy matters to us. This policy explains what information TryDos collects, how we use it, and the choices you have.":
      "Gizliliğiniz bizim için önemlidir. Bu politika, TryDos'un hangi bilgileri topladığını, bunları nasıl kullandığını ve sahip olduğunuz seçenekleri açıklar.",
    "Information We Collect": "Topladığımız Bilgiler",
    "We collect the details you provide when you create an account or place an order, along with basic device and usage data needed to operate and secure the service.":
      "Bir hesap oluşturduğunuzda veya sipariş verdiğinizde sağladığınız bilgileri, ayrıca hizmeti çalıştırmak ve güvende tutmak için gereken temel cihaz ve kullanım verilerini toplarız.",
    "How We Use Your Data": "Verilerinizi Nasıl Kullanırız",
    "Your data is used to process orders, personalise your experience, prevent fraud and improve TryDos. We never sell your personal information.":
      "Verileriniz; siparişleri işlemek, deneyiminizi kişiselleştirmek, dolandırıcılığı önlemek ve TryDos'u geliştirmek için kullanılır. Kişisel bilgilerinizi asla satmayız.",
    "The TryDos Terms & Conditions covering how to use the platform, orders, payments and your responsibilities as a shopper or seller.":
      "Platformun nasıl kullanılacağını, siparişleri, ödemeleri ve bir alışverişçi veya satıcı olarak sorumluluklarınızı kapsayan TryDos Şartlar ve Koşulları.",
    "By using TryDos you agree to these terms. Please read them carefully before shopping or selling on our platform.":
      "TryDos'u kullanarak bu şartları kabul etmiş olursunuz. Platformumuzda alışveriş yapmadan veya satış yapmadan önce lütfen bunları dikkatlice okuyun.",
    "Using TryDos": "TryDos'u Kullanmak",
    "You agree to use TryDos lawfully, to provide accurate account information, and to respect the rights of boutiques, sellers and other shoppers.":
      "TryDos'u yasalara uygun kullanmayı, doğru hesap bilgileri sağlamayı ve butiklerin, satıcıların ve diğer alışverişçilerin haklarına saygı göstermeyi kabul edersiniz.",
    "Orders & Payments": "Siparişler ve Ödemeler",
    "All orders are subject to availability and confirmation. Prices, fees and delivery times are shown at checkout and may vary by country.":
      "Tüm siparişler, stok durumuna ve onaya tabidir. Fiyatlar, ücretler ve teslimat süreleri ödeme sırasında gösterilir ve ülkeye göre değişebilir.",
    "Contact the TryDos team for help with your order, account, or to learn about selling on the platform as a boutique.":
      "Siparişiniz, hesabınız konusunda yardım almak veya bir butik olarak platformda satış yapmayı öğrenmek için TryDos ekibiyle iletişime geçin.",
    "We're here to help. Reach the TryDos team with any question about your order, account or selling on the platform.":
      "Yardım için buradayız. Siparişiniz, hesabınız veya platformda satış yapma konusundaki her soru için TryDos ekibine ulaşın.",
    "Customer Support": "Müşteri Desteği",
    "For help with orders, returns or your account, contact our support team through the app or by email at support@trydos.com.":
      "Siparişler, iadeler veya hesabınız konusunda yardım için uygulama üzerinden veya support@trydos.com e-posta adresinden destek ekibimizle iletişime geçin.",
    "Business & Boutiques": "İş Ortaklığı ve Butikler",
    "Interested in selling on TryDos? Get in touch and our team will guide you through joining as a boutique.":
      "TryDos'ta satış yapmak ister misiniz? Bize ulaşın, ekibimiz butik olarak katılım sürecinde size rehberlik etsin.",
  "Checkout": "Ödeme",
  "Wallet Checkout": "Cüzdan ile Ödeme",
  "Select Payment Currency": "Ödeme Para Birimini Seçin",
  "Available": "Mevcut",
  "Payment Summary": "Ödeme Özeti",
  "Cart Total:": "Sepet Toplamı:",
  "Available Balance:": "Mevcut Bakiye:",
  "Amount to Pay:": "Ödenecek Tutar:",
  "Insufficient balance. Please add funds or select a different currency.": "Yetersiz bakiye. Lütfen bakiye ekleyin veya farklı bir para birimi seçin.",
  "Confirm Payment": "Ödemeyi Onayla",
  "Microphone access denied": "Mikrofon erişimi reddedildi",
  "Try again with clear voice": "Net bir sesle tekrar deneyin",
  "Failed to process audio": "Ses işlenemedi",
  "Browser does not support this feature": "Tarayıcı bu özelliği desteklemiyor",
  "Story Uploaded": "Hikaye Yüklendi",
  "Error Uploading Story": "Hikaye yüklenirken hata oluştu",
  "only 3 pinned chats allowed": "Yalnızca 3 sohbet sabitlenebilir",
  "Connection Lost": "Bağlantı Koptu",
  "We are having trouble connecting to our servers. This might be due to:": "Sunucularımıza bağlanırken sorun yaşıyoruz. Bunun nedeni şunlar olabilir:",
  "Network connectivity issues": "Ağ bağlantısı sorunları",
  "Temporary server maintenance": "Geçici sunucu bakımı",
  "Slow internet connection": "Yavaş internet bağlantısı",
  "Reload Page": "Sayfayı Yenile",
  "Go Home": "Ana Sayfaya Git",
  "Oops! Something went wrong": "Hata! Bir şeyler ters gitti",
  "We encountered an unexpected error. Dont worry, our team has been notified and is working on it.": "Beklenmeyen bir hatayla karşılaştık. Endişelenmeyin, ekibimize bilgi verildi ve üzerinde çalışıyor.",
  "Error ID:": "Hata Kimliği:",
  "Item": "Ürün",
  "Collect Type": "Tahsilat Türü",
  "Collect": "Tahsil Et",
  "Duration To Do Action": "İşlem Yapma Süresi",
  "Color:": "Renk:",
  "Size:": "Beden:",
  "ID:": "Kimlik:",
  "Quantity:": "Adet:",
  "Status:": "Durum:",
  "Loading Call Information...": "Arama bilgileri yükleniyor...",
  "Copy Link": "Bağlantıyı Kopyala",
  "Add a new contact": "Yeni kişi ekle",
  "Contact Name": "Kişi Adı",
  "Already saved as": "Şu adla zaten kayıtlı",
  "File": "Dosya",
  "Image": "Resim",
  "Product": "Ürün",
  "Audio": "Ses",
  "Incoming": "Gelen",
  "Call": "Arama",
  "Crop Image": "Resmi Kırp",
  "Save": "Kaydet",
  "Min": "Min",
  "Max": "Maks",
  "Qty:": "Adet:",
  "Price": "Fiyat",
  "Subtotal": "Ara Toplam",
  "Error:": "Hata:",
  "Go Back": "Geri Dön",
  "Search": "Ara",
  "Search...": "Ara...",
  "No matching options.": "Eşleşen seçenek bulunamadı.",
  "Forward Message": "Mesajı İlet",
  "No more stories": "Başka hikaye yok",
  "Name": "İsim",
  "OR": "VEYA",
  "Session:": "Oturum:",
  "Under Development": "Geliştirme Aşamasında",
  "Browse Categories": "Kategorilere Göz At",
  "Popular Links:": "Popüler Bağlantılar:",
  "Boutiques": "Butikler",
  "Load more": "Daha fazla yükle",
  "Invalid cart data": "Geçersiz sepet verisi",
  "Insufficient balance in your wallet.": "Cüzdanınızda yetersiz bakiye.",
  "Payment successful!": "Ödeme başarılı!",
  "Payment failed. Please try again.": "Ödeme başarısız. Lütfen tekrar deneyin.",
  "Province": "İl",
  "Town": "Kasaba",
  "Suburb": "Mahalle",
  "Syncing...": "Eşitleniyor...",
  "Adding...": "Ekleniyor...",
  "Confirm Add": "Eklemeyi Onayla",
  "Send": "Gönder",
  "None": "Yok",
  "FILE": "DOSYA",
  "please select supported image format (jpeg, png, jpg, webp, svg, avif)": "lütfen desteklenen bir resim formatı seçin (jpeg, png, jpg, webp, svg, avif)",
  "No options found": "Seçenek bulunamadı",
  "Delivery Worker": "Teslimat Görevlisi",
  "Unknown": "Bilinmeyen",
  "Wrong Code": "Yanlış Kod",
  "Please Review Your Cart Info": "Lütfen Sepet Bilgilerinizi Gözden Geçirin",
  "Product not found": "Ürün bulunamadı",
  "Search request failed": "Arama isteği başarısız oldu",
  "Authentication required": "Kimlik doğrulama gerekli",
  "Customer Info Error": "Müşteri Bilgisi Hatası",
  "Failed": "Başarısız",
  "Permissions not granted for calls": "Aramalar için izin verilmedi",
  "Errors:": "Hatalar:",
  "Session for simulate user has ended": "Kullanıcı simülasyon oturumu sona erdi",
  "Media server upload is not configured": "Medya sunucusu yüklemesi yapılandırılmamış",
  "Media server upload failed": "Medya sunucusuna yükleme başarısız oldu",
  "Media server is not configured": "Medya sunucusu yapılandırılmamış",
  "No images were uploaded to the media server": "Medya sunucusuna hiç resim yüklenmedi",
  "Image upload failed": "Resim yükleme başarısız oldu",
  "Invalid upload response": "Geçersiz yükleme yanıtı",
  "Upload returned no file": "Yükleme hiçbir dosya döndürmedi",
  "Upload returned no files": "Yükleme hiçbir dosya döndürmedi",
  "Buyer": "Alıcı",
  "+(country_code)XXX": "+(ülke_kodu)XXX",
  "1 minutes video only": "Yalnızca 1 dakikalık video",
  "Active": "Aktif",
  "Add": "Ekle",
  "Adding User...": "Kullanıcı ekleniyor...",
  "Allow Purchase": "Satın Almaya İzin Ver",
  "Allow this product to be purchased?": "Bu ürünün satın alınmasına izin verilsin mi?",
  "An English (en) name is required to enable the product.": "Ürünü etkinleştirmek için İngilizce (en) bir ad gereklidir.",
  "Are you sure you want to delete the selected images? This action cannot be undone.": "Seçili görselleri silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.",
  "Are you sure you want to delete this image? This action cannot be undone.": "Bu görseli silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.",
  "Assign each uploaded image to a color. Every color needs at least one image, and every image must be assigned.": "Yüklenen her görseli bir renge atayın. Her rengin en az bir görsele ihtiyacı vardır ve her görsel atanmalıdır.",
  "Barcode": "Barkod",
  "Boutique": "Butik",
  "Boutiques created for this shop will appear here.": "Bu mağaza için oluşturulan butikler burada görünecek.",
  "Brand": "Marka",
  "Browse and manage your catalogue": "Kataloğunuzu görüntüleyin ve yönetin",
  "Bulk-import products by template": "Şablonla toplu ürün içe aktarın",
  "Cannot enable yet — resolve these first:": "Henüz etkinleştirilemiyor — önce şunları çözün:",
  "Categories": "Kategoriler",
  "Change Banner": "Afişi Değiştir",
  "Change Image": "Görseli Değiştir",
  "Change media": "Medyayı değiştir",
  "Change status": "Durumu değiştir",
  "Changes were submitted": "Değişiklikler gönderildi",
  "Your Trusted Commerce Partner": "Güvenilir Ticaret Ortağınız",
  "Changes were submitted and are pending admin approval — they go live once approved.": "Değişiklikler gönderildi ve yönetici onayı bekliyor — onaylandıktan sonra yayına girecek.",
  "Charge shipping per item ordered.": "Sipariş edilen her ürün için kargo ücreti al.",
  "Check back later for comments from customers.": "Müşteri yorumları için daha sonra tekrar kontrol edin.",
  "Choose a section to get started.": "Başlamak için bir bölüm seçin.",
  "Collected": "Teslim Alındı",
  "Color Images": "Renk Görselleri",
  "Confirm & Save": "Onayla ve Kaydet",
  "Confirm & Start Packing": "Onayla ve Paketlemeye Başla",
  "Confirm Upload": "Yüklemeyi Onayla",
  "Confirm changes": "Değişiklikleri onayla",
  "Confirmed": "Onaylandı",
  "Copied!": "Kopyalandı!",
  "Copy URL": "URL'yi Kopyala",
  "Could not change status": "Durum değiştirilemedi",
  "Country code must lead, e.g. AE = 971": "Ülke kodu başta olmalıdır, örn. AE = 971",
  "Country of Origin": "Menşe Ülkesi",
  "Cover": "Kapak",
  "Created At": "Oluşturulma Tarihi",
  "Current Stock": "Mevcut Stok",
  "Customers will be able to buy this product once it passes activation checks.": "Bu ürün etkinleştirme kontrollerini geçtikten sonra müşteriler satın alabilecek.",
  "Dashboard Menu": "Panel Menüsü",
  "Delete Image": "Görseli Sil",
  "Delete Reply": "Yanıtı Sil",
  "Delete this reply?": "Bu yanıt silinsin mi?",
  "Deleting...": "Siliniyor...",
  "Deselect": "Seçimi Kaldır",
  "Deselect all": "Tüm seçimi kaldır",
  "Disable": "Devre Dışı Bırak",
  "Disable purchasing?": "Satın alma devre dışı bırakılsın mı?",
  "Disabled": "Devre Dışı",
  "Discount": "İndirim",
  "Discount Price": "İndirimli Fiyat",
  "Download": "İndir",
  "Play": "Oynat",
  "This video format is not supported. Send an MP4 video instead.": "Bu video biçimi desteklenmiyor. Bunun yerine MP4 formatında bir video gönderin.",
  "This video cannot play here. Tap to download it.": "Bu video burada oynatılamıyor. İndirmek için dokunun.",
  "Drop images here": "Görselleri buraya bırakın",
  "Edit shop name, contact & media": "Mağaza adını, iletişimi ve medyayı düzenleyin",
  "Enter Dashboard": "Panele Gir",
  "Extra": "Ek",
  "Extra price": "Ek fiyat",
  "Failed to add user": "Kullanıcı eklenemedi",
  "Failed to add user to shop": "Kullanıcı mağazaya eklenemedi",
  "Failed to cancel order item": "Sipariş öğesi iptal edilemedi",
  "Failed to confirm order item": "Sipariş öğesi onaylanamadı",
  "Failed to delete image": "Görsel silinemedi",
  "Failed to delete images": "Görseller silinemedi",
  "Failed to delete user": "Kullanıcı silinemedi",
  "Failed to load boutiques": "Butikler yüklenemedi",
  "Failed to load images": "Görseller yüklenemedi",
  "Failed to load orders": "Siparişler yüklenemedi",
  "Failed to load permissions": "İzinler yüklenemedi",
  "Failed to load product": "Ürün yüklenemedi",
  "Failed to load products": "Ürünler yüklenemedi",
  "Failed to load roles": "Roller yüklenemedi",
  "Failed to load uploaded files": "Yüklenen dosyalar yüklenemedi",
  "Failed to load users": "Kullanıcılar yüklenemedi",
  "Failed to pack order item": "Sipariş öğesi paketlenemedi",
  "Failed to update order status": "Sipariş durumu güncellenemedi",
  "Failed to update product": "Ürün güncellenemedi",
  "Failed to update user role": "Kullanıcı rolü güncellenemedi",
  "Flat": "Sabit",
  "Gallery": "Galeri",
  "General": "Genel",
  "ID": "ID",
  "Identity & classification of the product.": "Ürünün kimliği ve sınıflandırması.",
  "Images": "Görseller",
  "Inactive": "Pasif",
  "Item is packed once an order is placed.": "Öğe, sipariş verildikten sonra paketlenir.",
  "KB": "KB",
  "Keep": "Sakla",
  "Labels": "Etiketler",
  "Labels & Tags": "Etiketler ve Etiketler",
  "Leave shop": "Mağazadan ayrıl",
  "Link": "Bağlantı",
  "Linked product": "Bağlı ürün",
  "Loading files...": "Dosyalar yükleniyor...",
  "Loading images...": "Görseller yükleniyor...",
  "Loading product…": "Ürün yükleniyor…",
  "Loading stories...": "Hikayeler yükleniyor...",
  "Luck": "Şans",
  "Luck Price": "Şans Fiyatı",
  "MB": "MB",
  "Manage team members and roles": "Ekip üyelerini ve rolleri yönetin",
  "Max Allowed Qty": "İzin Verilen Maks. Adet",
  "Member": "Üye",
  "Meta Description": "Meta Açıklaması",
  "Meta Image": "Meta Görseli",
  "Meta Title": "Meta Başlığı",
  "Model Number": "Model Numarası",
  "Multiply Shipping × Quantity": "Kargoyu × Adet ile Çarp",
  "Must be ≤ unit price": "Birim fiyattan ≤ olmalıdır",
  "Must stay unique across the marketplace": "Pazar yerinde benzersiz kalmalıdır",
  "N/A": "N/A",
  "New video ready": "Yeni video hazır",
  "Next": "İleri",
  "No Image": "Görsel Yok",
  "No banner yet": "Henüz afiş yok",
  "No changes to save.": "Kaydedilecek değişiklik yok.",
  "No comments found.": "Yorum bulunamadı.",
  "No description": "Açıklama yok",
  "No files uploaded yet.": "Henüz dosya yüklenmedi.",
  "No image": "Görsel yok",
  "No images found": "Görsel bulunamadı",
  "No items": "Öğe yok",
  "No notes available.": "Mevcut not yok.",
  "No options available for the current selection.": "Mevcut seçim için seçenek yok.",
  "No order selected": "Sipariş seçilmedi",
  "No per-country surcharges.": "Ülkeye özel ek ücret yok.",
  "No permissions assigned": "Atanmış izin yok",
  "No remaining time": "Kalan süre yok",
  "No roles available": "Mevcut rol yok",
  "No users found": "Kullanıcı bulunamadı",
  "No video attached.": "Ekli video yok.",
  "Notes": "Notlar",
  "Optional. Upload a new product video or remove existing ones.": "İsteğe bağlı. Yeni bir ürün videosu yükleyin veya mevcut olanları kaldırın.",
  "Origin & Countries": "Menşe ve Ülkeler",
  "Out of stock": "Stokta yok",
  "Packed": "Paketlendi",
  "Packed After Ordering": "Sipariş Sonrası Paketlenir",
  "Page": "Sayfa",
  "Pending Approval": "Onay Bekliyor",
  "Per-country Extra Price": "Ülkeye Özel Ek Fiyat",
  "Percent": "Yüzde",
  "Photo or video, up to": "Fotoğraf veya video, en fazla",
  "Pick a shop to open its dashboard.": "Panelini açmak için bir mağaza seçin.",
  "Pick colors & sizes, then set price and stock for each combination.": "Renkleri ve bedenleri seçin, ardından her kombinasyon için fiyat ve stok belirleyin.",
  "Pieces / Unit": "Adet / Birim",
  "More problems were reported": "Daha fazla sorun bildirildi",
  "Please fix the highlighted fields before saving.": "Kaydetmeden önce lütfen vurgulanan alanları düzeltin.",
  "Preparing your dashboard...": "Paneliniz hazırlanıyor...",
  "Previous": "Geri",
  "Prices are in your display currency; converted server-side.": "Fiyatlar görüntüleme para biriminizdedir; sunucu tarafında dönüştürülür.",
  "Pricing & Stock": "Fiyatlandırma ve Stok",
  "Product Name": "Ürün Adı",
  "Product updated successfully.": "Ürün başarıyla güncellendi.",
  "Products added to this shop will appear here.": "Bu mağazaya eklenen ürünler burada görünecek.",
  "Purchasable": "Satın Alınabilir",
  "Purchase Price": "Satın Alma Fiyatı",
  "Questions": "Sorular",
  "Ratio 6:1": "Oran 6:1",
  "Reactions": "Tepkiler",
  "Read only": "Salt okunur",
  "Ready To Collect": "Teslim Almaya Hazır",
  "Refresh": "Yenile",
  "Reply to FQA Comment": "SSS Yorumuna Yanıt Ver",
  "Reply to reviews and FAQ": "Yorumlara ve SSS'ye yanıt verin",
  "Report Ref. Number": "Rapor Ref. Numarası",
  "Required for pc / liter": "Adet / litre için gereklidir",
  "Temporarily read-only": "Geçici olarak salt okunur",
  "Must be a whole number between 1 and 100": "1 ile 100 arasında bir tam sayı olmalıdır",
  "Restricted Countries": "Kısıtlı Ülkeler",
  "Review your access in this shop": "Bu mağazadaki erişiminizi gözden geçirin",
  "Review your changes before saving.": "Kaydetmeden önce değişikliklerinizi gözden geçirin.",
  "Reviews": "Yorumlar",
  "SEO / Meta": "SEO / Meta",
  "SKU": "SKU",
  "Save Changes": "Değişiklikleri Kaydet",
  "Saving...": "Kaydediliyor...",
  "Saving…": "Kaydediliyor…",
  "Select Files": "Dosya Seç",
  "Select Folder": "Klasör Seç",
  "Select a role": "Bir rol seçin",
  "Select all": "Tümünü seç",
  "Select images to delete": "Silinecek görselleri seçin",
  "Seller Product ID": "Satıcı Ürün ID'si",
  "Share photo & video stories": "Fotoğraf ve video hikayeleri paylaşın",
  "Shares": "Paylaşımlar",
  "Shipping Days": "Kargo Günleri",
  "Shop Banner": "Mağaza Afişi",
  "Shop Logo": "Mağaza Logosu",
  "Shops you can manage will appear here.": "Yönetebileceğiniz mağazalar burada görünecek.",
  "Show notes": "Notları göster",
  "Status": "Durum",
  "Status updated.": "Durum güncellendi.",
  "Storefronts under this shop": "Bu mağazaya ait vitrinler",
  "Sub-categories shown match the current parents (re-open the page after changing parents to load others).": "Gösterilen alt kategoriler mevcut üst kategorilerle eşleşir (diğerlerini yüklemek için üst kategorileri değiştirdikten sonra sayfayı yeniden açın).",
  "Tags": "Etiketler",
  "Tax Type": "Vergi Türü",
  "The first image is the cover. Drag order with the arrows.": "İlk görsel kapaktır. Sıralamayı oklarla sürükleyin.",
  "The product is restricted to the selected countries.": "Ürün seçili ülkelerle sınırlandırılmıştır.",
  "The product will be hidden from purchase until you re-enable it.": "Yeniden etkinleştirene kadar ürün satın almadan gizlenecek.",
  "These fields will be updated": "Bu alanlar güncellenecek",
  "Seller Product ID is required": "Satıcı Ürün ID'si gereklidir",
  "This Seller Product ID is already used": "Bu Satıcı Ürün ID'si zaten kullanılıyor",
  "This will remove your access to the shop dashboard. You can be re-invited later.": "Bu, mağaza paneline erişiminizi kaldıracak. Daha sonra tekrar davet edilebilirsiniz.",
  "Title": "Başlık",
  "Track and fulfil customer orders": "Müşteri siparişlerini takip edin ve karşılayın",
  "Translations": "Çeviriler",
  "USD": "USD",
  "Unit": "Birim",
  "Unit Price": "Birim Fiyat",
  "Unnamed Boutique": "Adsız Butik",
  "Unnamed Product": "Adsız Ürün",
  "Untitled Product": "Başlıksız Ürün",
  "Upload Failed Try Again": "Yükleme Başarısız Tekrar Deneyin",
  "Upload Video": "Video Yükle",
  "Upload and reuse product images": "Ürün görsellerini yükleyin ve yeniden kullanın",
  "Upload failed": "Yükleme başarısız",
  "Upload images first.": "Önce görselleri yükleyin.",
  "Uploaded Excel Files": "Yüklenen Excel Dosyaları",
  "Uploaded product images will appear here.": "Yüklenen ürün görselleri burada görünecek.",
  "Uploading…": "Yükleniyor…",
  "Variants": "Varyantlar",
  "Video upload failed": "Video yükleme başarısız",
  "View": "Görüntüle",
  "View only": "Yalnızca görüntüle",
  "Height": "Boy",
  "Weight": "Ağırlık",
  "Welcome back": "Tekrar hoş geldiniz",
  "You don't have permission to view boutiques": "Butikleri görüntüleme izniniz yok",
  "You don't have permission to view or edit this product.": "Bu ürünü görüntüleme veya düzenleme izniniz yok.",
  "You need order viewing permissions to see this section": "Bu bölümü görmek için sipariş görüntüleme izinlerine ihtiyacınız var",
  "You're viewing this product. Tap Edit to make changes.": "Bu ürünü görüntülüyorsunuz. Değişiklik yapmak için Düzenle'ye dokunun.",
  "Your Shops": "Mağazalarınız",
  "Your role in this shop": "Bu mağazadaki rolünüz",
  "Your shop": "Mağazanız",
  "files selected": "dosya seçildi",
  "images": "görseller",
  "in stock": "stokta",
  "max 3": "en fazla 3",
  "of": "/",
  "or choose files / folder": "veya dosya / klasör seçin",
  "permissions": "izinler",
  "selected": "seçildi",
  "this shop": "bu mağaza",
  "Product Quality": "Ürün Kalitesi",
  "Damaged": "Hasarlı",
  "Not as described": "Açıklandığı gibi değil",
  "Poor material / quality": "Kötü malzeme / kalite",
  "Wrong item received": "Yanlış ürün alındı",
  "Expired / spoiled": "Süresi dolmuş / bozulmuş",
  "Delivery Time": "Teslimat Süresi",
  "Arrived too late": "Çok geç geldi",
  "Missed the delivery window": "Teslimat zaman aralığı kaçırıldı",
  "No clear ETA": "Net bir tahmini varış süresi yok",
  "Faster than expected": "Beklenenden daha hızlı",
  "Delivery Worker": "Teslimat Görevlisi",
  "Rude behavior": "Kaba davranış",
  "Unprofessional": "Profesyonel olmayan",
  "Did not show up": "Gelmedi",
  "Asked for an extra fee": "Ekstra ücret talep etti",
  "Polite & helpful": "Kibar ve yardımcı",
  "Delivery Car": "Teslimat Aracı",
  "Dirty vehicle": "Kirli araç",
  "No cooling / improper temperature": "Soğutma yok / uygun olmayan sıcaklık",
  "Unsafe handling": "Güvensiz taşıma",
  "No proper vehicle": "Uygun araç yok",
  // Hidden Orders widget
  "Action About Your Orders": "Siparişleriniz Hakkında İşlem",
  "Hidden Orders": "Gizli Siparişler",
  "See orders and products you hid": "Gizlediğiniz sipariş ve ürünleri görün",
  "These are orders and products you hid from your history. Tap the eye to restore any of them back to your list.": "Bunlar geçmişinizden gizlediğiniz sipariş ve ürünlerdir. Herhangi birini listenize geri getirmek için göz simgesine dokunun.",
  "You have no hidden orders.": "Gizli siparişiniz yok.",
  "Restore This Order": "Bu Siparişi Geri Yükle",
  "Restore This Product": "Bu Ürünü Geri Yükle",
  "Are you sure you want to restore this order to your list?": "Bu siparişi listenize geri yüklemek istediğinizden emin misiniz?",
  "Are you sure you want to restore this product to your list?": "Bu ürünü listenize geri yüklemek istediğinizden emin misiniz?",
  "Boutique details": "Butik detayları",
  "Shown across the storefront.": "Mağaza genelinde gösterilir.",
  "Boutique name": "Butik adı",
  Availability: "Kullanılabilirlik",
  Web: "Web",
  Mobile: "Mobil",
  "Web + Mobile": "Web + Mobil",
  "Choose where this boutique is available.": "Bu butiğin nerede kullanılabilir olacağını seçin.",
  "Boutique icon": "Butik simgesi",
  "Upload icon": "Simge yükle",
  "Available in countries": "Ülkelerde kullanılabilir",
  "Leave empty to make the boutique available everywhere.": "Butiği her yerde kullanılabilir yapmak için boş bırakın.",
  "Name, description and bio per language. English is required.": "Her dil için ad, açıklama ve biyografi. İngilizce gereklidir.",
  Bio: "Biyografi",
  Banner: "Afiş",
  Banners: "Afişler",
  "Add banner": "Afiş ekle",
  "Move left": "Sola taşı",
  "Move right": "Sağa taşı",
  "Loading boutique…": "Butik yükleniyor…",
  "Boutique not found.": "Butik bulunamadı.",
  "Failed to load boutique.": "Butik yüklenemedi.",
  "You don't have permission to view or edit this boutique.": "Bu butiği görüntüleme veya düzenleme izniniz yok.",
  "Set active": "Aktif yap",
  "Set inactive": "Pasif yap",
  "Boutique updated successfully.": "Butik başarıyla güncellendi.",
  "Failed to update boutique.": "Butik güncellenemedi.",
  "Could not change status.": "Durum değiştirilemedi.",
  "Your changes were saved, but the status could not be updated.": "Değişiklikleriniz kaydedildi ancak durum güncellenemedi.",
  "Status could not be changed:": "Durum değiştirilemedi:",
  "Image upload failed.": "Görsel yüklenemedi.",
  "Upload returned no file.": "Yükleme hiçbir dosya döndürmedi.",
  "Name is required.": "Ad gereklidir.",
  "Icon is required.": "Simge gereklidir.",
  "English name is required.": "İngilizce ad gereklidir.",
  "Choose an availability.": "Bir kullanılabilirlik seçin.",
  "Please choose an image file.": "Lütfen bir görsel dosyası seçin.",
  "Banner image must be 10 MB or smaller.": "Afiş görseli 10 MB veya daha küçük olmalıdır.",
  "Icon image must be 10 MB or smaller.": "Simge görseli 10 MB veya daha küçük olmalıdır.",
  "This banner may not display well": "Bu afiş iyi görüntülenmeyebilir",
  "For best results use a banner of": "En iyi sonuç için şu boyutta bir afiş kullanın:",
  "Keep important content centered — the storefront may crop the top and bottom.": "Önemli içeriği ortada tutun — mağaza üst ve altı kırpabilir.",
  "Your image is": "Görseliniz",
  "Ignore & upload": "Yoksay ve yükle",
  "Recommended size": "Önerilen boyut",
  "Used across all languages.": "Tüm dillerde kullanılır.",
  "New Boutique": "Yeni Butik",
  "Add Boutique": "Butik Ekle",
  "Add your first boutique": "İlk butiğinizi ekleyin",
  "Fill in the details and create your boutique.": "Ayrıntıları doldurun ve butiğinizi oluşturun.",
  "Create Boutique": "Butik Oluştur",
  "Boutique created successfully.": "Butik başarıyla oluşturuldu.",
  "Failed to create boutique.": "Butik oluşturulamadı.",
  "Delete boutique": "Butiği sil",
  "Delete this boutique?": "Bu butik silinsin mi?",
  "This boutique will be permanently removed from your shop and can't be undone.":
    "Bu butik mağazanızdan kalıcı olarak kaldırılacak ve bu işlem geri alınamaz.",
  "Boutique deleted successfully.": "Butik başarıyla silindi.",
  "Failed to delete boutique.": "Butik silinemedi.",
  "Failed to load boutique form.": "Butik formu yüklenemedi.",
  Translations: "Çeviriler",
  "Name, description, bio, icon and banners per language. English is required.":
    "Her dil için ad, açıklama, biyografi, simge ve afişler. İngilizce zorunludur.",
  "Copy from": "Şuradan kopyala",
  "Copy from another language": "Başka bir dilden kopyala",
  "No countries available.": "Ülke bulunmuyor.",
  "Description is required.": "Açıklama gereklidir.",
  "Bio is required.": "Biyografi gereklidir.",
  "At least one banner is required.": "En az bir afiş gereklidir.",
  "Restricted countries": "Kısıtlı ülkeler",
  "The boutique is restricted to the selected countries. Leave empty to make it available everywhere.":
    "Butik seçilen ülkelerle sınırlıdır. Her yerde kullanılabilir olması için boş bırakın.",
  "Name, description, bio, icon and at least one banner are required for every language.":
    "Her dil için ad, açıklama, biyografi, simge ve en az bir afiş gereklidir.",
  // Seller product editor
  "Add Image": "Resim Ekle",
  "Add selected": "Seçileni ekle",
  "An English (en) name is required": "İngilizce (en) bir ad gereklidir",
  "At least one product image is required": "En az bir ürün resmi gereklidir",
  "At most 3 labels allowed": "En fazla 3 etiket izinlidir",
  "Attributes": "Öznitelikler",
  "Choose from gallery": "Galeriden seç",
  "Color → Image Assignment": "Renk → Resim Ataması",
  "Colors": "Renkler",
  "Confirm new product": "Yeni ürünü onayla",
  "Country": "Ülke",
  "Description": "Açıklama",
  "Descriptors": "Tanımlayıcılar",
  "Discount must be ≤ unit price": "İndirim ≤ birim fiyat olmalıdır",
  "Edit": "Düzenle",
  "edited": "düzenlendi",
  "Enter a valid discount price": "Geçerli bir indirim fiyatı girin",
  "Enter a valid purchase price": "Geçerli bir satın alma fiyatı girin",
  "Enter a valid stock": "Geçerli bir stok girin",
  "Enter a valid weight": "Geçerli bir ağırlık girin",
  "Enter a valid unit price": "Geçerli bir birim fiyat girin",
  "Every color needs at least one image": "Her renk için en az bir resim gerekir",
  "Every image must be assigned to a color": "Her resim bir renge atanmalıdır",
  "Every variant needs a location": "Her varyant için bir konum gereklidir",
  "Every variant needs a quantity": "Her varyant için bir miktar gerekir",
  "Every variant needs an SKU": "Her varyant için bir SKU gerekir",
  "Failed to create product": "Ürün oluşturulamadı",
  "Failed to load gallery": "Galeri yüklenemedi",
  "Fill in the details and create your product.": "Ayrıntıları doldurun ve ürününüzü oluşturun.",
  "image(s)": "resim",
  "item(s)": "öğe",
  "Label": "Etiket",
  "Location is required": "Konum gereklidir",
  "Unit price must be greater than discount price": "Birim fiyatı indirimli fiyattan büyük olmalıdır",
  "Discount price must be greater than purchase price": "İndirimli fiyat satın alma fiyatından büyük olmalıdır",
  "Unit price must be greater than purchase price": "Birim fiyatı satın alma fiyatından büyük olmalıdır",
  "language(s)": "dil",
  "Loading…": "Yükleniyor…",
  "Location": "Konum",
  "Main Categories": "Ana Kategoriler",
  "missing": "eksik",
  "Multiply Shipping × Qty": "Kargo × Adet çarp",
  "New Product": "Yeni Ürün",
  "new upload": "yeni yükleme",
  "No images in your gallery yet.": "Galerinizde henüz resim yok.",
  "Off": "Kapalı",
  "On": "Açık",
  "Origin Country": "Menşe Ülke",
  "Product created successfully.": "Ürün başarıyla oluşturuldu.",
  "Product Images": "Ürün Resimleri",
  "Product name is required": "Ürün adı gereklidir",
  "Brand is required": "Marka gereklidir",
  "Qty": "Adet",
  "Remove": "Kaldır",
  "removed": "kaldırıldı",
  "Report Ref.": "Rapor Ref.",
  "Retry": "Yeniden dene",
  "Select": "Seç",
  "Select a category to see its attributes.": "Özniteliklerini görmek için bir kategori seçin.",
  "Select a valid unit": "Geçerli bir birim seçin",
  "Select an image.": "Bir resim seçin.",
  "Select one or more images.": "Bir veya daha fazla resim seçin.",
  "Seller product ID is required": "Satıcı ürün kimliği gereklidir",
  "set": "ayarlı",
  "Set attribute values for the selected categories. All optional.": "Seçili kategoriler için öznitelik değerlerini ayarlayın. Tümü isteğe bağlı.",
  "Sizes": "Bedenler",
  "Stock": "Stok",
  "Sub Categories": "Alt Kategoriler",
  "Sub-sub Categories": "Alt-alt Kategoriler",
  "Tax": "Vergi",
  "These details will be saved": "Bu ayrıntılar kaydedilecek",
  "unassigned": "atanmamış",
  "updated": "güncellendi",
  "Upload from device": "Cihazdan yükle",
  "Variant Pricing / Stock": "Varyant Fiyat / Stok",
  "variant(s)": "varyant",
  "Weight is required for pc / liter units": "Adet / litre birimleri için ağırlık gereklidir",
  // ---- i18n audit fixes ----
  "Failed to load wallet currencies": "Cüzdan para birimleri yüklenemedi",
  "Failed to load wallet data": "Cüzdan verileri yüklenemedi",
  "Failed to load wallet balance": "Cüzdan bakiyesi yüklenemedi",
  "Insufficient wallet balance": "Yetersiz cüzdan bakiyesi",
  "Wallet payment failed. Please try again": "Cüzdan ödemesi başarısız oldu. Lütfen tekrar deneyin",
  "Wallet Payment": "Cüzdan Ödemesi",
  "Select Currency": "Para Birimi Seçin",
  "Amount to Pay": "Ödenecek Tutar",
  "Insufficient wallet balance for this payment": "Bu ödeme için yetersiz cüzdan bakiyesi",
  "Confirm Wallet Payment": "Cüzdan Ödemesini Onayla",
  "Back to your wallet": "Cüzdanınıza dönün",
  "Order processing timed out. Please check your orders": "Sipariş işleme zaman aşımına uğradı. Lütfen siparişlerinizi kontrol edin",
  "Add To Bag Within": "Şu süre içinde sepete ekle",
  "Seconds": "Saniye",
  "Only": "Sadece",
  "Soon": "Yakında",
  "Province | District | Town | Street": "İl | İlçe | Kasaba | Sokak",
  "Search Province | District | Town | Street": "İl | İlçe | Kasaba | Sokak ara",
  "Select From List": "Listeden Seçin",
  "Error getting your location": "Konumunuz alınırken hata oluştu",
  "Geolocation is not supported by your browser": "Tarayıcınız coğrafi konumu desteklemiyor",
  "Your Address List Is Empty": "Adres listeniz boş",
  "Select Your Region": "Bölgenizi Seçin",
  "Switching Country...": "Ülke değiştiriliyor...",
  "Couldn't load your request": "İsteğiniz yüklenemedi",
  "We couldn't load your seller request status. Please try again.": "Satıcı talebinizin durumu yüklenemedi. Lütfen tekrar deneyin.",
  "Your seller request is under review": "Satıcı talebiniz inceleniyor",
  "We've received your seller request and it's currently being reviewed. We'll let you know once it's processed.": "Satıcı talebinizi aldık ve şu anda inceleniyor. İşlem tamamlandığında sizi bilgilendireceğiz.",
  "Your seller request was rejected": "Satıcı talebiniz reddedildi",
  "Unfortunately, your seller request was not approved.": "Maalesef, satıcı talebiniz onaylanmadı.",
  "Your seller request was approved": "Satıcı talebiniz onaylandı",
  "Your seller request has been approved. You can now start selling on Trydos.": "Satıcı talebiniz onaylandı. Artık Trydos'ta satış yapmaya başlayabilirsiniz.",
  "Sales": "Satışlar",
  "Add your first product": "İlk ürününüzü ekleyin",
  "Add Product": "Ürün Ekle",
  "Boutique not found": "Butik bulunamadı",
  "Position your product here": "Ürününüzü buraya yerleştirin",
  "Please Verify Your Phone Number": "Lütfen telefon numaranızı doğrulayın",
  "User Name": "Kullanıcı Adı",
  "Enter Full User Name": "Tam kullanıcı adını girin",
  "Full name is required": "Ad soyad gereklidir",
  "Please select your gender": "Lütfen cinsiyetinizi seçin",
  // ---- i18n audit fixes ----
  "cancel": "İptal",
  "Your Message": "Mesajınız",
  "Edt": "Düzenle",
  "Link copied to clipboard!": "Bağlantı panoya kopyalandı!",
  "Telegram": "Telegram",
  "Failed to Upload file": "Dosya yüklenemedi",
  "Failed to send message": "Mesaj gönderilemedi",
  "Please enable camera permissions to use camera features": "Kamera özelliklerini kullanmak için lütfen kamera izinlerini etkinleştirin",
  "Failed to Upload audio": "Ses yüklenemedi",
  "No available Microphone": "Kullanılabilir mikrofon yok",
  "Cookies Is Not Enabled": "Çerezler etkin değil",
  "Show OTP Statics": "OTP İstatistiklerini Göster",
  "failed to search with image": "Görselle arama başarısız oldu",
  "Take photo with camera": "Kamerayla fotoğraf çek",
  "Choose file from device": "Cihazdan dosya seçin",
  "a device": "bir cihaz",
  "Log in on this device?": "Bu cihazda oturum açılsın mı?",
  "Deny": "Reddet",
  "Approve": "Onayla",
  "Camera unavailable — allow camera access to scan": "Kamera kullanılamıyor — taramak için kamera erişimine izin verin",
  "Scan to sign in": "Oturum açmak için tarayın",
  "Point at the QR on your other screen": "Kamerayı diğer ekranınızdaki QR koduna doğrultun",
  "Failed To Upload Image..Try Again": "Görsel yüklenemedi.. Tekrar deneyin",
  "Please Login First": "Lütfen önce giriş yapın",
  "Removed from checklist": "Kontrol listesinden kaldırıldı",
  "Failed to remove from checklist": "Kontrol listesinden kaldırılamadı",
  "Failed to process image": "Görsel işlenemedi",
  "This device has only one camera": "Bu cihazda yalnızca bir kamera var",
  "Camera constraints not supported, trying fallback settings": "Kamera kısıtlamaları desteklenmiyor, yedek ayarlar deneniyor",
  "Camera access denied": "Kamera erişimi reddedildi",
  "No camera found": "Kamera bulunamadı",
  "Camera error occurred": "Kamera hatası oluştu",
  "Close camera": "Kamerayı kapat",
  "Photo captured!": "Fotoğraf çekildi!",
  "Retake": "Yeniden çek",
  "Use Photo": "Fotoğrafı kullan",
  "Switch camera": "Kamerayı değiştir",
  "Capture photo": "Fotoğraf çek",
  "Comment input": "Yorum girişi",
  "Quick Video": "Hızlı Video",
  "Please choose a JPEG, PNG or WebP image": "Lütfen JPEG, PNG veya WebP görseli seçin",
  "The photo must be 4 MB or less": "Fotoğraf 4 MB veya daha küçük olmalıdır",
  "Add a photo": "Fotoğraf ekle",
  "Report photo": "Fotoğrafı bildir",
  "Scan QR to sign in": "Giriş yapmak için QR kodunu tarayın",
  "Only image or PDF files are allowed": "Yalnızca görsel veya PDF dosyalarına izin verilir",
  "Only image and video files are allowed": "Yalnızca resim ve video dosyalarına izin verilir",
  "Notification is Denied": "Bildirim reddedildi",
  "User in Another Call": "Kullanıcı başka bir görüşmede",
  "New message": "Yeni mesaj",
  "Loading": "Yükleniyor",
  "Failed to update profile Info": "Profil bilgileri güncellenemedi",
  "Failed to update name": "Ad güncellenemedi",
  "Failed to send verification code": "Doğrulama kodu gönderilemedi",
  "Failed to Initialize Call": "Görüşme başlatılamadı",
  "Contacts API not supported on this browser": "Kişiler API'si bu tarayıcıda desteklenmiyor",
  "Add Contact Manually": "Kişiyi elle ekle",
  // ---- i18n audit fixes ----
  "Ex: Home, My Office, 2 Home Ect.": "Örn: Ev, Ofisim, 2. Ev vb.",
  "Enter Full Recipient Name": "Alıcının tam adını girin",
  "Contact Phone": "İletişim Telefonu",
  "Enter Recipient Phone": "Alıcı telefonunu girin",
  "Alternative Phone": "Alternatif Telefon",
  "Enter Alternative Recipient Phone": "Alternatif alıcı telefonunu girin",
  "Ship To You Accepted": "Size teslimat kabul edildi",
  "Piece": "Adet",
  "Availabilty": "Stok Durumu",
  "Out Of Stock": "Stokta Yok",
  "Out Of Bag!": "Sepetten Çıkarıldı!",
  "Time Running Out.": "Süre doluyor",
  "Add Again?": "Tekrar Ekle?",
  "Hide All": "Tümünü Gizle",
  "Hide": "Gizle",
  "Cash On Delivery": "Kapıda Ödeme",
  "Your Balance": "Bakiyeniz",
  "Credit Cards": "Kredi Kartları",
  "Crypto": "Kripto",
  "Coupon No": "Kupon No",
  "You Can Change Size, Color, Qty Of  The Product Without Any Conditions According To The Change Policy": "Değişim politikasına göre ürünün bedenini, rengini ve adedini herhangi bir koşul olmadan değiştirebilirsiniz",
  "Return Is Completely Free Without Any Extras": "İade, herhangi bir ek ücret olmadan tamamen ücretsizdir",
  "Languages": "Diller",
  // ---- i18n audit fixes ----
  "You Have New Messages From Delivery Worker": "Teslimat Elemanından Yeni Mesajlarınız Var",
  "Sorry This Product Not Available In Your Country": "Üzgünüz, bu ürün ülkenizde mevcut değil",
  "Sorry This Product Not Available Now": "Üzgünüz, bu ürün şu anda mevcut değil",
  "Not Available In Your Country": "Ülkenizde mevcut değil",
  "Not Available Now": "Şu anda mevcut değil",
  "These Shots Are Made By Users Who Have Already Purchased And Received The Product": "Bu görüntüler, ürünü satın alıp teslim almış kullanıcılar tarafından çekilmiştir",
  "Trydos Wallet": "Trydos Cüzdanı",
  "I Have Discount Coupon": "İndirim kuponum var",
  "color,": "renk,",
  "color": "renk",
  "Gift": "Hediye",
  "First Shopping": "İlk Alışveriş",
  "In Lebanon": "Lübnan'da",
  "Notify Me": "Bana Bildir",
  "m": "M",
  "k": "B",
  "Notification Premission:": "Bildirim İzni:",
  "Product is Shared Successfully": "Ürün Başarıyla Paylaşıldı",
  "Product Share error": "Ürün paylaşım hatası",
  "To": "Şuraya",
  "Choose The Verification Method, Receive Code Via": "Doğrulama Yöntemini Seçin, Kodu Şu Yöntemle Alın",
  "Tap \" Agree & Continue\" To Accept trydos": "trydos'u kabul etmek için \" Kabul Et ve Devam Et\"e dokunun",
  "Enabled Notifications Topic": "Etkinleştirilen Bildirim Konusu",
  "Disabled Notifications Topic": "Devre Dışı Bırakılan Bildirim Konusu",
  "No topics subscribed": "Abone olunan konu yok",
  "notifications subscription": "Bildirim aboneliği",
  "Pick Your Delivery Location Inside Your Country": "Ülkenizdeki teslimat konumunu seçin",
  // ---- i18n audit fixes ----
  "minutes": "dakika",
  "Errors": "Hatalar",
  "Edit Shipping Address": "Teslimat Adresini Düzenle",
  "Verify": "Doğrula",
  "Change ": "Değiştir ",
  "Found on a device — approve on your phone":
    "Bir cihazda bulundu — telefonunuzdan onaylayın",
  "Approved (demo)": "Onaylandı (demo)",
  "Request declined": "İstek reddedildi",
  "Code expired — reopen to refresh":
    "Kodun süresi doldu — yenilemek için tekrar açın",
  "Simulate scan (dev)": "Taramayı simüle et (geliştirici)",
  "Too many verification requests, please try again later":
    "Çok fazla doğrulama isteği, lütfen daha sonra tekrar deneyin",
  "We Recommend That You Complete Your Profile To Make The Most Of The App’s Features, Such As Shopping, Chatting, Stories, Taking Advantage Of Offers, Interests, And Much More":
    "Alışveriş, sohbet, hikayeler, tekliflerden yararlanma, ilgi alanları ve çok daha fazlası gibi uygulamanın özelliklerinden en iyi şekilde yararlanmak için profilinizi tamamlamanızı öneririz",
  "Added to checklist": "Kontrol listesine eklendi",
  "Removed From Compare": "Karşılaştırmadan kaldırıldı",
  "please select one contact at least": "lütfen en az bir kişi seçin",
  "Boutique is required": "Butik gereklidir",
  "Select at least one category": "En az bir kategori seçin",
  "Description is required": "Açıklama gereklidir",
  "Please check this field": "Lütfen bu alanı kontrol edin",
  "Check the product images and their order": "Ürün görsellerini ve sıralamasını kontrol edin",
  "Every color needs at least one image, and every image must be assigned": "Her renk en az bir görsel gerektirir ve her görsel bir renge atanmalıdır",
  "Attributes are not editable yet. This section is a preview and nothing here is saved.": "Özellikler henüz düzenlenemiyor. Bu bölüm bir önizlemedir ve burada hiçbir şey kaydedilmez.",
  "Share": "Paylaş",
  "SKU must be unique within the product": "SKU aynı ürün içinde benzersiz olmalıdır",
  "SKU must be unique": "SKU benzersiz olmalıdır",
  "Variation SKUs must be unique within the product": "Varyant SKU'ları ürün içinde benzersiz olmalıdır",
  "Save Draft": "Taslak Kaydet",
  "Load Draft": "Taslak Yükle",
  "Draft saved successfully.": "Taslak başarıyla kaydedildi.",
  "No saved draft found.": "Kaydedilmiş taslak bulunamadı.",
  "Draft loaded successfully.": "Taslak başarıyla yüklendi.",
  "Failed to save draft.": "Taslak kaydedilemedi.",
  "Failed to load draft.": "Taslak yüklenemedi.",
  "Variant quantity cannot be negative": "Varyant miktarı negatif olamaz",
  "Variant price cannot be negative": "Varyant fiyatı negatif olamaz",
  "Variant discount price cannot be negative": "Varyant indirim fiyatı negatif olamaz",
  "Variant luck price cannot be negative": "Varyant şans fiyatı negatif olamaz",
  "Luck price cannot be greater than unit price": "Şans fiyatı birim fiyattan büyük olamaz",
  "Luck price cannot be greater than discount price": "Şans fiyatı indirimli fiyattan büyük olamaz",
  "Variant luck price cannot be greater than unit price": "Varyant şans fiyatı birim fiyattan büyük olamaz",
  "Variant luck price cannot be greater than discount price": "Varyant şans fiyatı indirimli fiyattan büyük olamaz",
  "Product name is required for": "Ürün adı şu dil için gereklidir:",
  "Description is required for": "Açıklama şu dil için gereklidir:",
  "original": "orijinal",
  "Must be 0 or greater": "0 veya daha büyük olmalıdır",
  "Auto-calculated from variations": "Varyantlardan otomatik hesaplanır",
  "Default Language": "Varsayılan Dil",
  "Select default language": "Varsayılan dili seçin",
  "Select the primary language for this product. You can enter details for this language now and other languages will be translated automatically.": "Bu ürün için birincil dili seçin. Şimdi bu dil için ayrıntıları girebilirsiniz, diğer diller otomatik olarak çevrilecektir.",
  "Similar Words": "Benzer Kelimeler",
  "Type a word and press Enter": "Bir kelime yazın ve Enter'a basın",
  "Add": "Ekle",
  "Attribute": "Öznitelik",
  "Product updated, but attributes failed to save.": "Ürün güncellendi ancak öznitelikler kaydedilemedi.",
  // Seller dashboard — Locations section
  "Locations": "Konumlar",
  "Add Location": "Konum Ekle",
  "Edit Location": "Konumu Düzenle",
  "Address": "Adres",
  "Latitude": "Enlem",
  "Longitude": "Boylam",
  "No locations found": "Konum bulunamadı",
  "Locations added to this shop will appear here.": "Bu mağazaya eklenen konumlar burada görünecek.",
  "Add your first location": "İlk konumunuzu ekleyin",
  "Warehouses and pickup points for this shop": "Bu mağazanın depoları ve teslim alma noktaları",
  "You don't have permission to view locations": "Konumları görüntüleme yetkiniz yok",
  "Failed to load locations": "Konumlar yüklenemedi",
  "Failed to save location": "Konum kaydedilemedi",
  "Failed to change location status": "Konum durumu değiştirilemedi",
  "Location created successfully": "Konum başarıyla oluşturuldu",
  "Location updated successfully": "Konum başarıyla güncellendi",
  "Status changed successfully": "Durum başarıyla değiştirildi",
  "Pick the position on the map": "Konumu haritadan seçin",
  "Use my current location": "Mevcut konumumu kullan",
  "Location name": "Konum adı",
  "All statuses": "Tüm durumlar",
  "All countries": "Tüm ülkeler",
  "Deactivate": "Devre dışı bırak",
  "Activate": "Etkinleştir",
  "Name is required": "Ad gereklidir",
  "Country is required": "Ülke gereklidir",
  "Latitude must be between -90 and 90": "Enlem -90 ile 90 arasında olmalıdır",
  "Longitude must be between -180 and 180": "Boylam -180 ile 180 arasında olmalıdır",
  "Map is unavailable": "Harita kullanılamıyor",
  "Coordinates are optional": "Koordinatlar isteğe bağlıdır",
  // Product Editor & Changes Widget
  "Removed": "Kaldırıldı",
  "Updated": "Güncellendi",
  "New Images Added": "Yeni Görseller Eklendi",
  "Removed Images": "Kaldırılan Görseller",
  "Updated Gallery Preview": "Güncellenen Galeri Önizlemesi",
  "Original Colors": "Orijinal Renkler",
  "New Selection": "Yeni Seçim",
  "languages": "diller",
  "variants": "varyasyonlar",
  "countries": "ülkeler",
  "Attributes / Descriptors": "Özellikler / Tanımlayıcılar",
  "Stock Qty": "Stok Miktarı",
  "Variant Pricing & Stock": "Varyasyon Fiyatlandırması ve Stok",
  "Keywords": "Anahtar Kelimeler",
  "Keywords / Search Terms": "Anahtar Kelimeler / Arama Terimleri",
  "Default Language Code": "Varsayılan Dil Kodu",
  "New video file uploaded": "Yeni video dosyası yüklendi",
  "Removed Videos": "Kaldırılan Videolar",
  "video(s)": "video",
  "Select default language for product creation. Other languages will be translated automatically.": "Ürün oluşturma için varsayılan dili seçin. Diğer diller otomatik olarak çevrilecektir.",
  "Notifications Not Supported": "Bildirimler Desteklenmiyor",
  "Push notifications are not supported by your current browser or mode. To access features that depend on notifications, such as order updates and live chat, please switch to a supported browser like Chrome, Edge, or Safari.": "Anlık bildirimler mevcut tarayıcınız veya modunuz tarafından desteklenmiyor. Sipariş güncellemeleri ve canlı sohbet gibi bildirimlere bağlı özellikleri kullanmak için lütfen Chrome, Edge veya Safari gibi desteklenen bir tarayıcıya geçin.",
  "Got it": "Anladım",
    "Enter a Valid Value for Quantity": "Miktar için geçerli bir değer girin",
  "Enter a Valid Value for Quantity In Variants Table": "Varyasyonlar tablosunda miktar için geçerli bir değer girin",
  "Enter a valid Count of pieces": "Geçerli bir parça sayısı girin",
  "Select a valid origin country": "Geçerli bir menşe ülkesi seçin",
  "Changes were saved": "Değişiklikler kaydedildi",
  // RDB Authentication Widget
  "Get Started !": "Başlayın !",
  "Experience fast and secure digital banking.": "Hızlı ve güvenli dijital bankacılık deneyimi.",
  "I Have an Account": "Hesabım Var",
  "New Customer": "Yeni Müşteri",
  "Maybe Later": "Belki Daha Sonra",
  "To create an account, you must agree to our Terms of Service": "Hesap oluşturmak için Hizmet Şartlarımızı kabul etmelisiniz",
  "Terms of Service": "Hizmet Şartları",
  "Agree and Continue": "Kabul Et ve Devam Et",
  "Sign Up": "Kayıt Ol",
  "Sign In": "Giriş Yap",
  "Enter your mobile number to continue": "Devam etmek için cep telefonu numaranızı girin",
  "Verification code will be sent to this number": "Doğrulama kodu bu numaraya gönderilecektir",
  "Your details are protected with bank-grade encryption": "Bilgileriniz banka düzeyinde şifrelemeyle korunmaktadır",
  "Enter Phone Number": "Telefon Numarasını Girin",
  "Select your verification method": "Doğrulama yönteminizi seçin",
  "Edit": "Düzenle",
  "WhatsApp": "WhatsApp",
  "SMS": "SMS",
  "Enter the 6-digit verification code sent via": "Şununla gönderilen 6 haneli doğrulama kodunu girin:",
  "Resend code in": "Kodu tekrar gönder:",
  "Didn't receive code?": "Kod gelmedi mi?",
  "Resend Code": "Kodu Tekrar Gönder",
  "or": "veya",
  "Change Number": "Numarayı Değiştir",
  "Change Method": "Yöntemi Değiştir",
  "Verify Your Number !": "Numaranı Doğrula !",
  "Change Your Number !": "Numaranı Değiştir !",
  "Verification code has expired": "Doğrulama kodunun süresi doldu",
  "Enter Your Name !": "Adınızı Girin !",
  "Last Step And Enjoy Our Services": "Son Adım ve Hizmetlerimizin Tadını Çıkarın",
  "Ensure Greater Security And Protect Your Funds": "Daha Fazla Güvenlik Sağlayın ve Fonlarınızı Koruyun",
  "Enter Your Name Exact ID": "Kimlikteki Adınızı Tam Olarak Girin",
  "Already Registered": "Zaten Kayıtlı",
  "This phone number is already registered with an account.": "Bu telefon numarası zaten bir hesaba kayıtlı.",
  "Log In and Continue": "Giriş Yap ve Devam Et",
  "Not Registered": "Kayıtlı Değil",
  "No account found with this phone number.": "Bu telefon numarasıyla eşleşen hesap bulunamadı.",
  "Create New Account": "Yeni Hesap Oluştur",
  // RDB Auth Exact Copy Strings
  "Get Started !": "Başlayın !",
  "To Take Advantage Of All The Advantages Of The Application, Please Join Us In Quick And Easy Steps And For Just One Time": "Uygulamanın tüm avantajlarından yararlanmak için lütfen hızlı ve kolay adımlarla sadece bir kez bize katılın",
  "I Have Already Account": "Zaten Hesabım Var",
  "Later, Take A Look At The App": "Daha Sonra, Uygulamaya Göz Atın",
  "To Create New Account Tap “Agree & Continue” To Accept": "Yeni Hesap Oluşturmak İçin \"Kabul Et ve Devam Et\" Butonuna Dokunun",
  "Terms Of Services": "Hizmet Şartları",
  "Agree & Continue": "Kabul Et ve Devam Et",
  "Sign Up !": "Kayıt Ol !",
  "Login !": "Giriş Yap !",
  "Login": "Giriş Yap",
  "Enter Your Phone Number Registered With Us": "Bizde Kayıtlı Telefon Numaranızı Girin",
  "We Will Send A Verification Code To The Number": "Numaraya Doğrulama Kodu Göndereceğiz",
  "Your Privacy Is Completely Safe": "Gizliliğiniz Tamamen Güvende",
  "Enter Your Phone Number": "Telefon Numaranızı Girin",
  "Choose Verification Method": "Doğrulama Yöntemini Seçin",
  "Send WhatsApp": "WhatsApp Gönder",
  "Send SMS": "SMS Gönder",
  "Enter Verification Code Sent To Your ": "Şuraya Gönderilen Doğrulama Kodunu Girin: ",
  "Resend After - ": "Tekrar Gönder: - ",
  "Didn't You Receive A Code?": "Kod Almadınız mı?",
  "Method": "Yöntem",
  "Or": "Veya",
  "The Code Sent Has Expired": "Gönderilen Kodun Süresi Doldu",
  "Already Registered !": "Zaten Kayıtlı !",
  "This Number Already Registered With Us": "Bu Numara Zaten Bizde Kayıtlı",
  "Login & Continue": "Giriş Yap ve Devam Et",
  "Cancel & Take A Look At The App": "İptal Et ve Uygulamaya Göz At",
  "Not Registered !": "Kayıtlı Değil !",
  "Sorry, This Number Is Not Registered With Us !": "Üzgünüz, Bu Numara Bizde Kayıtlı Değil !",
  "Create New Account & Continue": "Yeni Hesap Oluştur ve Devam Et",
  // Auth flow — sign in / sign up
  "Sign in to your account": "Hesabınıza giriş yapın",
  "terms of services": "hizmet şartları",
  "Enter Verification Code Sent To Your": "Size Gönderilen Doğrulama Kodunu Girin:",
  "Whatsapp": "Whatsapp",
  "Resend After -": "Tekrar Gönder -",
  "Login Successfully !": "Giriş Başarılı !",
  "Welcome Back, Enjoy Our Services": "Tekrar Hoş Geldiniz, Hizmetlerimizin Tadını Çıkarın",
  "Sign Up Successfully !": "Kayıt Başarılı !",
  "Session limit reached. Try again later.": "Oturum sınırına ulaşıldı. Daha sonra tekrar deneyin.",
  // Auth flow — QR login
  "Login with QR": "QR ile Giriş",
  "Show a new code": "Yeni kod göster",
  "Login was declined on your phone": "Giriş telefonunuzdan reddedildi",
  "Scanned — confirm on your phone to continue": "Tarandı — devam etmek için telefonunuzdan onaylayın",
  "On your phone, open Settings → Linked Devices → Scan, then point the camera at this code.": "Telefonunuzda Ayarlar → Bağlı Cihazlar → Tara yolunu açın, ardından kamerayı bu koda doğrultun.",
  "This device appears to be in a different city": "Bu cihaz farklı bir şehirde görünüyor",
  // Auth flow — accessibility labels
  "Scan QR Code": "QR Kodu Tara",
  "Send phone number": "Telefon numarası gönder",
  "Continue": "Devam",

  // The login flow, worded exactly as the XD design file has it.
  "login !": "Giriş Yap !",
  "Sign up !": "Kayıt Ol !",
  "already registered !": "Zaten Kayıtlı !",
  "not registered !": "Kayıtlı Değil !",
  "enter your name !": "Adınızı Girin !",
  "Sign up successfully !": "Kayıt Başarılı !",
  "Enter your phone number registered with us": "Bizde Kayıtlı Telefon Numaranızı Girin",
  "Your privacy is completely safe": "Gizliliğiniz Tamamen Güvende",
  "We will send a verification code to the number": "Numaraya Doğrulama Kodu Göndereceğiz",
  "We have sent a verification code to the number": "Numaraya Doğrulama Kodu Gönderdik",
  "Choose verification method": "Doğrulama Yöntemini Seçin",
  "this number already registered with us": "Bu Numara Zaten Bizde Kayıtlı",
  "Sorry, this number is not registered with us !": "Üzgünüz, Bu Numara Bizde Kayıtlı Değil !",
  "Last step and enjoy our services": "Son Adım ve Hizmetlerimizin Tadını Çıkarın",
  "ensure greater security and protect your funds": "Daha Fazla Güvenlik Sağlayın ve Fonlarınızı Koruyun",
  "Enjoy with our services": "Hizmetlerimizin Tadını Çıkarın",
  "Enter your Name exact id": "Kimlikteki Adınızı Tam Olarak Girin",
  "Enter your phone number": "Telefon Numaranızı Girin",
  "I have already account": "Zaten Hesabım Var",
  "New customer": "Yeni Müşteri",
  "Later, take a look at the app": "Daha Sonra, Uygulamaya Göz Atın",
  "Cancel & take a look at the app": "İptal Et ve Uygulamaya Göz At",
  "login & Continue": "Giriş Yap ve Devam Et",
  "Create new account & Continue": "Yeni Hesap Oluştur ve Devam Et",
  "agree & continue": "Kabul Et ve Devam Et",
  "Terms of services": "Hizmet Şartları",
  ". quick preview .": ". Hızlı bakış .",
  "send WhatsApp": "WhatsApp Gönder",
  "Send Sms": "SMS Gönder",
  "edit": "Düzenle",
  "resend after -": "Tekrar Gönder -",
  "Didn’t you receive a code?": "Kod Almadınız mı?",
  "resend code": "Kodu Tekrar Gönder",
  "method": "Yöntem",
  "enter verification code sent to your": "Size Gönderilen Doğrulama Kodunu Girin:",
  "Switch From your App": "Uygulamanızdan geçiş yapın",
  "You can use your account on the web securely and easily.": "Hesabınızı web’de güvenli ve kolay bir şekilde kullanabilirsiniz.",
  "Open your Trydos application": "Trydos uygulamanızı açın",
  "choose Switch web": "Web’e geçiş’i seçin",
  "read this code from opposite side Camera": "Bu kodu karşı taraftaki kamerayla okutun",
  "To Create new account Tap “agree & Continue” to accept": "Yeni Hesap Oluşturmak İçin \"Kabul Et ve Devam Et\" Butonuna Dokunun",
  "Terms of services": "hizmet şartları",
  "The code sent has expired": "Gönderilen Kodun Süresi Doldu",
  "We Could Not Add More Of This Product": "Bu üründen daha fazla ekleyemedik",
  "Do You Want Us To Notify You When It Is Available?": "Ürün tekrar stoklara girdiğinde sizi bilgilendirelim mi?",
  "Scan this code in the RDB app": "Bu kodu RDB uygulamasında taratın",
  "Payment code": "Ödeme kodu",
  "Open the RDB app": "RDB uygulamasını aç",
  "Cancel payment": "Ödemeyi iptal et",
  "Waiting for your payment": "Ödemeniz bekleniyor",
  "Payment expired": "Ödeme süresi doldu",
  "Payment cancelled": "Ödeme iptal edildi",
  "Payment failed": "Ödeme başarısız",
  "Payment received": "Ödeme alındı",
  "Time left": "Kalan süre",
  "Amount to pay": "Ödenecek tutar",
  "You have a payment in progress": "Devam eden bir ödemeniz var",
  "Continue payment": "Ödemeye devam et",
  "Your cart is locked until you finish or cancel the payment": "Ödemeyi tamamlayana veya iptal edene kadar sepetiniz kilitli",
  "Could not start the payment. Please try again": "Ödeme başlatılamadı. Lütfen tekrar deneyin",
  "Could not cancel the payment. Please try again": "Ödeme iptal edilemedi. Lütfen tekrar deneyin",
  "This payment is already paid": "Bu ödeme zaten yapıldı",
  "Hide menu": "Menüyü gizle",
  "Device and browser info": "Cihaz ve tarayıcı bilgisi",
  "Main": "Ana menü",
  "Cart": "Sepet",
  "Chat": "Sohbet",
  "Women": "Kadın",
  "Children": "Çocuk",
  "Electronic": "Elektronik",
  "Search anything…": "Her şeyi ara…",
  "Voice search": "Sesli arama",
  "Scan": "Tara",
  "For you": "Sana özel",
  "Empty": "Boş",
  "Your cart is empty": "Sepetiniz boş",
  "Items you add to your cart will show here": "Sepetinize eklediğiniz ürünler burada görünecek",
  "No conversations yet": "Henüz sohbet yok",
  "Messages with sellers will show here": "Satıcılarla mesajlarınız burada görünecek",
  "lock": "kilit",
  "client ID": "müşteri kimliği",
  "Enter Address !": "Adres girin !",
  "Add photo": "Fotoğraf ekle",
  "Setup your size profile": "Beden profilini oluştur",
  "To get smart recommendations that fit you.": "Size uygun akıllı öneriler almak için.",
  "Setup size now": "Bedeni şimdi ayarla",
  "Unverified account": "Doğrulanmamış hesap",
  "Tap verify, skip the line, and enjoy exclusive offers.": "Doğrula'ya dokun, sırayı atla ve özel tekliflerin tadını çıkar.",
  "Verify now": "Şimdi doğrula",
  "Get trydos Business": "trydos Business'a geç",
  "Start Your Business now": "İşini şimdi başlat",
  "action": "işlem",
  "Login history": "Giriş geçmişi",
  "logout": "çıkış yap",
  "Profile Photo": "Profil Fotoğrafı",
  "Add & save": "Ekle ve kaydet",
  "Choose": "Seç",
  "Uploading profile photo …": "Profil fotoğrafı yükleniyor …",
  "copy": "kopyala",
  "download": "indir",
  "share": "paylaş",
  "client name": "müşteri adı",
  "client Phone Number": "müşteri telefon numarası",
  "client Info": "müşteri bilgisi",
  "Body measurements": "Vücut ölçüleri",
  "client Information": "müşteri bilgileri",
  "client Status": "müşteri durumu",
  "client since": "müşteri olma süresi",
  "client type": "müşteri türü",
  "client Verified": "müşteri doğrulaması",
  "days": "gün",
  "Personal": "Kişisel",
  "Change my Phone Number Request": "Telefon numaramı değiştirme talebi",
  "Delete my account Request": "Hesabımı silme talebi",
  "full Name": "tam ad",
  "enter Email Address": "e-posta adresini girin",
  "verification Email !": "e-posta doğrulama !",
  "enter verification code sent to your Email": "e-postanıza gönderilen doğrulama kodunu girin",
  "resend after": "tekrar gönder",
  "How tall are you?": "Boyunuz kaç?",
  "What is your weight?": "Kilonuz kaç?",
  "What is your Foot size?": "Ayak numaranız kaç?",
  "Add Shipping Address": "Teslimat adresi ekle",
  "Your address list is empty": "Adres listeniz boş",
  "You can also create multiple addresses to use": "Kullanmak için birden fazla adres de oluşturabilirsiniz",
  "Delete below address ?": "Aşağıdaki adres silinsin mi ?",
  "Sure, Delete": "Evet, sil",
  "Turkiye": "Türkiye",
  "Select from list": "Listeden seç",
  "Detailed address": "Açık adres",
  "Street address, building, Flat, Door, unit.": "Sokak, bina, daire, kapı, birim.",
  "Address title": "Adres başlığı",
  "Ex: Home, my office, 2 home ect.": "Örn: Ev, ofisim, 2. ev vb.",
  "your location on map": "haritadaki konumunuz",
  "Locate your location on map": "Konumunuzu haritada belirleyin",
  "location is accurate, making it easy to receive shipments": "doğru konum, gönderileri teslim almayı kolaylaştırır",
  "District": "İlçe",
  "Street": "Sokak",
  "Entering your information correctly allows us to provide you with better service and benefit from all services.": "Bilgilerinizi doğru girmeniz, size daha iyi hizmet sunmamızı ve tüm hizmetlerden yararlanmanızı sağlar.",
  "Why add a address?": "Neden adres eklemelisiniz?",
  "Accurate address details ensure faster delivery of your correct products and show you the best options in your area.": "Doğru adres bilgileri, doğru ürünlerinizin daha hızlı teslim edilmesini sağlar ve size bölgenizdeki en iyi seçenekleri gösterir.",
  "Learn more": "Daha fazla bilgi",
  // Chat: message edit, tags, reminders, archive and unread.
  "Forwarded": "İletildi",
  "Edited": "Düzenlendi",
  "Edit message": "Mesajı düzenle",
  "Tag message": "Mesajı etiketle",
  "Remind me": "Bana hatırlat",
  "Reminder set for": "Hatırlatma zamanı",
  "Cancel reminder": "Hatırlatmayı iptal et",
  "In 20 minutes": "20 dakika sonra",
  "In 1 hour": "1 saat sonra",
  "In 3 hours": "3 saat sonra",
  "Tomorrow morning": "Yarın sabah",
  "Pick a date and time": "Tarih ve saat seçin",
  "Set": "Ayarla",
  "Choose a time in the future": "Gelecekte bir zaman seçin",
  "Choose an earlier time": "Daha erken bir zaman seçin",
  "Urgent": "Acil",
  "Important": "Önemli",
  "To do": "Yapılacak",
  "Unarchive": "Arşivden çıkar",
  "Archived": "Arşivlenenler",
  "Reminders": "Hatırlatmalar",
  "No archived chats": "Arşivlenmiş sohbet yok",
  "No reminders": "Hatırlatma yok",
  "Message": "Mesaj",
  "Tagged messages": "Etiketli mesajlar",
  "Could not load the messages": "Mesajlar yüklenemedi",
  "No messages with this tag": "Bu etikete sahip mesaj yok",
  "Failed to edit the message": "Mesaj düzenlenemedi",
  "Failed to update the tag": "Etiket güncellenemedi",
  "Failed to set the reminder": "Hatırlatma ayarlanamadı",
  "Failed to cancel the reminder": "Hatırlatma iptal edilemedi",
  "Failed to archive the chat": "Sohbet arşivlenemedi",
  "Failed to unarchive the chat": "Sohbet arşivden çıkarılamadı",
  "Failed to mark the chat as unread": "Sohbet okunmadı olarak işaretlenemedi",
};

export default translations;


